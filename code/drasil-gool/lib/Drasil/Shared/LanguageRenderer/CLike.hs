...
litTrue :: (CommonRenderSym r tp) => SValue r
litTrue = mkStateVal IC.bool (text "true")

litFalse :: (CommonRenderSym r tp) => SValue r
litFalse = mkStateVal IC.bool (text "false")

litFloat :: (CommonRenderSym r tp) => Float -> SValue r
litFloat f = mkStateVal IC.float (D.float f <> text "f")

inlineIf :: (CommonRenderSym r tp) => SValue r -> SValue r -> SValue r -> SValue r
inlineIf c' v1' v2' = do
  c <- c'
  v1 <- v1'
  v2 <- v2'
...
libFuncAppMixedArgs :: (CommonRenderSym r tp) => Library -> MixedCall r tp
libFuncAppMixedArgs l n t vs ns = modify (addLibImportVS l) >>
  IC.funcAppMixedArgs n t vs ns

libNewObjMixedArgs :: (OORenderSym r tp) => Library -> MixedCtorCall r tp
libNewObjMixedArgs l tp vs ns = modify (addLibImportVS l) >>
  IG.newObjMixedArgs tp vs ns

-- Functions --

listSize :: (OORenderSym r tp) => String -> SValue r -> SValue r
...
increment :: (CommonRenderSym r tp) => SVariable r -> SValue r -> MSStatement r
increment vr' v'= do
  vr <- zoom lensMStoVS vr'
  v <- zoom lensMStoVS v'
  mkStmt $ R.addAssign vr v

increment1 :: (CommonRenderSym r tp) => SVariable r -> MSStatement r
increment1 vr' = do
  vr <- zoom lensMStoVS vr'
  (mkStmt . R.increment) vr

decrement1 :: (CommonRenderSym r tp) => SVariable r -> MSStatement r
decrement1 vr' = do
  vr <- zoom lensMStoVS vr'
  (mkStmt . R.decrement) vr

varDec :: (OORenderSym r TypeData, UnRepr r TypeData, TypeElim r TypeData) =>
...
varDecDef :: (CommonRenderSym r tp) => Terminator -> SVariable r ->
...
setDecDef :: (CommonRenderSym r tp) => Terminator -> SVariable r ->
  r ScopeData -> SValue r -> MSStatement r
...
listDec :: (CommonRenderSym r tp) => (r (Value r) -> Doc) -> SValue r ->
...
extObjDecNew :: (OORenderSym r tp) => Library -> SVariable r -> r ScopeData ->
...
switch :: (CommonRenderSym r tp) => (Doc -> Doc) -> MSStatement r -> SValue r ->
...
for :: (CommonRenderSym r tp) => Doc -> Doc -> MSStatement r -> SValue r ->
...
while :: (CommonRenderSym r tp) => (Doc -> Doc) -> Doc -> Doc -> SValue r ->
...
intFunc :: (OORenderSym r tp) => Bool -> Label -> r (Visibility  r) ->
