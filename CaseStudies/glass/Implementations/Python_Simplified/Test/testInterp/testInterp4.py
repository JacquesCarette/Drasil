...
class TestInterp(unittest.TestCase):
    ...
    def test_lin_interp(self):
        y = interp.lin_interp(-5.6, -0.5, -3.4, 2, -4.5) # x < x2 < x1
        self.assertEqual(y, -6.6388888888888888)

    def test_find_bounds(self):
        idx, jdx, kdx, num_interp1, num_interp2 = interp.find_bounds(self.data1, self.data2, 200, 4.7)
        self.assertEqual(idx, 3)
        self.assertEqual(jdx, 3)
        self.assertEqual(kdx, 0)
        self.assertEqual(num_interp1, 1)
        self.assertEqual(num_interp2, 1)

    def test_interp(self):
        interp_value = interp.interp(3, 3, 0, 1, 1, self.data1, self.data2, self.data3, 200, 4.7)
        self.assertEqual(interp_value, 5.3806451612903219)
