...
\newcommand{\Gl}{\textsf{GlassBR}\xspace}
...
%%%%%%%%%%%%%%%%%%%%%
% Tikz Styles
\tikzstyle{class}=[rectangle, draw=black, rounded corners, fill=white!40,
  drop shadow, text centered, anchor=north, text=black, text width=3cm]
...
RESULTS: Case studies -- \Gl to show capture and transformation. SWHS and
...
\ds{Also note: Should point out maintainability/traceability/design for change
somewhere in here, since it is relevant later on.}
...
\wss{In the scope section we should clarify that our generator will focus on the
  ...
  should help.}
\ds{I'll start that (removing assumptions and trying not to imply things)
discussion with something along the lines of "For the sake of transparency..."}
...
To generate all software artifacts, we must first enumerate all the kinds
of artifacts in scope, and then understand how they are written.
We have chosen to focus on the Smith Et Al. templates
\cite{Smith????} \ds{grab proper citation later}, as
we have access to a number of case studies using these templates.
...
\jc{I find the above introductory sentence almost devoid of useful information}
...
Up first is \Gl -- software created for
checking whether a given pane of glass will be able to withstand the force of a
nearby explosion. It has a number of glass and explosion-related parameters
which are used to compute the probability that the glass will break. We will
use \Gl as a
running example throughout this paper, with asides to illustrate
particular features found in the other case studies.

\ds{Figure of the \Gl system for context}

The next two -- \emph{SWHS} and \emph{NoPCM} -- are members of the
...
Chipmunk2D
game physics engine (\url{https://chipmunk-physics.net/}). We chose this
project specifically for its rich use of
physics knowledge, but as it is a very large physics library, we narrowed the
scope for this case study.

There is also a trivial example, affectionately nicknamed \emph{tiny}, which
is taken from modelling a fuel-pin inside a nuclear reactor. This toy
example helped bootstrap Drasil development.
...
\emph{mean}. From this, we can extrapolate these commonalities to other
pieces of SCS. One strikingly obvious commonality in SCS
is the use of the Syst\`{e}me International d'unit\'{e}es (SI) and derived units.
...
\jc{This is the 'wrong' way to tell this part of the story: of course the same knowledge
is present, it would be crazy otherwise. It is what knowledge is duplicated,
its level of granularity, and its transformations which are interesting}
...
see the same knowledge from \Gl's SRS and MG. \ds{or is it MIS?
Double-check.}

The SRS displays more knowledge related to the abstract theory, while the
\ds{MG} provides a view meant to transition from the abstract to something
...
\jc{this subsection feels too shallow to me. The list of artifacts is fine,
but the 'investigation' into the commonalities is not. What point are we
really trying to make here -- and are we making it? To really show this,
I think we need 3, preferably 4, quite distinct examples (all in \Gl) of
...
knowledge that have been composed from a variety of parts. Each part
...
\caption{Data Definition for !FIXME: Load or something?! from \Gl SRS}
\label{Fig:GlassBRDDSRS}
\end{figure}

\begin{figure}
%Fig goes here. Or lsting
\caption{Data Definition code from \Gl implementation}
\label{Fig:GlassBRDDCode}
\end{figure}

If we take a look at one particular example \ds{name it} across artifacts from
\Gl (Figures~\ref{Fig:GlassBRDDSRS},\ref{Fig:GlassBRDDCode}), we can see
...
\jc{I agree. We need the examples to see if that list is what we came up post
facto, or could be recognized as valid by a reader}
...
and nothing else. \jc{id actually worries me a lot: it feels like a design
decision (which can, and should be, justified)}
The symbolic representations are just the symbols we use when
...
\jc{And these are also rational reconstructions. So we have to guide the 
reader into seeing them. For example, the 2 symbols.}
...
\wss{Going through the \Gl example like this seems like a good motivating
...
\jc{Right. But please be systematic. ``similar examples of knowledge'' and
so on doesn't say much to the reader. Plus we need to show the commonalities,
by showing different kinds of explicit knowledge, so that when we say that
``they all have X'', the reader will believe us.  Not everything from the
table should be shown, of course, but more.}
...
\jc{Table is good. Lack of definition for each of the things in the
header row and header column, bad.}
...
\jc{Is the table up to date wrt current Drasil?}
\end{table}
...
existing classes.
\jc{Also not a good way to tell the story. We invented all of these. So
we need to reason out why we did. You're doing it backwards, as if these
were all obvious and natural (or were grabbed from somewhere else) and
justifying their use and adequacy post facto}
...
mandatory symbol.
\jc{This will make no sense to any reader. If you want to make this point,
show it via an explicit example}

\section{Our solution -- A combined approach}
\label{S:solution}

\jc{And this section does not ``logically follow'' from the previous,
as the previous section is not solely an analysis of the problem, but
contains many pieces of solution (and design decisions). If you want
to arrange things in this way, you need to make sure that the previous
section is 100\% in the problem space, and presents domain analysis
that could have been done purely on the original case studies (by hand)
without writing a single line of Haskell.}
...
a symbol associated with it, however, a \emph{Quantity must} have a
...
appropriate transformation.  \wss{It would be great if we could show an example
  ...
  this below.} \ds{Something like that, yes. I'd like to show off (particularly
  when dealing with Kolmogorov complexity) that NoPCM is using a lot of things
  from SWHS}
...
in more depth in Section~\ref{S:DrasilBackEnd}.
...
\subsection{The Generator \& Back-end DSLs}
\label{S:DrasilBackEnd}

Drasil is composed of a number of DSLs working together to generate software
artifacts. While there are other DSLs within Drasil, namely the recipe language
(See Section~\ref{S:RecipeLang}), they are not exactly necessary for generating
documentation and source code -- they provide useful high-level abstractions.
The required DSLs for generating documentation and source code using Drasil are
the:
\begin{itemize}
\item Expression language
\item Sentence specification language
\item Knowledge-capture language
\item Document layout language
\item (Modified) Generic Object Oriented Language (GOOL) framework
\item Printing translation language
\item LaTeX and HTML printers
\end{itemize}
%What follows is a fairly dry overview of each of these DSLs.

The expression language is used for capturing mathematic expressions, such as
defining equations. With it we can capture equations, relations, and more. We
have included a host of standard unary, binary, and $n$-ary arithmetic,
trigonometric, boolean, and matrix operations. For example, the equation
captured in Drasil for linear displacement (from the game physics example)
$\mathbf{r}(t)=\frac{d\,\mathbf{p}\left(t\right)}{d\,t}$ can be seen in
Figure~\ref{Fig:ExprEq}. The equation is simply defined as a derivative of the
position function with respect to time. At this point in time, the expression
language is untyped, however, we recognize the advantages of using a typed
expression language and intend to implement expression typing in the future.

\begin{figure}
\lstinputlisting[firstline=64,
lastline=65]{../../code/drasil-example/Drasil/GamePhysics/DataDefs.hs}
\caption{Linear displacement equation
$\mathbf{r}(t)=\frac{d\,\mathbf{p}\left(t\right)}{d\,t}$ in Drasil}
\label{Fig:ExprEq}
\end{figure}

The sentence specification language is used for capturing knowledge at the
``sentence" level. These are typically English-language sentences which combine
captured knowledge in different ways. We can build mixtures of strings
with knowledge-level or document-layout-level references, expressions, symbols
and special characters, units, and any other captured information. An example
sentence (shortened for the sake of brevity) can be seen in
Figure~\ref{Fig:Sentence}. Note the use of references to existing pieces of
knowledge (\verb|plate_len|, \verb|plate_width|, \verb|millimetre|, and
\verb|metre|) allowing us to look up the relevant symbols ($a$ and $b$ for
\verb|plate_len| and \verb|plate_width| respectively) and other information on
generation to maintain consistency. The binary infix constructor \verb|:+:| is
used for sentence concatenation.

\begin{figure}
\begin{lstlisting}
inputGlassPropsDesc :: Sentence
inputGlassPropsDesc = -* \dots{} *- :+:
...
\end{lstlisting}
\caption{A sentence captured in Drasil}
\label{Fig:Sentence}
\end{figure}

The knowledge-capture language is the collection of data structures, smart
constructors, lenses, transformations, and other functions related specifically
to capturing and operationalizing knowledge. The knowledge capture language
uses the expression and sentence specification languages as building blocks for
capturing different properties. For example, a defining equation like that seen
in Figure~\ref{Fig:ExprEq} will be captured as one property of a chunk using
the expression language, whereas the English-language term referring to the
same chunk will be captured using the sentence specification language. See
Section~\ref{S:DrasilKC} for more of the implementation details.

The document layout language is where things really start to come together.
This is the lowest-level specification for how a document artifact should be
pieced together. A \emph{Document} is simply defined as a data structure which
includes a title, author(s), and a list of (one or more) sections. The sections
are then composed of (sub-)sections and contents (tables, figures, paragraphs,
lists, equations, etc.) which are in turn composed of sentences and
expressions.

The document layout language is \emph{not} meant to be used to
generate artifacts directly -- it is far too naive and very tedious to
use. We show evidence of this in Figure~\ref{Fig:DumbDoc} which shows the
effort required to create a document given a simple title, authors, and an
introduction section composed of an introductory paragraph and an empty
``Scope" subsection.

It seems clear that this approach is not much better than using tools
like LaTeX macros, other than our ability to print in both
LaTeX and HTML. The only real benefits of assembling documents at this level
are the highly-specific control of the layout and the
consistent-by-construction output when (re-)using captured knowledge. Alone,
that is not enough, hence why we need higher level abstractions (see
Section~\ref{S:RecipeLang}).
...
-* \dots \emph{omitted for brevity} *-
\end{lstlisting}
\caption{Assembling a document using only the low-level document language is tedious}
\end{figure}

The printing translation language and printers are very closely related. 
The translation language is a mid-level representation between the
higher-level semantic constructs in the document, sentence, and expression
languages and the lower-level printable HTML and LaTeX syntax.
The printers themselves deal with printing the actual HTML and LaTeX found in
the generated artifacts, based solely on the translation-level constructs.

Finally, GOOL~\cite{Costabile2012} is a generic object-oriented language used
to generate source code for a number of different programming languages
(Python, Java, Objective C, C++, C\#, and Lua) from a more abstract
specification. As we like to practice what we preach, we have chosen to
incorporate a modified version of GOOL into Drasil to facilitate code
generation without reinventing the wheel.

\subsection{Knowledge Capture \& Transformations in Drasil}
\label{S:DrasilKC}

Before we get into the specifics of transforming knowledge in Drasil, we should
first explain how knowledge is captured. We originally borrowed the idea of
\emph{Chunks} from Literate Programming, but have since expanded our knowledge
capture mechanism to more closely resemble the classification scheme mentioned
in Section~\ref{S:KnowCapt}, specifically following the property-based
knowledge classes seen in Figure~\ref{Fig:ChunkHierarchy}.

Knowledge is captured in a number of various data structures representing the
common combinations of properties we have observed. For instance, a
\emph{QuantityDict} (Quantity dictionary), is shown in
Figure~\ref{Fig:QuantityDict}.
...
\end{lstlisting}
\caption{The \emph{QuantityDict} data structure definition and class instances}
\label{Fig:QuantityDict}
\end{figure}

To understand exactly what is being captured, we need to first look at the
definition of an \emph{IdeaDict}, which is itself composed of a
\emph{NamedChunk} and may have an abbreviation. A \emph{NamedChunk} has a
Unique IDentifier (UID) and a term -- it is the minimal complete definition for
the \emph{NamedIdea} class of properties. The \emph{IdeaDict} then is the
minimal complete definition for the \emph{Idea} class. Finally, the
\emph{QuantityDict} also has a space (domain/type), symbol, and may have units.
Thus it is the minimal complete definition of the \emph{Quantity} class.

Since Haskell is strictly-typed, having a number of different data structures
and expecting them to behave with each other can be difficult. As such, we have
implemented each class property using lenses. For those unaware, lenses are
composable functional references -- they allow us to read and write data
deep within our data structures. Combining lenses with Haskell's type classes
allows us to capture our knowledge in a variety of data structures, then use
the lenses to project out only that which is relevant in a given context, or
related to a specific property. Classes and lenses work in tandem to give us
data-structure-agnostic setters and getters.

We have already started to capture knowledge from several areas into our
knowledge base. A brief, high-level overview can be seen in
Figure~\ref{Fig:CaptKnowAreas}. While we have started capturing knowledge, we
have not strayed far from the specifics of our case studies. Our knowledge-base
should by no means be considered an ontology or even somewhat comprehensive
collection of knowledge at this time. It is quite interesting even, to see how
little we have captured, yet how much we can achieve.

%TODO: Work in "People.hs" somewhere in here

\begin{figure}
\caption{Knowledge areas we have started to capture (See: SE-CSE paper) \ds{I
can't find this in the old papers, hmm, oh well, I know what to draw}}
\label{Fig:CaptKnowAreas}
\end{figure}
...
sentence or description block. An example of the generated output can be seen in
Figure~\ref{Fig:GlassBRDDSRS}.

\subsection{The Recipe Language}
\label{S:RecipeLang}

The recipe language in Drasil is known under the name
\texttt{Drasil.DocumentLanguage}. While it is not necessary to generate
documentation artifacts, it provides a number of higher-level semantic
constructors for combining document-specific layout information alongside
document meta-knowledge (captured in \texttt{Data.Drasil}).

The recipe language essentially defines new custom data-types specific to the
documentation artifact we wish to generate. We are creating representations
which combine (document) layout information with system information (captured
knowledge) in a meaningful way. Alongside the recipe language, the system
information is used to define exactly what knowledge is relevant to a
particular software family member.

Within our \emph{SystemInformation} data structure we maintain a database of 15
different chunk classes (all the quantities, concepts, inputs, outputs,
constraints, constants, etc.) required to generate (all) our artifacts for the
given piece of software. Without a recipe, however, these chunks are
essentially useless -- Drasil can not do anything with them without a recipe.
We need at least one recipe for each of the artifact types we wish to create.
Let us consider the SRS as our running example for the rest of this section.

\texttt{Drasil.DocumentLanguage} currently contains one recipe for
generating the SmithEtAl SRS template. We have created custom, meaningful
constructors for the different document sections relevant to the current
context of the SRS, see Figure~\ref{Fig:DocSection} for the top-level
constructors. Each of these top-level sections, with the exception of Verbatim,
can be decomposed in a meaningful way, thus allowing us to specify the
subsections we would like to include as well.

\begin{figure}
\lstinputlisting[firstline=52,
lastline=66]{../../code/drasil-docLang/Drasil/DocumentLanguage.hs}
\caption{Our top-level SRS recipe section constructors}
\label{Fig:DocSection}
\end{figure}

With our recipe, we enforce certain conventions, for example subsections can
only belong to the appropriate parent sections as outlined in the original
template. We have attempted to enforce the bare minimum necessary to maintain a
cohesive document, allowing for a greater level of customization for the user.
Options we give to our users include such trivial things as customizing the
order sections should appear (for example, should the reference information be
up-front, somewhere in the middle, or at the end?). We also include less
trivial decisions such as the verbosity of description fields (should all
relevant symbols be defined in the context of a defining equation,
or only the symbol(s) defined by that equation?), or whether to include model
derivations.

An example of the SRS recipe in use can be seen in
Figure~\ref{Fig:GlassBRSRSDL} and is explained further in
Section~\ref{S:GlassBR}.

\subsection{Packaging Drasil}

Since Drasil has a lot of moving parts, it may be difficult to see where to
start or how things fit together. We have attempted to simplify this by
splitting the current version (0.1.23 as of this writing) of Drasil into a
group of Haskell packages (see Figure~\ref{Fig:Packages}).
...
\section{Case studies reimplemented in Drasil}

Drasil's development has relied heavily on the case studies, as mentioned in
Section~\ref{S:Drasil}. Our current selection of case studies
(Section~\ref{S:IntroCases}) were determined by a number of factors including
their sizes, domains, availability (both of source code and of documentation
artifacts), and selection variety. At the very least, we needed various, small
case studies which included at least two members of the same software family.

Our case studies give us a finish line. If we cannot generate our examples with
at least the same level of quality as the original manually-created versions,
then Drasil is not useful. \ds{Reword previous.} Reimplementing
these case studies is our guide through development, and it is not as simple as
it seems.

The first task in reimplementing the case studies is understanding exactly what
is system-specific information versus common knowledge. Our litmus test for
common knowledge was ``is this knowledge a fundamental concept of [domain]" or
``does this piece of knowledge occur in at least two
[non-family member] examples?" The distinction of ``non-family member" was
necessary to ensure we didn't encode \emph{all} knowledge relating to a given
example as being ``common knowledge", since it may only be specific to the
given software family and not truly common across the domain.

Our distinction between common knowledge and specific knowledge in the case
studies is not ideal given our small sample size, but it is a start. We may
eventually classify more of the system-specific knowledge as
common knowledge if it crops up in a number of future examples.

Next, we need to differentiate artifact-template boilerplate text from
meaningful text. This is a simple, but not easy task involving a cycle of
abstracting out the meaning of a given sentence, paragraph, section, etc. and
determining what, if any, specific knowledge is being used therein.

The abstraction process is what led us to the Drasil.DocumentLanguage recipe
for our SRS (Section~\ref{S:RecipeLang}). At times it was as simple as pulling
out a sentence as ``boilerplate text", or a slightly more complex ``choice
between boilerplate text options". Options include trivial things
such as how to represent vectors (are they shown in bold, italics, or something
else?), what type of source is being referenced in a given section (existing
literature, documentation, or manuals), or which combination of boilerplate
text to use. Other cases are more complex, where a sentence includes specific
information related to the system. By abstracting out the meaning of the
sentence, we see the specific information is only required for a singular
purpose. For example, the section ``characteristics of the intended
reader" from the SmithEtAl SRS template has a fairly self-explanatory meaning,
but the specifics will vary greatly depending on the system being implemented.

Finally, the only way to know that our reimplementation is truly on the right
track is to have something to test it against. To that end, we created a very
simple test suite using continuous integration tools to ensure after each
update to the Drasil framework or our reimplemented examples, we could ensure
the generated output remains as expected. Overall, the tests were a simple
series of \texttt{diff}s between the manually-produced case studies and the
generated output. Any difference was considered a test failure, even though
that was often not the case. We often found improvements in the
Drasil-generated output, which in turn led us to incorporate these improvements
into our test-suite.

Now let us look at some of the specifics of the reimplementation process. First
we will introduce the common knowledge database (\texttt{Data.Drasil}) and some
of its components, then we will take a look at the \Gl example
reimplemented in Drasil. Following that we will discuss reimplementing the
other case studies, particularly SWHS and NoPCM.
...
\ds{Somewhere in here (probably in \Gl) I need to really emphasize the
separation of System Information from the DocLang. It comes up later.}
...
Important to note we have not captured even a minute fraction of all the domain
knowledge, yet the results with our limited knowledge-capture already speak for
themselves.

\subsection{GlassBR}
\label{S:GlassBR}

- Brief intro to problem \Gl is solving - how it works
- Show off the doc language here
  \fgr{} \Gl SRS in (truncated) DocLang format
...
We will now discuss the results of what we have observed thus far. In general,
had the manually written versions of these case studies attempted to be
certified, they would (and should) have failed.

Overall there were a number of common problems, alongside some case-study
specific issues, many of which had gone unnoticed for years. It was not until
the case studies were re-developed in Drasil that they were finally resolved.

We will also discuss other observations and enhancements provided by Drasil
that we hope will lead to long-term quality improvements for the generated
software.

\wss{I like this Results section - examples like these are something people can
  understand and hopefully be convinced by.}

\subsection{Freebies - Compliments of System Information} \ds{Change the title}

We have reduced the burden of knowledge capture for a project by separating the
system information from the layout specification and template boilerplate for
our artifacts. While this in itself is a good start, it also gives us a number
of convenient, and free, perks.

First off, thanks to the recipe language we can generate a number of sections
manually. One of the most tedious sections to write and maintain is that of
reference material. Manually maintaining the tables of symbols, units, and
abbreviations and acronyms is necessary, but ultimately uninteresting.
By separating out the system information we have a smaller, more maintainable
data structure from which we can generate all of the previously mentioned
tables (as well as many other sections and subsections).

Let us first look at the \emph{tiny} case study. As our first toy-example, it
lacked the rigour of the other case studies. Particularly, there was no
reference information section, only a table of units. However, since Drasil
takes the monotony out of creating a reference information section, one is
currently being generated in our software artifacts. See
Figure~\ref{Fig:tinyRef} for a comparison of the original version of tiny to
the freebie-inclusive one.

\begin{figure}
\caption{Tiny before (left) and after (right) use of Document Language and
System Information}
\label{Fig:tinyRef}
\ds{This might need to be an appendix since I'm not sure how to fit it in here}
\end{figure}

Not only does the generator populate the reference material from our system
information, it also provides a number of sanity-checks. First and foremost, if
a symbol is not defined, it cannot be used anywhere else within the document.
This actually extends to cross-referencing as well, only pieces of knowledge
that have been properly captured and added to the system context can be
referenced.

There are other sanity-checks being implemented, such as guarding against
captured constraints and ensuring unit consistency in equations, but those
features are as yet incomplete. We can already see instances of where they
would be very useful, and an example related to unit consistency will be given
in Section~\ref{S:SSPResults}.

With that in mind, we can still see the benefits of running even the most
trivial sanity-checks every time we generate our artifacts. We no longer need
to worry if the reference material will remain consistent with the rest of the
artifact, as if a system is modified, we can ensure that no requisite
definitions were accidentally removed.

\ds{Move - Bibliography - to section explaining System Information as a
concept?}
...
Otherwise, the most common issue to crop up across the case studies was that of
implicit information. There is tacit knowledge (for domain experts) and
undocumented assumptions in a number of the manually-written case studies.
Identifying and capturing that knowledge is necessary when attempting to use
Drasil, otherwise, there is no way to properly define the system relative to
its domain.

\subsection{NoPCM and SWHS}

As previously mentioned, NoPCM and SWHS are members of the same software
family. They share a lot of the same core knowledge. It should be noted that 
NoPCM was manually created after SWHS.

When implementing NoPCM in Drasil, we found PCM-related knowledge that should
not have existed in the case study. It is likely that in the manual version,
some information was copied directly from the SWHS case study and pasted into
the NoPCM documentation.

This information was trivial to remove in the Drasil
version, as a number of the PCM-related symbols were not defined and ended up
causing errors during the sanity-checking portion of artifact generation. The
PCM-related concepts were not defined in the context of NoPCM, and as such were
not in the table of symbols or other reference material. However, prior to
implementing our sanity-checking, the PCM-related concepts did actually occur
in Drasil, namely due to the lack of a means to specifically exclude knowledge
that \emph{should not} exist in a given project. That may be something worth
looking into for specific software family-related knowledge, however, on the
whole it is not feasible to force developers to specify excluded knowledge for
every project.

Another interesting result was the reduction in Kolmogorov complexity across
these two case studies. As of this writing the total number of 80-character
lines of code written in Drasil, including import blocks and some deprecated
code, needed to generate both SWHS and NoPCM is 4460 (ignoring whitespace and
comments). This value ignores all back-end packages that define the
languages used to write the case study examples in Drasil, as they are not part
of the case study knowledge or recipe itself. However, to ensure we are getting
an upper bound on the number of lines of code required, this number also
includes the entirety of our knowledge base (\emph{Data.Drasil}).

The generated artifacts for SWHS and NoPCM contain a total of 13695 lines of
code (again, ignoring whitespace and comments). This is again, an upper limit,
as there are many lines of boilerplate, particularly in the HTML, that could be
condensed. However, we also do not limit the length of lines in the
generated output, many of which far exceed 80 characters. Overall, we need only
write less than one third of the lines of code to produce the same output!

If we consider each of NoPCM or SWHS independent of the other, the results are
still interesting, but less so. For SWHS the amount of Drasil code written
(again, including \emph{Data.Drasil}) is 3678 and the number of generated lines
for that example is 8550. While this is still a reduction of over 50\%, we can
see there is a substantial benefit when producing multiple software family
members.

Finally, the code generator is still in the process of being updated, so we are
not generating any source code for SWHS. Thus the 13695
figure \emph{will} increase, while the 4460 (or 3678) figure should remain near
constant. We can support this claim as our NoPCM example is currently
generating source code and it requires less than 100 dedicated lines of code in
Drasil to do so (included in the above figure of 4460).

We hope these results will lead to an overall reduction in the time required to
create and maintain software artifacts while using Drasil, particularly for
software families, since there will be less code to manage. However, we make no
claim to that effect without experimental evidence to support it. We
will discuss plans for experiments in Section~\ref{S:Future}.

%LoC Count excluding whitespace and comments, but including imports for source
%NoPCM: 782 lines (incl. imports)
%SWHS: 2284 ""
%Data: 1394 ""
%Total: 4460

%LoC Count excluding whitespace and comments, but including imports for gen'd
%NoPCM TeX, HTML, CSS, src, and makefiles: 829 + 3466 + 89 + 740 + 21 = 5145
%SWHS TeX, HTML, CSS, src, and makefiles: 1340+7100+89+0+21 = 8550
%Total: 13695

\subsection{SSP}
\label{S:SSPResults}
A major inconsistency was found in the SSP case study regarding three symbols:
$S_i$, $P_i$, and $\tau{}_i$. In the table of symbols, $S_i$ and $P_i$ were
defined as shown in Figure~\ref{Fig:SiPi} and there was no instance of a
definition for $\tau{}_i$.

\begin{figure}
\fbox{\includegraphics[width=\textwidth]{sipi.png}}
\caption{Definitions of $P_i$ and $S_i$ from the SSP case study}
\end{figure}

The lack of definition was a simple enough problem to find, as mentioned in
Section~\ref{S:CommIssues}. However, there was a second, more insidious problem
to be solved: $\tau{}_i$ actually represented $S_i$ and at some point $S_i$ and
$P_i$ had their definitions swapped! If you are interested in the full details
of the problem and solution, they can be found at
\url{https://github.com/JacquesCarette/Drasil/issues/348}.

This seemingly minor error in consistency was only caught after Drasil alerted
us that $\tau{}_i$ had not been defined. We then traced through the equations
using $\tau{}_i$ to determine its meaning. It took several passes through the
manually-written documentation to determine all of the requisite changes.
First, the definition of $\tau{}_i$ was added, then the equations were
double-checked for correctness, and any place where $S_i$ and $P_i$ were
swapped had to be identified.

Another issue arose while fixing the symbol-mixup situation: there was an
implicit assumption in one of the equations which caused inconsistencies in the
units of the output. Namely, there is an assumption that a particular force per
meter is being applied over one unit of width ``into the page", thus changing
the expected units from $\frac{N}{m}$ to $N$.

Overall, the changes required in the Drasil source were minimal. A definition
and assumption were added and a few symbols were shuffled. Yet there was an
immediate improvement in the quality of the documentation, particularly with
...
then were it hiding in some corner of one artifact.
...
\wss{Related to the previous \ds{now next} point, the act of formalizing
the knowledge that goes into the requirements documentation forces us to deeply
understand the distinctions between difference concepts, like scope, goal,
theory, assumption, simplification, etc.  With this knowledge we can improve
the focus and effectiveness of existing templates, and existing requirements
solicitation and analysis efforts.  Teaching it to a computer.}

\section{Future Work} \ds{This might end up being really long}
\label{S:Future}

\wss{Once we are capable of true variability in the documentation, we can
...
%Definitely need to keep experiments in mind here! See SWHS/NoPCM section for
%details.
...
- More recipes for current artifacts (ie. not just SmithEtAl template), and for
