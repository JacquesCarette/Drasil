...
assumpS11 = foldlSent [
  ...
  tempPCM)), S "can only increase" `sC` S "or remain",
...
assumpS16 = foldlSent [
  ...
  S "; therefore" `sC` phraseNP (the volHtGen) `S.is` S "zero"]
assumpS17 = foldlSent [
...
assumpS20 = foldlSent [
  ...
  `sC` phraseNP (vol `the_ofThe` coil) `S.is` S "assumed to be negligible"]
