...
pcmUnits = map uw [coil_SA,hIn_SA,hOut_SA,htCap,htCap_Liq,htCap_W,tank_D,ht_gen_vol,
  ht_xfer_co,ht_xfer_CW,tank_L,water_m, -- norm_vect, 
  ht_flux, thFluxVect,
  ...
  water_vol,water_dense,dummyVar]
...
tank_L      = uc' "tank_L" (length `of_` tank) fixme cL metre
...
temp_coil   = uc' "temp_coil" (temperature `of_` coil)
...
temp_water  = uc' "temp_water" (temperature `of_` water)
...
t1descr = 
  ...
  (P $ htCap ^. symbol) :+: S " and density " :+: (P $ QPP.density ^. symbol) :+:
