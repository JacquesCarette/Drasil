...
We believe it is beneficial to conduct a commonalty analysis for all four selected libraries, because the Drasil framework want to generate a program family. A Program families~\citep{dp1976} is a sets of programs whose common properties are so extensive that it is advantageous to study the common properties of the programs before analyzing individual members. In this case, we may want instruct the Code Generator to create programs which solves ODEs in multiple algorithms or which allow other output types interact with other modules. Those programs variate in application demand and different algorithms, so we can take advantages to develope them as a family~\citep{ss2004}.
