...
import Drasil.GOOL (OOProg, LoggerCode, InstanceVarSelfSym(..),
...
-- be generated in.
generateCode :: (OOProg progRepr, InstanceVarSelfSym (LoggerCode progRepr),
...
genPackage :: (OOProg progRepr, InstanceVarSelfSym (LoggerCode progRepr),
...
genProgram :: (OOProg r, InstanceVarSelfSym (LoggerCode r)) => GenState (OO.GSProgram r)
genProgram = do
  g <- get
...
-- of modularity.
chooseModules :: (OOProg r, InstanceVarSelfSym (LoggerCode r)) => Modularity ->
  GenState [OO.SFile r]
...
genUnmodular :: (OOProg r, InstanceVarSelfSym (LoggerCode r)) => GenState (OO.SFile r)
genUnmodular = do
  g <- get
...
genModules :: (OOProg r, InstanceVarSelfSym (LoggerCode r)) => GenState [OO.SFile r]
genModules = do
  g <- get
  mn     <- genMain
  inp    <- genInputMod
