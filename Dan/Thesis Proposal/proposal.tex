...
The current state of Scientific Computing software development leaves a lot of
room for improvement. I want to facilitate the improvement of qualities
(maintainability, traceability, reproducibility, etc.) across all artifacts of a
piece of software, without adding much effort for the developers. I believe a
...
I want everyone to make better software. Specifically by improving software
qualities such as maintainability, traceability, verifiability, reproducibility,
and reusability. That may be too broad of a target, so I will be focusing on
well-understood domains; specifically scientific computing (SC). To refine the
idea even more, I want to focus on applications where agile development is
ill-suited or completely unusable such as those that are safety-critical.
...
knowledge underlying a piece of software.
...
To that end, I am also creating a framework to facilitate my approach. 
The framework is known as ``Drasil'' and will be explained in 
Section~\ref{sec:Drasil}. Development of Drasil is following a practical,
...
Drasil employs techniques from literate programming and code generation
in order to automate as much as possible in the software development process.
...
LSD was developend with the intent to overcome the disadvantages of both LP and
box structure. Specifically, it was intended to overcome LP's inability to
specify interfaces between modules, box structure's inability to decompose boxes
and the inability to implement the design created by box structures, as well as
the lack of tools to support box structure~\cite{Deck1996}.
...
to decompose boxes at any level.
...
Org-mode. The most popular of those being Sweave~\cite{SchulteEtAl2012}.

\section{Research Objectives and Approach}
\subsection{Objectives}

Looking at the current state of SC software development, there are many areas
for improvement. The traditional approach for improving development would be to
get better developers, which is not necessarily feasible for SC software due to
resource limitations. My first objective, therefore, is to simplify the software
development process by allowing scientists to focus on the science.
...
The first step toward this improvement will be to better understand the
scientific background of the software, hence the focus on science.
...
other over time, so ensuring they remain consistent is one way to improve.
...
A full schedule of my work plan can be found in Section~\ref{subsec:schedule}.
...
implementations. It is my hope that this experiment will provide insight onto
...
Table~\ref{table:sum_schedule} shows a current breakdown of what I will be doing
for the rest of my tenure as a PhD student in broad strokes.
\begin{table}
