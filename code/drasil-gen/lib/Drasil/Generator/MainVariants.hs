...
-- | Builds case studies in a specific pattern that the Makefile expects.
module Drasil.Generator.MainVariants
  ( caseStudyMainSRS,
...
fixLocaleEncoding :: IO ()
fixLocaleEncoding = setLocaleEncoding utf8
...
-- JupyterNotebook-based lesson plan. Reasoning: Projectile.
--
-- FIXME: Realistically, the lesson plan should be treated as a separate project
-- as Projectile (the software project; SRS+code).
