...
class TestInterp(unittest.TestCase):
    ...
    def test_lin_interp(self):
        y = interp.lin_interp(-2, -8, -9, -5, -2) # negative, x1 < x2 < x
        self.assertEqual(y, -12.5)

# value1 in data1 and jdx == (data2[:, idx]).argmax()
    def test_find_bounds(self):
        idx, jdx, kdx, num_interp1, num_interp2 = interp.find_bounds(self.data1, self.data2, 9.1, 3.8)
        self.assertEqual(idx, 1)
        self.assertEqual(jdx, 8)
        self.assertEqual(kdx, 5)
        self.assertEqual(num_interp1, 0)
        self.assertEqual(num_interp2, 1)

    def test_interp(self):
        interp_value = interp.interp(1, 8, 5, 0, 1, self.data1, self.data2, self.data3, 9.1, 3.8)
        self.assertEqual(interp_value, 4.1)
