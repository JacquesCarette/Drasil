...
between two existing chunks, ie. \codeH{deriv a b} implies taking the 
...
}{SRS Recipe for GlassBR}{fig:glassBRSRSRecipe}
...
around within the \codeH{SRSDecl}. As for the chunks necessary for generating 
the document, each section includes references to them through a supplied list 
in something we call the \codeH{System Information} object 
(Figure~\ref{fig:systemInformation}).

\fig{
\lstinputlisting[language=Haskell, linerange={63-82}, 
firstnumber=63]{code/Body.hs}
}{System Information for GlassBR}{fig:systemInformation}

The system information object uses helper functions to keep track of the lists 
of chunks necessary for \ds{TODO - basically the list is obviously referencing 
...
The combination of system information and the SRS recipe is all we need to move 
on to creating our generator for the finished SRS document. Before we move on 
to discussing the generator we will need to follow a similar procedure to 
define the rest of our \sfs{}. As the other \sf{} recipes were developed in 
tandem with the SRS, they follow a similar structure and in the interest of 
brevity we will skip the breakdown of each recipe. It is left to the reader to 
investigate them by diving into our examples via the Drasil github. 

There is one \sf{} that is significantly different, and as such, we will go 
into more depth in discussing in the following section. The source code recipe, 
allows us to not only specify knowledge and the way we wish it structured, but 
also implementation choices that we would like to make in the final generated 
source code.

\ds{For the glassbr example, make sure to say something like ``The complete 
recipe for the SRS specification (not including chunks referenced from within 
our knowledge bases) is contained in a single file in 
less than 370 lines of code. The generated SRS in PDF format is 44 pages in 
length." It has stripped away all of the common boilerplate because we don't 
need to worry about that right now, as that will be a problem for the generator 
(Section~\ref{sec:gen} and what want out of a recipe is a simplified, 
duplication-avoiding representation of our target \sf{}.}

\ds{example goes here}
\ds{Make sure to touch on selected vector representation format as an example 
choice, seeing as we alluded to it earlier}

\subsection{Example: Source Code recipe for GlassBR}





The recipe DSL is an intermediate representation. It selects which 
