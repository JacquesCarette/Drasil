...
This plan outlines the process for verifying the SRS \sjc{add link} and this
...
Each teammate has its role listed in \nameref{table_team} according to the
following roles:
...
rubric for CAS 741 \sjc{add links}; the only exception is that the Drasil team
...
Drasil does not currently generate design documentation \sjc{add source?}, this
...
%, the VnV checklist, -- does this exist?
and the VnV rubric for CAS 741 \sjc{add links}.
...
compiles \sjc{double check}. Since the code for \progname{} will be generated
by Drasil, the Drasil team may potentially use other tools as part of its
verification (e.g., linters). \sjc{Are Makefiles generated?}
...
\wss{If there is any external data that can be used for validation, you should
  point to it here.} \sjc{Would ``external data'' include a database of
  chemical reactions?}
...
\begin{enumerate}
  ...
  \item[T\refstepcounter{testnum}\thetestnum \label{test_element_support}:]
    ...
    \progname{}. This includes the input stage (from R1 from SRS \sjc{add
      link}), the conversion stage (from R2 from SRS \sjc{add link}), and the
    output stage (from R4 and R6 from SRS \sjc{add link}).
...
feasible chemical reaction satisfies R1, R2, R3, R5, and R6 from SRS \sjc{add
  link}. Note that the input/output format
...
will be performed automatically. Providing the correct output for an inputted
infeasible chemical reaction satisfies R1, R2, R3, and R4 from SRS \sjc{add
  link}. Note that the input/output format
for each test is an abstract representation, as the specific format of each
input/output is a design decision that is not made at this stage.
...
\begin{enumerate}
  ...
  \item[T\refstepcounter{testnum}\thetestnum \label{test_bal_accuracy}:]
    ...
    the \nameref{sec_sysFunReqs} using integer comparison and satisfies R1, R2,
    R3, R5, R6, and NFR1 from SRS \sjc{add link}.
...
\begin{enumerate}
  ...
  \item[T\refstepcounter{testnum}\thetestnum \label{test_portable}:]
    ...
    performed. % This satisfies R1, R2,
    % R3, R5, R6, and NFR1 from SRS \sjc{add link}.
