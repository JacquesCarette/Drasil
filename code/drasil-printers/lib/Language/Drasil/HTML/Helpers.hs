...
image f Nothing wp = 
  figure $ vcat [img $ [("src", f), ("alt", text "")] ++ if wp == 100 then [] else [("width", text $ show wp ++ "%")]]
image f (Just c) wp =
  figure $ vcat [img $ [("src", f), ("alt", c)] ++ if wp == 100 then [] else [("width", text $ show wp ++ "%")], figcaption $ text "Figure: " <> c]
