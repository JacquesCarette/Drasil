...
findMassConstruct fr m ims ddefs = cic "findMass" (foldlSent [
  ...
  m, S "needed for", foldlList Comma List (fmap refS ims) `sC`
  S "using", foldlList Comma List (fmap refS ddefs)])
...
outputList l = foldlList Comma List $
  fmap (\x -> ch (x ^. output) :+: sParen (ch time) +:+ fromSource x) l
