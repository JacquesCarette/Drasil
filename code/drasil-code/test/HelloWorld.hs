...
helloWorldOO :: (OOProg r tp vis smt par fun) => OO.GSProgram r
...
-- | Creates the HelloWorld program and necessary files.
helloWorldProc :: (ProcProg r tp vis smt par fun) => GProc.GSProgram r
...
myOtherList :: (SharedProg r tp vis smt par fun) => SVariable r
...
helloWorldMainOO :: (OOProg r tp vis smt par fun) => SMethod r
...
-- | Main function. Initializes variables and combines all the helper functions defined below.
helloWorldMainProc :: (ProcProg r tp vis smt par fun) => SMethod r
...
helloInitVariables :: (SharedProg r tp vis smt par fun) => MSBlock r
helloInitVariables = block [comment "Initializing variables",
  ...
  listDecDef myOtherList mainFn [litDouble 1.0,
    litDouble 1.5],
  arrayDec 3 (var "arr_copy" (arrayType int)) mainFn,
...
objectTests :: (OOProg r tp vis smt par fun) => MSBlock r
...
mySlicedList, mySlicedList2, mySlicedList3, mySlicedList4, mySlicedList5,
  ...
  mySlicedList10, mySlicedList11 :: (SharedProg r tp vis smt par fun) => SVariable r
...
listSliceTests :: (SharedProg r tp vis smt par fun) => [MSBlock r]
listSliceTests = [
...
helloIfBody :: (SharedProg r tp vis smt par fun) => MSBody r
...
helloElseBody :: (SharedProg r tp vis smt par fun) => MSBody r
helloElseBody = bodyStatements [printLn (arg 5)]

-- | If-else statement checking if a list is empty.
helloIfExists :: (SharedProg r tp vis smt par fun) => MS (r smt)
...
helloSwitch :: (SharedProg r tp vis smt par fun) => MS (r smt)
...
helloTryCatch :: (SharedProg r tp vis smt par fun) => MS (r smt)
helloTryCatch = tryCatch (oneLiner (throw "Good-bye!"))
  (oneLiner (printStrLn "Caught intentional error"))

helloWorldClass :: (OOProg r tp vis smt par fun) => SClass r
