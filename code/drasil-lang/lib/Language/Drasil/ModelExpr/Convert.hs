...
expr (E.Lit a)               = Lit a
expr (E.AssocA ao es)        = AssocA ao $ fmap expr es
expr (E.AssocB bo es)        = AssocB (assocBoolOper bo) $ fmap expr es
expr (E.AssocC ao es)        = AssocC ao $ fmap expr es
expr (E.C u)                 = C u
expr (E.FCall u es)          = FCall u (fmap expr es)
expr (E.Case c ces)          = Case c (fmap (bimap expr expr) ces)
expr (E.Matrix es)           = Matrix $ fmap (fmap expr) es
expr (E.Set s e)             = Set s $ fmap expr e
