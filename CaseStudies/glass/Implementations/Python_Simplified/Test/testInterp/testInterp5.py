...
class TestInterp(unittest.TestCase):
    ...
    def test_lin_interp(self):
        y = interp.lin_interp(-9.9, -1.3, -3.5, -2.4, -12.6) # negative, x < x2 < x1
        self.assertEqual(y, -81.04545454545455)

    def test_find_bounds(self):
        idx, jdx, kdx, num_interp1, num_interp2 = interp.find_bounds(self.data1, self.data2, 12, 3.4)
        self.assertEqual(idx, 1)
        self.assertEqual(jdx, 6)
        self.assertEqual(kdx, 1)
        self.assertEqual(num_interp1, 1)
        self.assertEqual(num_interp2, 2)

    def test_interp(self):
        interp_value = interp.interp(1, 6, 1, 1, 2, self.data1, self.data2, self.data3, 12, 3.4)
        self.assertEqual(interp_value, 3.9207062600321025)
