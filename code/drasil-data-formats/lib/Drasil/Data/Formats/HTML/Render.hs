...
renderHeadSec opt heads = wrapBlock opt "head" [] (fmap (renderHead opt) heads)

-- | Internal: Render the 'body' section
renderBodySec :: HTMLRenderOptions -> [HTMLBody] -> Doc ann
renderBodySec opt bodies = wrapBlock opt "body" [] (fmap (renderBody opt) bodies)
...
renderBody opt (List Unordered attrs items) = wrapBlock opt "ul" attrs (fmap renderIList items)
  where
    renderIList (LItem iAttrs ch) = renderLine opt "li" iAttrs ch
renderBody opt (List Ordered attrs items) = wrapBlock opt "ol" attrs (fmap renderIList items)
  where
...
renderBody opt (DescriptionList attrs items) = wrapBlock opt "dl" attrs (fmap renderDItem items)
  where
    renderDItem (DTerm iAttrs ch) = renderLine opt "dt" iAttrs ch
    renderDItem (DDetails iAttrs ch) = renderBlockInline opt "dd" iAttrs ch
renderBody opt (Table attrs rows) = wrapBlock opt "table" attrs (fmap renderRow rows)
  where
    renderRow (Row rAttrs cells) = wrapBlock opt "tr" rAttrs (fmap renderCell cells)
...
renderLine opt tag attrs ch = wrapLine tag attrs $ fmap (renderBody opt) ch

-- | Internal: Render the children breaking lines
renderBlock :: HTMLRenderOptions -> Text -> [Attr] -> [HTMLBody] -> Doc ann
renderBlock opt tag attrs ch = wrapBlock opt tag attrs $ fmap (renderBody opt) ch
...
wrapBlockInline = wrapBlockWith hcat
...
wrapBlockWith catDocs opt tag attrs docs =
  vcat
    [ angles (pretty tag <> renderAttrs attrs),
      indent (indentationSize opt) (catDocs docs),
      angles ("/" <> pretty tag)
    ]
...
renderAttrs [] = mempty
renderAttrs attrs = space <> hsep (fmap rAttr attrs)
  where
...
normalizeNode (List t attrs items) = List t attrs (fmap normItem items)
  where
    normItem (LItem a ch) = LItem a (normalizeBody ch)
normalizeNode (DescriptionList attrs items) = DescriptionList attrs (fmap normDItem items)
  where
    normDItem (DTerm a ch) = DTerm a (normalizeBody ch)
    normDItem (DDetails a ch) = DDetails a (normalizeBody ch)
normalizeNode (Table attrs rows) = Table attrs (fmap normRow rows)
  where
    normRow (Row a cells) = Row a (fmap normCell cells)
