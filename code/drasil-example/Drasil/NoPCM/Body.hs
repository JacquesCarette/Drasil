...
orgDocEnd im_ od pro = foldlSent_ [S "The", phrase im_,
  ...
  S "to be solved is referred to as" +:+. makeRef eBalanceOnWtr_new,
...
assumpS9_npcm = 
  ...
  sSqBr ((makeRef eBalanceOnWtr_new) `sC` (makeRef likeChg3_npcm))])
...
assumpS12 = 
  ...
  sSqBr (makeRef eBalanceOnWtr_new))
...
req4 = mkRequirement "req4" (
  ...
  `sAnd` ch tau_W +:+. sParen(S "from" +:+ makeRef eBalanceOnWtr_new) ) "Output-Input-Derived-Quantities"
...
nr4 = cic "Output-Input-Derivied-Quantities" (titleize' output_ `sAnd` plural input_ 
    ...
    sParen (S "from" +:+ makeRef eBalanceOnWtr_new)) "Output-Input-Derivied-Quantities" funcReqDom
