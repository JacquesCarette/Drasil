...
information sources, it makes creating documentation more difficult, reducing a
...
Scientists and engineers also heavily rely on models that share common
mathematical/scientific knowledge. Common sense would dictate that these models
should be created once and reused as necessary, across any applicable projects.
However, that is often not the case as many models are reimplemented for each
new project. Therefore, I intend to address this problem and hope to improve the
reuse of knowledge for scientific models. 
%Maybe move the ``I intend'' to objectives?
...
the qualities of the project as a whole suffer. This usually happens because
these qualities (traceability, maintainability, verifiability, reproducibility,
etc.) are hard to quantify and not seen as being as critical as traits such as
correctness, accuracy, or performance.

The impact on both traceability and maintainability arising from knowledge
duplication is fairly obvious. When artifacts become inconsistent, traceability
can be lost completely and maintenance becomes much more difficult. There are
also impacts on other software qualities such as reproducibility and
verifiability.
