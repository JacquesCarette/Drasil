...
glassBR_srs' = mkDoc' mkSRS (for'' fterm titleize phrase) glassSystInfo
...
glassBR_mg :: Document
glassBR_mg = mgDoc'' glassBRProg (for'' fterm titleize phrase) mg_authors mgBod

s2 = introductionF (gLassBR) (startIntro, (fterm short gLassBR)) (s2_1_intro_p1) (incScoR, endScoR)
...
s2_1_intro_p1 = foldlSent [S "The main", fterm phrase purpose, S "of this", 
  fterm phrase document, S "is to predict whether a given", (phrase $ glaSlab ^. term),
  S "is likely to resist a specified" +:+. (phrase $ blast ^. term),
  S "The", fterm plural goal, S "and", fterm plural thModel, S "used in the", 
  (fterm short gLassBR), S "code are provided" `sC`
  S "with an emphasis on explicitly identifying",   (plural assumption), 
  S "and unambiguous" +:+. fterm plural definition, S "This", fterm phrase document,
  S "is intended to be used as a", fterm phrase reference, S "to provide all",
  fterm phrase information, S "necessary to understand and verify the" +:+.
  fterm phrase analysis, S "The", (fterm short srs), S "is abstract because the",
  fterm plural content, S "say what", fterm phrase problem,
  S "is being solved, but not how to solve it"]

--for Organization of Document Section
s2_3_intro = foldlSent [S "The", fterm phrase organization, S "of this",
  fterm phrase document, S "follows the", fterm phrase template, S "for an", (fterm short srs),
  S "for", fterm phrase sciCompS, S "proposed by" +:+ sSqBr (S "1"), S "and",
  sSqBr (S "2"), S "(in", (makeRef s10) :+: 
  S "), with some aspects taken from Volere", fterm phrase template, S "16",
  sSqBr (S "3")]
...
s4_1_bullets = enumBullet [(S "The" +:+ fterm phrase endUser +:+ S "of" +:+ (fterm short gLassBR) +:+ 
  ...
  (S "The" +:+ fterm phrase endUser +:+ S "is expected to have an understanding of" +:+
  fterm phrase theory +:+ S "behind" +:+ (phrase $ glBreakage ^. term) +:+ S "and" +:+.
  (phrase $ blastRisk ^. term)), (S "The" +:+ fterm phrase endUser +:+
  S "is expected to have basic" +:+ fterm phrase computer +:+ S "literacy to handle the"
  +:+. fterm phrase software)]

s4_2 = systCon [] []

s5 = SRS.scpOfTheProj [s5_intro] [s5_1, s5_2]

s5_intro = Paragraph $ foldlSent [S "This", fterm phrase section_,
  S "presents the" +:+. fterm phrase (scpOfTheProj phrase),
  S "It describes the expected use of", (fterm short gLassBR), 
  S "as well as the", fterm plural input_, S "and", fterm plural output_, 
  S "of each action. The", fterm plural useCase, S "are", fterm phrase input_, S "and",
  fterm phrase output_ `sC` S "which defines the action of getting the",
  fterm phrase input_, S "and displaying the", fterm phrase output_]

s5_1 = SRS.prodUCTable [s5_1_table] []

s5_1_table = Table [titleize useCase +:+. S "NO", fterm titleize useCase +:+
  fterm titleize name_, S "Actor", fterm titleize input_ +:+ S "and" +:+ fterm titleize output_]
  (mkTable
  [(\x -> (x!!0)),(\x -> (x!!1)), (\x -> (x!!2)), (\x -> (x!!3))]
  [[S "1", fterm titleize' input_, fterm titleize user, fterm titleize' characteristic +:+
  S "of the" +:+ (phrase $ glaSlab ^. term) +:+ S "and of the" +:+.
  (phrase $ blast ^. term) +:+ S "Details in" +:+ (makeRef s5_2)],
  [S "2", fterm titleize output_, (fterm short gLassBR), S "Whether or not the" +:+
  (phrase $ glaSlab ^. term) +:+ S "is safe for the calculated" +:+
  (phrase $ load ^. term) +:+ S "and supporting" +:+
  S "calculated" +:+ fterm plural value]]) 
  (fterm titleize table_ +: S "1" +:+ fterm titleize useCaseTable) True
...
s5_2_bt_sent1 = foldlSent [titleize useCase, S "1 refers to the", fterm phrase user, 
  S "providing", fterm phrase input_, S "to", (fterm short gLassBR),
  S "for use within the" +:+. fterm phrase analysis, S "There are two classes of"
  +: fterm plural input_, (phrase $ glassGeo ^. term), S "and" +:+.
  (phrase $ blastTy ^. term), (glassGeo ^. defn), (blastTy ^. defn), S "These",
  (plural $ parameter ^. term), S "describe", (phrase $ char_weight ^. term),
  S "and stand off" +:+. (phrase $ blast ^. term), S "Another", fterm phrase input_,
  S "the", fterm phrase user, S "gives" `isThe` S "tolerable", fterm phrase value, S "of",
  (phrase $ prob_br ^. term)]

s5_2_bt_sent2 :: Sentence
s5_2_bt_sent2 = foldlSent [S " Use Case 2", (fterm short gLassBR), 
  fterm plural output_, S "if the", (phrase $ glaSlab ^. term), S "will be safe by", 
  ...
  fterm phrase requirement +:+. S "which" `isThe` (demandq ^. defn), S "The second", 
  fterm phrase condition, S "is to check whether the calculated", 
  (phrase $ probability ^. term), sParen (P $ prob_br ^. symbol), 
  S "is less than the tolerable", (phrase $ probability ^. term), 
  sParen (P $ pb_tol ^. symbol), S "which is obtained from the", fterm phrase user,
  S "as an" +:+. fterm phrase input_, S "If both", fterm plural condition, 
  ...
  S "All the supporting calculated", fterm plural value, S "are also displayed as",
  fterm phrase output_]

s6 = specSysDesF (S "and" +:+ fterm plural definition) [s6_1, s6_2]

s6_1 = probDescF start gLassBR ending [s6_1_1, s6_1_2, s6_1_3]
  where start = foldlSent [S "A", fterm phrase system,
                S "is needed to efficiently and correctly predict the", 
                (phrase $ blastRisk ^. term) +:+ S "involved with the glass"]
        ending = foldl (+:+) EmptyS [S "interpret the", fterm plural input_,
                S "to give out the", fterm plural output_, S "which predicts whether the", 
                (phrase $ glaSlab ^. term), S "can withstand the",
                (phrase $ blast ^. term), S "under the", fterm plural condition]
...
s6_1_1_bullets = Enumeration $ (Number $ 
  [Flat $ ((at_start $ aspectR ^. term) :+: sParenDash (fterm short aspectR)) {-P $ aspectR ^. symbol))-} -- conceptually correct to call abbreviation as a symbol?
  :+: (aspectRatio ^. defn)] ++
  ...
  (Bullet $ map (\c -> Flat c) [(((at_start $ annealedGl ^. term) :+: 
    sParenDash (fterm short annealedGlass)) :+: (annealedGl ^. defn)),
  (((at_start $ fTemperedGl ^. term) :+: sParenDash (fterm short fullyTGlass)) :+:
    (fTemperedGl ^. defn)),
  (((at_start $ hStrengthGl ^. term) :+: sParenDash (fterm short heatSGlass)) :+:
    (hStrengthGl ^. defn))])] ++
  map (\c -> Flat c)
  [(((at_start $ glTyFac ^. term) :+: sParenDash (fterm short glassTypeFac)) :+: 
  (glTyFac ^. defn)),
  ...
  (((at_start $ loadResis ^. term) :+: sParenDash (fterm short lResistance)) :+:
    (loadResis ^. defn)),
  ...
  map (\c -> Flat c)
  [(((at_start $ loadShareFac ^. term) :+: sParenDash (fterm short lShareFac)) :+: 
  (loadShareFac ^. defn)),
...
s6_1_2 = physSystDesc (fterm short gLassBR) (fig_glassbr) [s6_1_2_list, fig_glassbr]

fig_glassbr = Figure (at_start $ the physicalSystem) "physicalsystimage.png"
  
s6_1_2_list = enumSimple 1 (fterm short physSyst) s6_1_2_list_physys1
...
s6_1_3_list = enumSimple 1 (fterm short goalStmt) s6_1_3_list_goalStmt1
...
s6_1_3_list_goalStmt1 = [foldlSent [S "Analyze and predict whether the",
  ...
  S "of a certain degree which is calculated based on", fterm phrase user, 
  fterm phrase input_]]
...
s6_2_intro = Paragraph $ foldlSent [S "This", fterm phrase section_, 
  S "explains all the", (plural assumption), S "considered and the",
  fterm plural thModel, S "which are supported by the", (plural dataDefn)]
  
s6_2_1 = assumpF' (s6_2_2) (s6_2_4) (s6_2_3) (s8) (s6_2_1_list)

s6_2_1_list = 
  [(enumSimple 1 (fterm short assumption) s6_2_1_list_assum1),
  ...
  (enumSimple 5 (fterm short assumption) s6_2_1_list_assum2)]
...
s6_2_1_list_assum1 = [foldlSent [S "The standard E1300-09a for", 
  ...
  S "support", fterm plural condition, S "are simply supported and free to slip in",
  ...
  foldlSent [S "This", fterm phrase system, S "only considers the external", 
  (phrase $ explosion ^. term), S "scenario for its", 
  (plural $ calculation ^. term)],
  (S "Standard" +:+ fterm plural value +:+ S "used for" +:+
  (phrase $ calculation ^. term) +:+ S "in" +:+ (fterm short gLassBR) +: S "are")]
...
s6_2_1_list_assum2 = [(foldlSent [S "Glass under consideration", 
  ...
  fterm phrase value, S "of", (fterm short lShareFac), S "is equal to 1 for all", 
  (plural $ calculation ^. term), S "in", (fterm short gLassBR)]),
  (foldlSent [S "Boundary", fterm plural condition, S "for the", 
  (phrase $ glaSlab ^. term), S "is assumed to be 4-sided",
  S "support for", (plural $ calculation ^. term)]),
  (foldlSent [S "The response type considered in", (fterm short gLassBR), 
  S "is flexural"]),
  (foldlSent [S "With", fterm phrase reference, S "to A4 the", fterm phrase value, 
  S "of", (phrase $ loadDF ^. term), sParen (P $ loadDF ^. symbol), 
  S "is a constant in" +:+. (fterm short gLassBR), S "It is calculated by the" +: 
...
s6_2_5_table2 = Table [S "Var", fterm titleize value] (mkTable 
  [(\x -> fst x), (\x -> snd x)] 
  ...
  Sy (unit_symb sd_max))])
  (fterm titleize table_ +: S "3" +:+ fterm titleize specification +:+
  (titleize $ parameter ^. term) +:+ fterm titleize' value) True

s6_2_5_intro2 = Paragraph $ foldlSent [(makeRef s6_2_5_table3), S "shows the",
  fterm plural constraint, S "that must be satisfied by the", fterm phrase output_]
...
s7_1_list = 
  ...
  [(((fterm short requirement) :+: S "1"), fterm at_start input_ +:+ S "the following" +:+
    fterm plural quantity `sC` S "which define the glass dimensions" `sC` 
  ...
  ++ (map qs [glass_type, pb_tol, tNT])) (\x -> fterm at_start)),
...
--  (S "Input Parameters") False
  (Enumeration $ Simple $
  [(((fterm short requirement) :+: S "2"), Nested (S "The" +:+ fterm phrase system +:+
  S "shall set the known" +:+ fterm plural value +:+ S "as follows: ")
   (Bullet $ map (\c -> Flat c) 
    [(P $ sflawParamM ^. symbol) `sC` (P $ sflawParamK ^. symbol) `sC` 
    (P $ mod_elas ^. symbol) `sC` (P $ load_dur ^. symbol) +:+ 
    S "following" +:+ (fterm short assumption) :+: S "4",
    (P $ loadDF ^. symbol) +:+ S "following" +:+ (fterm short assumption) 
    :+: S "8",
    (fterm short lShareFac) +:+ S "following" +:+ (fterm short assumption) 
    :+: S "5"]))] ++
  map (\(a, b) -> (a, Flat b))
  [(((fterm short requirement) :+: S "3"), S "The" +:+ fterm phrase system +:+
  S "shall check the entered" +:+ fterm phrase input_ +:+ fterm plural value +:+ 
  S "to ensure that they do not exceed the"  +:+ fterm plural datumConstraint +:+
  S "mentioned in" +:+. (makeRef s6_2_5) +:+ S "If any of" +:+
  S "the" +:+ fterm phrase input_ +:+ (plural $ parameter ^. term) +:+
  S "is out of bounds, an error message is displayed and the" +:+
  (plural $ calculation ^. term) +:+. S "stop"), (((fterm short requirement)
  :+: S "4"), fterm titleize output_ +:+ S "the" +:+ fterm phrase input_ +:+
  fterm plural quantity +:+ S "from" +:+ (fterm short requirement) :+: S "1 and the known"
  +:+ fterm plural quantity +:+ S "from" +:+ (fterm short requirement) :+: S "2."),
  (((fterm short requirement) :+: S "5"), S "If" +:+ (P $ is_safe1 ^. symbol)
  +:+ S "and" +:+ (P $ is_safe2 ^. symbol) +:+ S "(from" +:+ 
  (makeRef (gbSymbMapT t1SafetyReq)) +:+ S "and" +:+ 
  (makeRef (gbSymbMapT t2SafetyReq)) :+: S ") are true" `sC`
  fterm phrase output_ +:+ S "the message" +:+ Quote (safeMessage ^. defn) +:+
  S "If the" +:+ fterm phrase condition +:+ S "is false, then" +:+ fterm phrase output_ +:+
  S "the message" +:+ Quote (notSafe ^. defn))] ++ 
  [(((fterm short requirement) :+: S "6"), Nested (fterm titleize output_ +:+
  S "the following" +: fterm plural quantity)
  (Bullet $ 
    [Flat $ (at_start $ prob_br ^. term) +:+ sParen (P $ prob_br ^. symbol) +:+ 
    sParen (makeRef (gbSymbMapT probOfBr))] ++
    [Flat $ (titleize $ lResistance ^. term) +:+ sParen (fterm short lResistance) +:+ 
    ...
    [Flat $ (titleize $ glassTypeFac ^. term) +:+ sParen (fterm short glassTypeFac) +:+ 
    ...
    [Flat $ (titleize $ aspectR ^. term) +:+ sParen (fterm short aspectR {-P $ aspectR ^. symbol-})  
    --S " = a/b)"
    ]))])]
...
s7_2_intro = Paragraph $ foldlSent [
  ...
  fterm phrase problem `sC` (phrase $ performance ^. term), S "is not a" +:+. fterm phrase priority,
  S "Any reasonable", fterm phrase implementation +:+. 
  S "will be very quick and use minimal storage", 
  S "Rather than", (phrase $ performance ^. term) `sC` S "the", fterm phrase priority,
  fterm phrase nonfunctional, (fterm short requirement) :+:
...
s8_likelychg1 = foldlSent [(fterm short assumption) :+: S "3 - The", fterm phrase system,
...
s8_likelychg2 = foldlSent [(fterm short assumption) :+: S "4" `sC` 
  (fterm short assumption) :+: S "8 - Currently the", fterm plural value,
  S "for", (P $ sflawParamM ^. symbol) `sC` (P $ sflawParamK ^. symbol) `sC`
  S "and", (P $ mod_elas ^. symbol), S "are assumed to be the",
  S "same for all glass. In the future these", fterm plural value, 
  S "can be changed to", fterm phrase variable, fterm plural input_]

s8_likelychg3 = foldlSent [(fterm short assumption) :+: S "5 - The", fterm phrase software,
  S "may be changed to accommodate more than a single", (phrase $ lite ^. term)]

s8_likelychg4 = foldlSent [(fterm short assumption) :+: S "6 - The", fterm phrase software,
  S "may be changed to accommodate more boundary", fterm plural condition,
  S "than 4-sided support"]

s8_likelychg5 = foldlSent [(fterm short assumption) :+: S "7 - The", fterm phrase software,
...
s8_list = enumSimple 1 (fterm short likelyChg) s8_likelychg_list

s9 = traceMGF [s9_table1, s9_table2, s9_table3]
  [(plural thModel `sC` (plural dataDefn) +:+ S "and" +:+ fterm plural inModel +:+.
  S "with each other"), (plural requirement +:+ S "on" +:+ fterm plural thModel `sC`
  (plural inModel) `sC` (plural dataDefn) +:+ S "and" +:+. fterm plural datumConstraint),
  (plural thModel `sC` (plural dataDefn) `sC` fterm plural inModel
  `sC` fterm plural likelyChg +:+ S "and" +:+ (plural requirement) +:+ S "on the" +:+ 
...
s9_table1 = Table (EmptyS:s9_row_header_t1) 
  ...
  (showingCxnBw (traceyMatrix) (fterm titleize' item +:+ S "of Different" +:+ fterm titleize' section_)) True
...
s9_table2 = Table (EmptyS:s9_row_header_t2)
  ...
  (showingCxnBw (traceyMatrix) (fterm titleize' requirement +:+ S "and Other" +:+ fterm titleize' item)) True
...
s9_table3 = Table (EmptyS:s9_row_header_t3)
  ...
  (showingCxnBw (traceyMatrix) (fterm titleize' assumption +:+ S "and Other" +:+ fterm titleize' item)) True

--

s9_intro2 = traceGIntro [fig_2, fig_3, fig_4]
  [(plural requirement +:+ S "on" +:+ fterm plural thModel `sC` fterm plural inModel
  `sC` fterm plural dataDefn +:+ S "and" +:+. fterm plural datumConstraint),
  (plural requirement +:+ S "on" +:+ fterm plural thModel `sC` fterm plural inModel
  `sC` (plural dataDefn) +:+ S "and" +:+. fterm plural datumConstraint),
  (plural thModel `sC` fterm plural inModel `sC` fterm plural dataDefn `sC`
  fterm plural requirement +:+ S "and" +:+ fterm plural likelyChg +:+ S "on" +:+
  fterm plural assumption)]

fig_2 = figureLabel "2" (traceyMatrix)
  (fterm titleize' item +:+ S "of Different" +:+ fterm titleize' section_)
  ("Trace.png")

fig_3 = figureLabel "3" (traceyMatrix)
  (fterm titleize' requirement +:+ S "and Other" +:+ fterm titleize' item)
  ("RTrace.png")

fig_4 = figureLabel "4" (traceyMatrix)
  (fterm titleize' assumption +:+ S "and Other" +:+ fterm titleize' item)
  ("ATrace.png")

s10 = SRS.reference [s10_list] []

s10_list = mkRefsList 1 
  [(S "N. Koothoor" `sC` Quote (S "A" +:+ fterm phrase document +:+ 
  S "drive approach to certifying" +:+ fterm phrase sciCompS :+: S ",") +:+
  S "Master's thesis" `sC` S "McMaster University, Hamilton, Ontario, Canada, 2013."),
  (S "W. S. Smith and L. Lai" `sC` Quote (S "A new" +:+ fterm plural requirement +:+
  fterm phrase template +:+ S "for scientific computing,") +:+ S "in Proceedings of the" +:+
  S "First International Workshop on Situational" +:+ fterm titleize' requirement +:+ 
  S "Engineering Processes - Methods, Techniques and Tools to Support Situation-Specific" +:+
  fterm titleize' requirement +:+ S "Engineering Processes, SREP'05 (J.Ralyt" :+: 
  (F Acute 'e') :+: S ", P.Agerfalk, and N.Kraiem, eds.), (Paris, France),"
  +:+ S "pp. 107-121, In conjunction with 13th IEEE International" +:+
  fterm titleize' requirement +:+. S "Engineering Conference, 2005"),
  (S "J. Robertson and S. Robertson" `sC` Quote (S "Volere" +:+
  fterm plural requirement +:+ fterm phrase specification +:+ fterm phrase template +:+. S "edition 16") +:+ 
  ...
  +:+ Quote (S "Standard" +:+ fterm phrase specification +:+. S "for flat glass,C1036")),
...
s11_intro = Paragraph $ foldlSent [
  S "This", fterm phrase appendix, S "holds the", (plural $ graph ^. term),
  sParen ((makeRef fig_5) +:+ S "and" +:+ (makeRef fig_6)),
  S "used for interpolating", fterm plural value, S "needed in the", fterm plural model]

fig_5 = Figure (fterm titleize figure +: S "5" +:+ (demandq ^. defn) +:+ sParen
  (P (demand ^. symbol)) +:+ S "versus" +:+ (at_start $ sD ^. term) +:+
  S "versus" +:+ (at_start $ char_weight ^. term) +:+ sParen
  (P (sflawParamM ^. symbol))) "ASTM_F2248-09.png"

fig_6 = Figure (fterm titleize figure +:+ S "6: Non dimensional" +:+ 
  (phrase $ lateral ^. term) +:+
  (phrase $ load ^. term) +:+ sParen
  (P (dimlessLoad ^. symbol)) +:+ S "versus" +:+ (titleize $ aspectR ^. term) +:+ 
  sParen (fterm short aspectR) {-(P (aspectR ^. symbol))-} +:+ S "versus" +:+
