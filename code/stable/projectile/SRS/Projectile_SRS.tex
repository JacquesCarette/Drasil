...
\item[launchOrigin:\phantomsection\label{launchOrigin}]The launcher is coincident with the origin. \hyperref[IM:calOfLandingTime]{IM: calOfLandingTime}.
\item[targetXAxis:\phantomsection\label{targetXAxis}]The target lies on the x-axis. \hyperref[IM:calOfLandingTime]{IM: calOfLandingTime}.
...
\item[accelYGravity:\phantomsection\label{accelYGravity}]The acceleration in the y-direction is the acceleration due to gravity. \hyperref[IM:calOfLandingTime]{IM: calOfLandingTime}.
...
Assume we have rectilinear motion of a particle (of negligible size and shape \hyperref[pointMass]{A: pointMass}); that is, motion in a straight line. The position is $p$ and the velocity is $v$. The motion in \hyperref[TM:velocity]{TM: velocity} is now one-dimensional. The initial position (at $t=0$) is represented by ${p^{i}}$. From \hyperref[TM:velocity]{TM: velocity}, using the above symbols we have:
\begin{displaymath}
v=\frac{d\,p}{d\,t}
\end{displaymath}
Rearranging and integrating, we have:
...
\\ \midrule \\
Equation & \begin{displaymath}
           t'=\frac{2 {v^{i}} \sin\left(θ\right)}{a}
           \end{displaymath}
\\ \midrule \\
Description & \begin{symbDescription}
...
We know that ${{p_{y}}^{i}}=0$ (\hyperref[launchOrigin]{A: launchOrigin}) and ${{a_{y}}^{c}}=-a$ (\hyperref[accelYGravity]{A: accelYGravity}). Substituting these values into the y-direction of \hyperref[GD:posVec]{GD: posVec} gives us:
\begin{displaymath}
{p_{y}}={{v_{y}}^{i}} t-\frac{a t^{2}}{2}
\end{displaymath}
