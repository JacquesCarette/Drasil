...
s1_2_intro = Paragraph (S "The table that follows summarizes the" :+:
             ...
             S " with the " :+: (sMap (map toLower) (heat_trans ^. term)) :+: 
             S " literature " :+:
...
s2_1_contents = [Paragraph (S "The main purpose of this document is to " :+:
                ...
                S ". The " :+: (sMap (map toLower) (goalStmt ^. defn)) :+: 
                ...
                S " " :+: (sMap (map toLower) (assumption ^. defn)) :+: 
...
s2_2_contents = Paragraph (S "The scope of the requirements is limited " :+:
                ...
                (temp ^. defn) :+: S " and " :+: (sMap (map toLower)  
                (thermal_energy ^. term)) :+: S " histories for the " :+:
                (sMap (map toLower)  (water ^. term)) :+: S " and the " :+:
                (phsChgMtrl ^. term) :+: S ". This entire document" :+:
                S " is written assuming that the substances inside the " :+:
                (sMap (map toLower) (tank ^. defn)) :+: S " are " :+:
                (sMap (map toLower) (water ^. term)) :+: S " and " :+:
                (phsChgMtrl ^. term) :+: S ".")
...
s2_3_contents = [Paragraph (S "The organization of this document follows" :+:
                ...
                defn)) :+: S "s, " :+: (sMap (map toLower) (thModel ^. 
                defn)) :+: S "s, " :+: (sMap (map toLower) (dataDefn ^.
                defn)) :+: S "s, and " :+: (sMap (map toLower) (assumption ^.
                defn)) :+: S "s. For readers that would like a more bottom" :+: 
                S " up approach, they can start reading the " :+: (sMap (map  
                toLower) (inModel ^. defn)) :+: S "s in " :+: 
                makeRef s4_2_5 :+: S " and trace back to find any " :+:
                S "additional information they require. The " :+:
                (sMap (map toLower) (inModel ^. defn)) :+: S "s provide " :+:
                S "the " :+: (ordDiffEq ^. defn) :+: S " (" :+: (ordDiffEq ^.
                term) :+: S "s) and algebraic equations that model the " :+:
                (swhs_pcm ^. term) :+: S ". " :+: (progName ^. term) :+:
                S " solves these " :+: (ordDiffEq ^. term) :+: S "s."),
                Paragraph (S "The " :+: (sMap (map toLower) (goalStmt ^. defn))
                :+: S "s are refined to the " :+: (sMap (map toLower) 
                (thModel ^. defn)) :+: S "s, and " :+: (sMap (map toLower) 
                (thModel ^. defn)) :+: S "s to the " :+: (sMap (map toLower)
                (inModel ^. defn)) :+: S "s. The " :+: (sMap (map toLower) 
...
s4_intro = Paragraph (S "This section first presents the problem " :+:
           ...
           (sMap (map toLower) (genDefn ^. defn)) :+: S "s, " :+: 
           (sMap (map toLower) (dataDefn ^. defn)) :+: S "s, and finally " :+:
           S "the " :+: (sMap (map toLower) (inModel ^. defn)) :+: S "s (" :+:
...
s4_1_2_list = Enumeration (Simple $ [((physSyst ^. term) :+: S "1", Flat
              ((tank ^. term) :+: S " containing " :+: (sMap (map toLower) 
              ((water ^. term))) :+: S ".")),
              ((physSyst ^. term) :+: S "2", Flat ((coil ^. term) :+: 
              S " at bottom of " :+: (sMap (map toLower) (tank ^. term)) :+:
              S ". (" :+: P (ht_flux_C ^. symbol) :+: S " represents the " :+:
              (ht_flux_C ^. defn) :+: S ".)")),
              ((physSyst ^. term) :+: S "3", Flat ((phsChgMtrl ^. term) :+: 
              S " suspended in " :+: (sMap (map toLower) (tank ^. term)) :+:
...
s4_1_3_intro = Paragraph (S "Given the " :+: (temp_C ^. defn) :+: S ", " :+:
               ...
               (sMap (map toLower) (goalStmt ^. defn)) :+: S "s are:")
...
s4_2_1_intro = Paragraph (S "This section simplifies the original problem " :+:
               ...
               S "], " :+: (sMap (map toLower) (genDefn ^. defn)) :+: 
               S " [" :+: (genDefn ^. term) :+: S "], " :+: (sMap (map 
               toLower) (dataDefn ^. defn)) :+: S " [" :+: (dataDefn ^.
               term) :+: S "], " :+: (sMap (map toLower) (inModel ^. defn)) :+:
               S " [" :+: (inModel ^. term) :+: S "], or " :+: (sMap (map 
               toLower) (likelyChg ^. defn)) :+: S " [" :+: (likelyChg ^. 
...
s4_2_1_list = Enumeration (Simple [((assumption ^. term) :+: S "1", Flat 
              ...
              (time ^. defn) :+: S " [GD1].")),
              ((assumption ^. term) :+: S "3", Flat (S "The " :+: (sMap 
              (map toLower) ((water ^. term))) :+: S " in the " :+: (sMap 
              (map toLower) ((tank ^. term))) :+: S " is fully mixed, so " :+:
              S "the " :+: (temp_W ^. defn) :+: S " is the same throughout" :+:
              S " the entire " :+: (sMap (map toLower) ((tank ^. term))) :+:
              ...
              defn) :+: S " applies between the " :+: (sMap (map toLower) (
              (coil ^. term))) :+: S " and the " :+: (sMap (map toLower) (
              ...
              S "accounts for " :+: (sMap (map toLower) (charging ^. defn)) :+:
              S ", not " :+: (sMap (map toLower) ((discharging ^. term))) :+:
              ...
              S "the " :+: (sMap (map toLower) ((tank ^. term))) :+:
...
s4_2_3_intro = Paragraph (S "This section collects the laws and equations " :+:
               ...
               S "build the " :+: (sMap (map toLower) (inModel ^. defn)) :+: 
...
s4_2_5_deriv1 = [Paragraph (S "Derivation of the energy balance on " :+: 
                ...
                (sMap (map toLower) ((tank ^. term))) :+: S ", since it " :+:
...
s4_2_5_deriv2 = [Paragraph (S "Detailed derivation of the energy balance on" :+:
                ...
                toLower) ((solid ^. term))) :+: S " " :+: (phsChgMtrl ^. 
                ...
                S "expected with " :+: (sMap (map toLower) ((melting ^. 
                term))) :+: S ", this is not included, since the " :+:
                (volume ^. defn) :+: S " change of the " :+: (phsChgMtrl ^. 
                term) :+: S " with " :+: (sMap (map toLower) ((melting ^. 
                ...
                (sMap (map toLower) ((boiling ^. term))) :+: S " of the " :+: 
...
s4_2_7_deriv = [Paragraph (S "A correct solution must exhibit the " :+: 
               (sMap (map toLower) (law_cons_energy ^. defn)) :+:
               ...
               term) :+: S " from the " :+: (sMap (map toLower) ((water ^.
...
s5_intro = Paragraph (S "This section provides the functional " :+: (sMap (map 
           toLower) (requirement ^. defn)) :+: S "s, the business tasks " :+:
...
s5_1_list = [Enumeration (Simple [((requirement ^. term) :+: S "1", Flat 
            ...
            S "output the " :+: (time ^. defn) :+: S " at which the " :+: 
            (phsChgMtrl ^. term) :+:
            S " stops " :+: (sMap (map toLower) ((melting ^. term))) :+: 
...
s6_list = Enumeration (Simple [((likelyChg ^. term) :+: S "1", Flat 
          ...
          (sMap (map toLower) (thermal_conductor ^. defn)) :+: S ", so " :+:
          S "the " :+: (sMap (map toLower) (assumption ^. defn)) :+:
          ...
          (sMap (map toLower) ((water ^. term))) :+: S " within it cools.")),
...
s7_intro1 = Paragraph (S "The purpose of the traceability matrices is to " :+: 
            ...
            defn)) :+: S "s, " :+: (sMap (map toLower) (genDefn ^. defn)) :+:
            S "s, " :+: (sMap (map toLower) (dataDefn ^. defn)) :+: S "s, " :+:
            S "and " :+: (sMap (map toLower) (inModel ^. defn)) :+: S "s " :+:
            S "with each other. " :+: makeRef s7_table2 :+: S " shows the " :+:
            S "dependencies of " :+: (sMap (map toLower) (inModel ^. defn)) :+:
            S "s, " :+: (sMap (map toLower) (requirement ^. defn)) :+:
            S "s, and data constraints on each other. " :+: 
            makeRef s7_table3 :+: S " shows the dependencies of " :+: 
            (sMap (map toLower) (thModel ^. defn)) :+: S "s, " :+:
            (sMap (map toLower) (genDefn ^. defn)) :+: S "s, " :+: 
            (sMap (map toLower) (dataDefn ^. defn)) :+: S "s, " :+:
            (sMap (map toLower) (inModel ^. defn)) :+: S "s, and " :+: 
            (sMap (map toLower) (likelyChg ^. defn)) :+: S "s on the " :+:
            (assumption ^. defn) :+: S "s.")
...
s7_intro2 = [Paragraph (S "The purpose of the traceability graphs is also " :+:
            ...
            (sMap (map toLower) (assumption ^. defn)) :+: S "s on each " :+:
            S "other. " :+: makeRef s7_fig2 :+: S " shows the dependencies " :+:
            S "of " :+: (sMap (map toLower) (inModel ^. defn)) :+: S "s, " :+:
            (sMap (map toLower) (requirement ^. defn)) :+: S "s, and data " :+:
