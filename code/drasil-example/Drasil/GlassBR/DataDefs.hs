...
risk :: DataDefinition
risk = mkDD riskQD 
  (Just $ map makeCite [astm2009, beasonEtAl1998 {- FIXME +:+ sParen (S "Eq. 4-5") -},
...
hFromt = mkDD hFromtQD (Just [makeCite astm2009]) [{-derivation-}] "min_thick" [hMin]
...
loadDF = mkDD loadDFQD (Just [makeCite astm2009]) [{-derivation-}] "loadDurFactor" [makeRef2S assumpSV,
...
strDisFac = mkDD strDisFacQD (Just [makeCite astm2009]) [{-derivation-}] "stressDistFac"
  [jRef2, qHtRef, arRef]
...
nonFL = mkDD nonFLQD (Just [makeCite astm2009]) [{-derivation-}] "nFL"
...
glaTyFac = mkDD glaTyFacQD (Just [makeCite astm2009]) [{-derivation-}] "gTF"
...
dimLL = mkDD dimLLQD (Just $ map makeCite [astm2009, campidelli {- +:+ sParen (S "Eq. 7") -}])
  [{-derivation-}] "dimlessLoad"
...
tolPre = mkDD tolPreQD (Just [makeCite astm2009]) [{-derivation-}] "tolLoad"
  [qHtTlExtra]
...
tolStrDisFac = mkDD tolStrDisFacQD (Just [makeCite astm2009]) [{-derivation-}] "sdf_tol"
...
standOffDis = mkDD standOffDisQD (Just [makeCite astm2009]) [{-derivation-}] "standOffDist" []
...
aspRat = mkDD aspRatQD (Just [makeCite astm2009]) [{-derivation-}] "aspect_ratio" [aGrtrThanB]
...
eqTNTWDD = mkDD eqTNTWQD (Just [makeCite astm2009]) [] "eqTNTW" []
...
probOfBreak = mkDD probOfBreakQD (Just $ map makeCite [astm2009, beasonEtAl1998]) [{-derivation-}] "probOfBreak" [glassBreak]
...
calofCapacity = mkDD calofCapacityQD (Just [makeCite astm2009]) [{-derivation-}] "calofCapacity" capacityS
...
calofDemand = mkDD calofDemandQD (Just [makeCite astm2009]) [{-derivation-}] "calofDemand" [calofDemandDesc]
