...
that the equation for $h_c$ is written in our expression language (Expr) and the
units are defined by a piece of common knowledge known as \verb|heat_transfer|. We
use an EqChunk as it allows us to capture all of the relevant knowledge.
...
is composed of two sections: \verb|s1| and \verb|s2|. We then show the
definition of the first section which is titled "Table of Units" and includes an
introductory paragraph and a table. This table simply extracts the symbol and
description information from the SI units to display them in our view. It should
be fairly obvious at this point, but these macro-scale layout objects (sections,
tables, paragraphs, etc.) are specified using our document layout language.
