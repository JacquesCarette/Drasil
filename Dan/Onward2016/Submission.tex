...
software they are developing~\cite{Kelly}. %TODO: Fix citation, which year? 
...
DOC++ (for C++), Doxygen (for multiple languages), and more. New tools came with
new and interesting features, examples include:
...
the relevant code, data, etc. This means is called a ``compendium'' and it
encapsulates the full work of the author, as opposed to the publication version.
It also allows the reader to re-run computations (and explicitly shows the
computational details)~\cite{Lang2004}. The authors proposed that peer review
and distribution of scientific work should be done using compendia.
