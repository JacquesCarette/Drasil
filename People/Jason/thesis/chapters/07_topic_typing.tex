...
\Cref{chap:lang-division}, \Expr{} has become the expression language that
...
coherent and usable. At the moment, it is possible to give Drasil's instance
...
and generation steps to produce software artifacts. In order to build the type
context, we would need to build it manually, in a very tedious fashion, for each
expression we build. However, this does not scale well. In order to localize
...
By using this function anywhere before our code generation step, we are able to
...
Interestingly, we also found that we had used temporary operations used when we
meant other things (e.g., using absolute value in place where we intended to use
