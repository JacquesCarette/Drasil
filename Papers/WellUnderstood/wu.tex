...
\lstset{language=haskell, basicstyle=\ttfamily, breaklines=true,
...
risk = ddE riskQD
  ...
  where riskQD = mkQuantDef riskFun riskEq
      ...
      \end{verbatim}
    \item %3b
      As per the requirements template for scientific software\cite{SmithAndLai2005}\cite{SmithEtAl2007} by Spencer Smith and Lei Lai, we have a goal:
      \begin{verbatim}
...
willBreakGS = cic "willBreakGS" (foldlSent [S "Analyze" `S.and_`
  ...
  "Predict-Glass-Withstands-Explosion" goalStmtDom
    ...
    \item %3e
      TODO: Needs clarification
    \end{enumerate}
  \item\label{it:ex:refine} %4
    \begin{enumerate}
    \item %4a
      We might want to note one of our assumptions:
      \begin{verbatim}
