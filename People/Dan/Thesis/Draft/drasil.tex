...
\begin{itemize}
    ...
    integration.
	\item \ds{\codeH{gen...} Put the other ones here.}
\end{itemize}
...
After rendering the \gb{} SRS to \LaTeX{}, the \codeH{prntDoc'} function 
writes the source file, while \codeH{prntMake} generates a Makefile for 
...
For code, \codeH{createCodeFiles} (Figure~\ref{fig:createCodeFiles}) is called 
...
\ds{TODO: Add something about \codeH{render code} from the figure?}
...
\ds{TODO: Add a ref to the recipe fig here}
...
the target output (as a \ds{HTML/\LaTeX} table or 
section). \ds{TODO: Raw HTML/\LaTeX of the definition (figure)}
...
\ds{(PDF/HTML)} with $\hat{q}$ fully integrated and accessible 
alongside all other relevant information.
\ds{TODO: Figure of rendered HTML/LaTeX snippets}
...
\codeH{CodeSpec} recipe for \gb{}. The recipe dictates which variables and 
...
\ds{TODO: Fix the figure -- reference the existing one?}
\fig{
\codeBlock{CodeSpec}{40}{72}}{Reference to $\hat{q}$ in the code 
specification}{fig:qhatCodeSpecRef}
...
remaining details for the time being \ds{however, an intermediate 
representation can be seen in figure [ADD A FIGURE]}

\subsubsection{Generated Source Code}

Finally, the GOOL backend emits concrete code for $\hat{q}$ in each selected 
language. For example, in Python, $\hat{q}$ \ds{appears as a function computing 
its value from its dependencies, with comments referencing its origin and 
documentation... or something}. The generated code maintains traceability 
through naming conventions and comments drawn from the original chunk.

\ds{TODO: Python figure}
