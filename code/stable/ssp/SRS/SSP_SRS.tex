...
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{A:Geo-Slope-Mat-Props-of-Soil-Inputs}:]The geometry of the slope, and the material properties of the soil layers are given as inputs {[}\hyperref[IM:forDisEqlb]{Definition~IM:forDisEqlb}{]}.
\end{description}
\begin{description}
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & The Mohr-Coulomb resistive shear strength of a slice $τ$ from \hyperref[T:mcShrStrgth]{Definition~T:mcShrStrgth} is multiplied by the area $b \sec\left(α\right)\cdot{}1$ to obtain the resistive shear force $P$. Note the extra $1$ is to represent a unit of width which is multiplied by the total base length of a slice ${ℓ_{b}}$ of the plane where the normal occurs, where ${ℓ_{b}}=b \sec\left(α\right)$ and $b$ is the x width of the base. This accounts for the effective normal force $N'=N-{U_{b}}$ of a soil from \hyperref[T:effStress]{Definition~T:effStress} where the normal stress is multiplied by the same area to obtain the effective normal force $σ b \sec\left(α\right)\cdot{}1=N'$.
\\ \midrule \\
Source & \cite{chen2005}
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & From the definition of the factor of safety in \hyperref[T:factOfSafety]{Definition~T:factOfSafety}, and the new definition of $P$, a new relation for the net mobile shear force of the slice $T$ is found as the resistive shear $P$ (\hyperref[GD:resShr]{Definition~GD:resShr}) divided by the factor of safety $FS$.
\\ \midrule \\
Source & \cite{chen2005}
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & For a slice of mass in the slope the moment equilibrium to satisfy \hyperref[T:equilibrium]{Definition~T:equilibrium} in the direction perpendicular to the base surface of the slice. Moment equilibrium is derived from the free body diagram of Section~\ref{Sec:PhysSyst}. Index i refers to the values of the properties for slice/interslices following convention in Section~\ref{Sec:PhysSyst}. Variable definitions can be found in \hyperref[DD:sliceWght]{Definition~DD:sliceWght} to \hyperref[DD:lengthLs]{Definition~DD:lengthLs}.
\\ \midrule \\
Source & \cite{chen2005}
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{GD:netForcex}
\phantomsection 
\label{GD:netForcex}
\\ \midrule \\
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & This equation and the equation in \hyperref[GD:netForcey]{Definition~GD:netForcey} show the net sum of the forces acting on a slice for the RFEM model and the forces that create an applied load on the slice. ${F_{x}}$ refers to the load in the direction perpendicular to the direction of the force of gravity for slice $i$, while ${F_{y}}$ refers to the load in the direction parallel to the force of gravity for slice $i$. Forces are found in the free body diagram of Section~\ref{Sec:PhysSyst}. In this model the elements are not exerting forces on each other, so the interslice forces $G$ and $X$ are not a part of the model. Index $i$ refers to the values of the properties for slice/interslices following convention in Section~\ref{Sec:PhysSyst}. Force variable definitions can be found in \hyperref[DD:sliceWght]{Definition~DD:sliceWght} to \hyperref[DD:lengthLb]{Definition~DD:lengthLb}.
\\ \midrule \\
Source & \cite{chen2005}
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{GD:netForcey}
\phantomsection 
\label{GD:netForcey}
\\ \midrule \\
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & This equation and the equation in \hyperref[GD:netForcex]{Definition~GD:netForcex} show the net sum of the forces acting on a slice for the RFEM model and the forces that create an applied load on the slice. ${F_{x}}$ refers to the load in the direction perpendicular to the direction of the force of gravity for slice $i$, while ${F_{y}}$ refers to the load in the direction parallel to the force of gravity for slice $i$. Forces are found in the free body diagram of Section~\ref{Sec:PhysSyst}. In this model the elements are not exerting forces on each other, so the interslice forces $G$ and $X$ are not a part of the model. Index $i$ refers to the values of the properties for slice/interslices following convention in Section~\ref{Sec:PhysSyst}. Force variable definitions can be found in \hyperref[DD:sliceWght]{Definition~DD:sliceWght} to \hyperref[DD:lengthLb]{Definition~DD:lengthLb}.
\\ \midrule \\
Source & \cite{chen2005}
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{DD:resShearWO}
\phantomsection 
\label{DD:resShearWO}
\\ \midrule \\
Label & Resistive shear force
...
\\ \bottomrule \end{tabular}
\end{minipage}\\
The resistive shear force of a slice is defined as $P$ in \hyperref[GD:resShr]{Definition~GD:resShr}. The effective normal force in the equation for $P$ of the soil is defined in the perpendicular force equilibrium of a slice from \hyperref[GD:bsShrFEq]{Definition~GD:bsShrFEq}, using the effective normal force $N'$ of \hyperref[T:effStress]{Definition~T:effStress} shown in equation (1):
\begin{dmath}
...
\end{dmath}
The values of the interslice forces $G$ and $X$ in the equation are unknown, while the other values are found from the physical force definitions of \hyperref[DD:sliceWght]{Definition~DD:sliceWght} to \hyperref[DD:lengthLs]{Definition~DD:lengthLs}. Consider a force equilibrium without the affect of interslice forces, to obtain a solvable value as done for $N*$ in equation (2):
\begin{dmath}
...
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{DD:mobShearWO}
\phantomsection 
\label{DD:mobShearWO}
\\ \midrule \\
...
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{DD:displcmntRxnF}
\phantomsection 
\label{DD:displcmntRxnF}
\\ \midrule \\
Label & Pressure
\\ \midrule \\
Symbol & $p$
...
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{DD:displcmntBasel}
\phantomsection 
\label{DD:displcmntBasel}
\\ \midrule \\
Label & Pressure
\\ \midrule \\
Symbol & $p$
...
\\ \bottomrule \end{tabular}
\end{minipage}\\
Using the force-displacement relationship of \hyperref[GD:netForcey]{Definition~GD:netForcey} to define stiffness matrix ${K_{st}}$, as seen in equation (6).
\begin{dmath}
...
\end{bmatrix}
\end{dmath}
For interslice surfaces the stiffness constants and displacements refer to an unrotated coordinate system, $δ$ of \hyperref[DD:lengthLs]{Definition~DD:lengthLs}. The interslice elements are left in their standard coordinate system, and therefore are described by the same equation from \hyperref[GD:netForcey]{Definition~GD:netForcey}. Seen as ${K_{st}}$ in \hyperref[DD:intrsliceF]{Definition~DD:intrsliceF}. ${K_{st}}$ is the shear element in the matrix, and ${K_{sn}}$ is the normal element in the matrix, calculated as in \hyperref[DD:mobShearWO]{Definition~DD:mobShearWO}.
For basal surfaces the stiffness constants and displacements refer to a system rotated for the base angle alpha (\hyperref[DD:angleA]{Definition~DD:angleA}). To analyze the effect of force-displacement relationships occurring on both basal and interslice surfaces of an element $i$ they must reference the same coordinate system. The basal stiffness matrix must be rotated counter clockwise to align with the angle of the basal surface. The base stiffness counter clockwise rotation is applied in equation (7) to the new matrix $N*$.
\begin{dmath}
{K_{st,i}}=\begin{bmatrix}
\cos\left(α_{i}\right) & -\sin\left(α_{i}\right)\\
\sin\left(α_{i}\right) & \cos\left(α_{i}\right)
...
\end{bmatrix}
\end{dmath}
The Hooke's law force displacement relationship of \hyperref[GD:netForcey]{Definition~GD:netForcey} applied to the base also references a displacement vector $ε$ of \hyperref[GD:hookesLaw2d]{Definition~GD:hookesLaw2d} rotated for the base angle of the slice $α$ The basal displacement vector. $δ$ is rotated clockwise to align with the interslice displacement vector $δ$, applying the definition of $ε$ in terms of $δ$ as seen in \hyperref[GD:hookesLaw2d]{Definition~GD:hookesLaw2d}. Using this with base stiffness matrix ${K_{bt}}$, a basal force displacement relationship in the same coordinate system as the interslice relationship can be derived as done in equation (8).
\begin{dmath}
\begin{bmatrix}
p_{i}\\
p_{i}
...
\end{bmatrix}
\end{dmath}
The new effective base stiffness matrix ${K_{bt}}$ as derived in equation (7) is defined in equation (9). This is seen as matrix ${K_{bt}}$ in \hyperref[DD:displcmntBasel]{Definition~DD:displcmntBasel} ${K_{bt}}$ is the shear element in the matrix, and ${K_{bn}}$ is the normal element in the matrix, calculated as in \hyperref[DD:mobShearWO]{Definition~DD:mobShearWO}. The notation is simplified by the introduction of the constants ${K_{bA}}$ and ${K_{bB}}$, defined in equation (10) and equation (11) respectively.
\begin{dmath}
...
\end{dmath}
A force-displacement relationship for an element $i$ can be written in terms of displacements occurring in the unrotated coordinate system $δ$ of \hyperref[GD:hookesLaw2d]{Definition~GD:hookesLaw2d} using the matrix ${K_{bt}}$, and ${K_{bt}}$ as seen in \hyperref[DD:intrsliceF]{Definition~DD:intrsliceF}.
~\newline
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{DD:netFDsplcmntEqbm}
\phantomsection 
\label{DD:netFDsplcmntEqbm}
\\ \midrule \\
Label & Force
\\ \midrule \\
Symbol & $F$
...
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{DD:shearStiffness}
\phantomsection 
\label{DD:shearStiffness}
\\ \midrule \\
...
\\ \midrule \\
Equation & \begin{dmath}
           {K_{bt}}=\frac{G}{2 \left(1+ν\right)} \frac{100.0\cdot{}10^{-3}}{b}+\frac{{c'}_{i}-σ \tan\left({φ'}_{i}\right)}{|δu|+a}
           \end{dmath}
\\ \midrule \\
Description & \begin{symbDescription}
...
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
\toprule \textbf{Refname} & \textbf{DD:soilStiffness}
\phantomsection 
\label{DD:soilStiffness}
\\ \midrule \\
...
100.0\cdot{}10^{-3} \frac{G \left(1-ν\right)}{\left(1+ν\right) \left(1-2 ν+b\right)}+\frac{κ}{δv+A}, & ν\geq{}0
\end{cases}
           \end{dmath}
\\ \midrule \\
Description & \begin{symbDescription}
...
\\ \bottomrule \end{tabular}
\end{minipage}\\
Using equation (21) from \hyperref[IM:intsliceFs]{Definition~IM:intsliceFs}, rearranging, and applying the boundary condition that $G_{0}$ and $G_{n}$ are equal to $0$, an equation for the factor of safety is found as equation (12), also seen in \hyperref[IM:fctSfty]{Definition~IM:fctSfty}:
\begin{dmath}
FS=\frac{\displaystyle\sum_{v=1}^{n-1}{R_{v} \displaystyle\prod_{u=i}^{n-1}{\frac{Ψ_{u}}{Φ_{u}}}}+R_{n}}{\displaystyle\sum_{v=1}^{n-1}{T_{v} \displaystyle\prod_{u=i}^{n-1}{\frac{Ψ_{u}}{Φ_{u}}}}+T_{n}}
\end{dmath}
The constants $Ψ$ and $Φ$ described in equation (20) and equation (19) are functions of the unknowns: the interslice normal/shear force ratio $λ$ (\hyperref[IM:nrmShrFor]{Definition~IM:nrmShrFor}) and the factor of safety $FS$ (\hyperref[IM:fctSfty]{Definition~IM:fctSfty}).
~\newline
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
...
\end{dmath}
Taking the base shear force equilibrium of \hyperref[GD:bsShrFEq]{Definition~GD:bsShrFEq} with the definition of mobilized shear force from \hyperref[GD:mobShr]{Definition~GD:mobShr} and the assumption of \hyperref[GD:normShrR]{Definition~GD:normShrR}, the equilibrium equation can be rewritten as equation (17):
\begin{dmath}
...
\end{dmath}
Where $R$ and $T$ are the resistive and mobile shear of the slice, without the influence of interslice forces $G$ and $X$, as defined in \hyperref[DD:resShearWO]{Definition~DD:resShearWO} and \hyperref[DD:mobShearWO]{Definition~DD:mobShearWO} Making use of the constants, and with full equations found below in equation (19) and equation (20) respectively, then equation (18) can be simplified to equation (21), also seen in \hyperref[IM:intsliceFs]{Definition~IM:intsliceFs}:
\begin{dmath}
...
\\ \bottomrule \end{tabular}
\end{minipage}\\
Using the net force-displacement equilibrium equation of a slice from \hyperref[DD:netFDsplcmntEqbm]{Definition~DD:netFDsplcmntEqbm} with the definitions of the stiffness matrices from \hyperref[DD:displcmntRxnF]{Definition~DD:displcmntRxnF} and the force definitions from \hyperref[GD:netForcex]{Definition~GD:netForcex} a broken down force displacement equilibrium equation can be derived. equation (22) gives the broken down equation in the $x$ direction, and equation (23) gives the broken down equation in the $y$ direction:
\begin{dmath}
-{ΔH}_{i}-{K_{c}} W_{i}-{U_{b,i}} \sin\left(α_{i}\right)+{U_{t,i}} \sin\left(β_{i}\right)+Q_{i} \sin\left(ω_{i}\right)={δx}_{i-1} -{ℓ_{s,i-1}} {K_{sn,i-1}}+{δx}_{i} \left(-{ℓ_{s,i-1}} {K_{sn,i-1}}+{ℓ_{s,i}} {K_{sn,i}}+{ℓ_{b,i}} {K_{bA,i}}\right)+{δx}_{i+1} -{ℓ_{s,i}} {K_{sn,i}}+{δy}_{i} -{ℓ_{b,i}} {K_{bB,i}}=-W_{i}-{U_{b,i}} \cos\left(α_{i}\right)+{U_{t,i}} \cos\left(β_{i}\right)+Q_{i} \cos\left(ω_{i}\right)={δy}_{i-1} -{ℓ_{s,i-1}} {K_{st,i-1}}+{δy}_{i} \left(-{ℓ_{s,i-1}} {K_{st,i-1}}+{ℓ_{s,i}} {K_{sn,i}}+{ℓ_{b,i}} {K_{bA,i}}\right)+{δy}_{i+1} -{ℓ_{s,i}} {K_{st,i}}+{δx}_{i} -{ℓ_{b,i}} {K_{bB,i}}
\end{dmath}
Using the known input assumption of A\ref{A:Geo-Slope-Mat-Props-of-Soil-Inputs}, the force variable definitions of \hyperref[DD:sliceWght]{Definition~DD:sliceWght} to \hyperref[DD:surfLoads]{Definition~DD:surfLoads} on the left side of the equations can be solved for. The only unknown in the variables to solve for the stiffness values from \hyperref[DD:soilStiffness]{Definition~DD:soilStiffness} is the displacements. Therefore taking the equation from each slice a set of $2 n$ equations, with $2 n$ unknown displacements in the $x$ and $y$ directions of each slice can be derived. Solutions for the displacements of each slice can then be found. The use of displacement in the definition of the stiffness values makes the equation implicit, which means an iterative solution method, with an initial guess for the displacements in the stiffness values is required.
~\newline
\noindent \begin{minipage}{\textwidth}
\begin{tabular}{p{0.2\textwidth} p{0.73\textwidth}}
...
\\ \bottomrule \end{tabular}
\end{minipage}\\
RFEM analysis can also be used to calculate the factor of safety for the slope. For a slice element $i$ the displacements $δx$ and $δy$, are solved from the system of equations in \hyperref[IM:forDisEqlb]{Definition~IM:forDisEqlb}. The definition of $ε$ as the rotation of the displacement vector $δ$ is seen in \hyperref[GD:hookesLaw2d]{Definition~GD:hookesLaw2d}. This is used to find the displacements of the slice parallel to the base of the slice $δu$ in equation (24) and normal to the base of the slice $δv$ in equation (25).
\begin{dmath}
...
\end{dmath}
With the definition of normal stiffness from \hyperref[DD:soilStiffness]{Definition~DD:soilStiffness} to find the normal stiffness of the base ${K_{bn}}$ and the now known base displacement perpendicular to the surface $δv$ from equation (25) the normal base stress can be calculated from the force-displacement relationship of \hyperref[T:hookesLaw]{Definition~T:hookesLaw}. Stress $σ$ is used in place of force $F$ as the stiffness hasn't been normalized for the length of the base. Results in equation (26):
\begin{dmath}
σ_{i}={K_{bn,i}} {δv}_{i}
\end{dmath}
The resistive shear to calculate the factor of safety $FS$ is found from the Mohr Coulomb resistive strength of soil in \hyperref[T:mcShrStrgth]{Definition~T:mcShrStrgth} Using the normal stress $σ$ from equation (26) as the stress, the resistive shear of the slice can be calculated from equation (27):
\begin{dmath}
...
\begin{dmath}
{K_{bt,i}}=\frac{G_{i}}{2 \left(1+ν_{i}\right)} \frac{100.0\cdot{}10^{-3}}{b_{i}}+\frac{{c'}_{i}-σ_{i} \tan\left({φ'}_{i}\right)}{|{δu}_{i}|+a}
\end{dmath}
With shear stiffness ${K_{bt}}$ calculated in equation (28) and shear displacement $δu$ calculated in equation (24) values now known the resistive shear stress acting on the base of a slice $τ$ can be calculated using \hyperref[T:hookesLaw]{Definition~T:hookesLaw}, as done in equation (29). Again, stress $τ$ is used in place of force $F$ as the stiffness has not been normalized for the length of the base:
\begin{dmath}
τ_{i}={K_{bt,i}} {δu}_{i}
\end{dmath}
The resistive shear stress acting on the base of a slice $τ$ acts as the mobile shear acting on the base. Using the definition Factor of Safety equation from \hyperref[T:factOfSafety]{Definition~T:factOfSafety}, with the definitions of resistive shear strength of a slice $s$ from equation (27) and resistive shear stress $τ$ from equation (29) the local factor of safety ${FS_{Loc,i}}$ can be found from as seen in equation (30) and \hyperref[IM:rfemFoS]{Definition~IM:rfemFoS}:
\begin{dmath}
{FS_{Loc,i}}=\frac{s_{i}}{τ_{i}}=\frac{{c'}_{i}-{K_{bn,i}} {δv}_{i} \tan\left({φ'}_{i}\right)}{{K_{bt,i}} {δu}_{i}}
\end{dmath}
The global Factor of Safety is then the ratio of the summation of the resistive and mobile shears for each slice, with a weighting for the length of the slice's base. Shown in equation (31) and \hyperref[IM:rfemFoS]{Definition~IM:rfemFoS}:
\begin{dmath}
