...
(programming language independent), and more. Tools such as javadoc (for Java)
and Doxygen (for multiple languages) were also influenced by LP. The development
of new tools led to the introduction of many new features including, but not
...
domains (for example: Agda, Haskell, and R support LP to some extent). I will
use the knowledge gained throughout LP's history to help develop the Drasil
framework into a full-featured and useful tool.
...
including, but not limited to, Sweave~\cite{SchulteEtAl2012}, SASweave~\cite{},
Statweave~\cite{}, Scribble~\cite{}, and Org-mode~\cite{}. The most popular of
those being Sweave~\cite{SchulteEtAl2012}. The aforementioned tools maintain a
focus on code and certain computational details. It is my hope that the Drasil
framework will one day surpass these tools due to its ability to simplify the
creation of each software artifact.
