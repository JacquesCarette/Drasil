...
acceleration           = uc CP.acceleration           (Concat [vec lA, label "(", lT, label ")"]) (vectND "n" Real)   accelU
...
displacement           = uc CP.displacement           (vec lU)                                    (vectND "n" Real)         metre
distance               = uc CP.distance               lD                                          Real        metre
energy                 = uc CP.energy                 cE                                          Real        joule
force                  = uc CP.force                  (vec cF)                                    (vectND "n" Real)         newton
frequency              = uc CP.frequency              lF                                          Real        hertz
gravitationalAccel     = uc CP.gravitationalAccel     (vec lG)                                    (vectND "n" Real)         accelU
...
impulseV               = uc CP.impulseV               (vec cJ)                                    (vectND "n" Real)        impulseU
...
momentOfInertia        = uc CP.momentOfInertia        (vec cI)                                    (vectND "n" Real)         momtInertU
chgMomentum            = uc CP.chgMomentum            (Concat [cDelta,vec cP])                    Real        impulseU
momentum               = uc CP.momentum               (vec cP)                                    (vectND "n" Real)         impulseU
moment                 = uc CP.moment                 (vec cM)                                    (vectND "n" Real)         torqueU
...
positionVec            = uc CP.positionVec            (vec lR)                                    (vectND "n" Real)         metre
...
tension                = uc CP.tension                (vec cT)                                    (vectND "n" Real)         newton
time                   = uc CP.time                   lT                                          Real        second
torque                 = uc CP.torque                 (vec lTau)                                  (vectND "n" Real)         torqueU
velocity               = uc CP.velocity               (Concat [vec lV, label "(", lT, label ")"]) (vectND "n" Real)        velU
