...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & The net torque $τ$ on a rigid body is proportional to its angular acceleration $α$.
        
        Here, $\mathbf{I}$ denotes the moment of inertia of the rigid body.
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & Kinetic energy is The measure of the energy a body possess due to its motion..
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & The coefficient of restitution ${C_{\text{R}}}$ determines the elasticity of a collision between two rigid bodies. ${C_{\text{R}}}=1$ results in an elastic collision, ${C_{\text{R}}}<1$ results in an inelastic collision, and ${C_{\text{R}}}=0$ results in a totally inelastic collision.
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & The above equation expresses the total acceleration of the rigid body $i$ as the sum of gravitational acceleration (GD3) and acceleration due to applied force ${\mathbf{F}_{i}}\left(t\right)$ (from \hyperref[TM:NewtonSecLawMot]{TM: NewtonSecLawMot}). The resultant outputs are then obtained from this equation using \hyperref[DD:linDisp]{DD: linDisp}, \hyperref[DD:linVel]{DD: linVel}, and \hyperref[DD:linAcc]{DD: linAcc}. \hyperref[DD:ctrOfMass]{DD: ctrOfMass}.
        
        All bodies are assumed to be rigid (from \hyperref[assumpOT]{A: objectTy}) and two-dimensional (from \hyperref[assumpOD]{A: objectDimension}).
        
        It is currently assumed that there is no damping occurs during the simulation (from \hyperref[assumpDI]{A: dampingInvolvement}) and that no constraints are involved (from \hyperref[assumpCAJI]{A: constraintsAndJointsInvolvement}).
...
\hyperref[GD:accelGravity]{GD: accelGravity} &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  &  & 
\\
\hyperref[IM:transMot]{IM: transMot} & X & X & X & X &  &  &  &  &  &  &  &  &  &  &  &  & X &  &  &  &  &  &  & 
\\
