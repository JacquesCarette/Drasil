...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_const_density}:] Density
  of the water and PCM have not spatial variation; that is, they are each
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_int_heat}:] No internal
  heat is generated be either the water or the PCM; therefore, the volumetric
...
\hline
  Source &
  \url{http://www.thermopedia.com/content/975/}\\
  % The above web link should be replaced with a proper citation to a publication
  \hline
...
\noindent \begin{longtable}{l l l c} 
  \toprule
  \textbf{Var} & \textbf{Physical Constraints} & \textbf{Typical Value} & \textbf{Uncertainty}\\
  \midrule 
  $L$	& $L > 0$		& 1.5 \si[per-mode=symbol]	{\metre}	& 10\%
  \\
  $D$	& $D > 0$		& 0.412 \si[per-mode=symbol] {\metre}	& 10\%
  \\
  $V_P$ & $V_P > 0$ (*)	& 0.05 \si[per-mode=symbol] {\cubic\metre}	& 10\%
  \\
  $A_P$ & $A_P > 0$ (*)	& 1.2 \si[per-mode=symbol] {\square\metre}	& 10\%
  \\
  $\rho_P$ & $\rho_P > 0$	& 1007 \si[per-mode=symbol] {\kilogram\per\cubic\metre}	& 10\%
  \\
  $T_\text{melt}^{P}$ 	& $0 < T_\text{melt}^{P} < T_C$	&	44.2 \si[per-mode=symbol] {\celsius} & 10\%
  \\
  $C_P^S$ & $C_P^S > 0$ & 1760 \si[per-mode=symbol] {\joule\per\kilo\gram\per\celsius} & 10\%
  \\
  $C_P^L$ & $C_P^L > 0$ & 2270 \si[per-mode=symbol] {\joule\per\kilo\gram\per\celsius} & 10\%
  \\
  $H_f$ & $H_f > 0$ &	211600 \si[per-mode=symbol] {\joule\per\kilo\gram} & 10\%
  \\
  $A_C$ & $A_C > 0$ (*)	& 0.12 \si[per-mode=symbol] {\square\metre}	& 10\%
  \\
  $T_C$ & $T_C > 0$ (+)	& 50 \si[per-mode=symbol] {\celsius} & 10\%
  \\
  $\rho_W$ & $\rho_W > 0$	& 1000 \si[per-mode=symbol] {\kilo\gram\per\cubic\metre} & 10\%
  \\
  $C_W$ & $C_W > 0$	& 4186 \si[per-mode=symbol] {\joule\per\kilo\gram\per\celsius} & 10\%
  \\
  $h_C$ & $h_C > 0$	& 1000 \si[per-mode=symbol] {\watt\per\square\metre\per\celsius} & 10\%
  \\
  $h_P$ & $h_P > 0$	& 1000 \si[per-mode=symbol] {\watt\per\square\metre\per\celsius} & 10\%
  \\
  $T_\text{init}$ & $0 < T_\text{init} < T_\text{melt} $ (+) & 40 \si[per-mode=symbol] {\celsius} & 10\%
  \\
  $t_\text{final}$ & $t_\text{final} > 0$ & 50000 \si[per-mode=symbol] {\second} & 10\%
  \\
  \bottomrule
\end{longtable}
\end{table}
