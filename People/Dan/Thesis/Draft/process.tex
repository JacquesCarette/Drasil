...
commonality we will ignore superficial differences between the MG sections.
...
Changes" section, we see that again the introductions to the section and 
subsections are generic boilerplate, however the details of each section are 
not. Both subsections are written in the same way: as a list of labeled 
...
Level 1 modules include: Hardware-Hiding Module, Behaviour-Hiding Module, and 
Software Decision Module), but the modules within are problem-domain or system 
specific. An example of a system-specific module is shown in 
Figure~\ref{fig:gbrmgcm}. 

Each module is described by its secrets, services, 
and what it will be implemented by. For example, a given module could be 
implemented by the operating system (OS), the system being described (ex. 
\gb{}), or a third party system that will be interfaced with the given system.
...
obvious link between the SRS and the MG, where other connections between the 
two \sfs{} have been implicit until this point. Generally, the next \sf{} would 
be the MIS, however as it is structured so similarly to the MG (one section per 
module, each section is organized the same way, the use hierarchy is repeated, 
...
for our exercise does not provide any revelations beyond that of the MG. We 
continue by looking into the details of the source code itself.
