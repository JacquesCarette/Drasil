...
flightDur' = int 2 `mulRe` sy launSpeed `mulRe` sin (sy launAngle) $/ sy gravitationalAccelConst
iyPos = int 0                                   -- launchOrigin
yConstAccel = neg $ sy gravitationalAccelConst  -- accelYGravity
iSpeed = sy launSpeed

timeDerivEqn1, timeDerivEqn2, timeDerivEqn3, timeDerivEqn4 :: Expr
timeDerivEqn1 = sy yPos $= (sy iyVel `mulRe` sy time) $- ((sy gravitationalAccelConst `mulRe` square (sy time)) $/ int 2)
timeDerivEqn2 = (sy iyVel `mulRe` sy flightDur) $- ((sy gravitationalAccelConst `mulRe` square (sy flightDur)) $/ int 2) $= int 0
timeDerivEqn3 = sy iyVel $- ((sy gravitationalAccelConst `mulRe` sy flightDur) $/ int 2) $= int 0
timeDerivEqn4 = sy flightDur $= int 2 `mulRe` sy iyVel $/ sy gravitationalAccelConst

landPosExpr, landPosDerivEqn1, landPosDerivEqn2, landPosDerivEqn3 :: Expr
landPosExpr = int 2 `mulRe` square (sy launSpeed) `mulRe` sin (sy launAngle) `mulRe` cos (sy launAngle) $/ sy gravitationalAccelConst
landPosDerivEqn1 = sy xPos    $= sy ixVel `mulRe` sy time
landPosDerivEqn2 = sy landPos $= sy ixVel `mulRe` int 2 `mulRe` sy launSpeed `mulRe` sin (sy launAngle) $/ sy gravitationalAccelConst
landPosDerivEqn3 = sy landPos $= sy launSpeed `mulRe` cos (sy launAngle) `mulRe` int 2 `mulRe` sy launSpeed `mulRe` sin (sy launAngle) $/ sy gravitationalAccelConst
...
message = completeCase [case1, case2, case3]
  where case1 = (Str "The target was hit.",        abs_ (sy offset $/ sy targPos) $< sy tol)
        case2 = (Str "The projectile fell short.", sy offset $< int 0)
        case3 = (Str "The projectile went long.",  sy offset $> int 0)
...
rectVelDerivEqn2 = defint (eqSymb speed) (sy QP.iSpeed) (sy speed) (int 1) $=
                   defint (eqSymb time) (int 0) (sy time) (sy QP.constAccel)

scalarPos' :: Expr
scalarPos' = sy iPos `addRe` (sy QP.iSpeed `mulRe` sy time `addRe` ((sy QP.constAccel `mulRe` square (sy time)) $/ int 2))

rectPosDerivEqn1, rectPosDerivEqn2, rectPosDerivEqn3 :: Expr
rectPosDerivEqn1 = sy speed $= deriv (sy scalarPos) time
rectPosDerivEqn2 = defint (eqSymb scalarPos) (sy iPos) (sy scalarPos) (int 1) $=
                   defint (eqSymb time) (int 0) (sy time) (sy speed)
rectPosDerivEqn3 = defint (eqSymb scalarPos) (sy iPos) (sy scalarPos) (int 1) $=
                   defint (eqSymb time) (int 0) (sy time) (sy QP.iSpeed `addRe` (sy QP.constAccel `mulRe` sy time))
...
posVecExpr :: Expr 
posVecExpr = vec2D
              (sy QP.ixPos `addRe` (sy ixVel `mulRe` sy time) `addRe` ((sy QP.xConstAccel `mulRe` square (sy time)) $/ int 2))
              (sy QP.iyPos `addRe` (sy iyVel `mulRe` sy time) `addRe` ((sy QP.yConstAccel `mulRe` square (sy time)) $/ int 2))
