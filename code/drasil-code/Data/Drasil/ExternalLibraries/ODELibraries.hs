...
import Language.Drasil (HasSymbol(symbol), HasUID(uid), MayHaveUnit(getUnit),
  ...
  Idea(getA), NamedIdea(term), Stage(..), (+++.), showUID)
...
vecDepVar info = quantvar $ implVar (showUID dv) (dv ^. term) vecT
...
arrayVecDepVar info = quantvar $ implVar (showUID dv ++ "vec") (dv ^. term)
...
diffCodeChunk c = quantvar $ implVar' ("d" ++ showUID c )
