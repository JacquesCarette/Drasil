...
\hline
\rule{0pt}{4.5ex}\sw{} & Investigate how phase change material (PCM) affects 
heating and energy storage in a solar water heating tank. \\
\hline
\rule{0pt}{4.5ex}\np{} & Investigate heating and energy storage in a solar 
...
Figure~\ref{fig:gptos} (\gp{}). These variations reveal the obvious: the 
variability between systems is greater than simply a difference in choice of 
symbols, and so there is some system-specific knowledge being encoded. While we 
can intuitively infer this conclusion based solely on each system addressing a 
different problem, our observation of the (structural) patterns within this SRS 
section confirms it.
...
where $\hat{q}$ appears helps make the repetition more explicit:

\begin{itemize}
\item In the SRS table of symbols, where $\hat{q}$ is introduced in abbreviated 
form.
\item In the data definition shown in Figure~\ref{fig:gbrddq}, where it is 
defined more fully.
\item In other SRS data definitions, such as the Stress Distribution Factor 
($J$) and Non-Factored Load ($NFL$), which refer to $\hat{q}$ directly.
\item In the SRS calculation of Capacity ($LR$), where $\hat{q}$ appears 
indirectly through intermediary quantities such as $NFL$.
\item In the MG, where the \emph{loadASTM} and \emph{Calc} modules mention or 
rely on the quantity.
\item In the source code, where the corresponding calculation is implemented 
explicitly (Figure~\ref{fig:gbrsrccalc}, lines 11-14).
\end{itemize}
