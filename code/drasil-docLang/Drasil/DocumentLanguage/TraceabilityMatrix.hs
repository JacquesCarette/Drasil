...
generateTraceTableView u _ [] _ _ = error $ "Expected non-empty list of column-view categories for traceability matrix " ++ uidToStr u
generateTraceTableView u _ _ [] _ = error $ "Expected non-empty list of row-view categories for traceability matrix " ++ uidToStr u
generateTraceTableView u desc cols rows c = llcc (makeTabRef $ uidToStr u) $ Table
...
ensureItems :: UID -> [a] -> [a]
ensureItems u [] = error $ "Expected non-empty matrix dimension for traceability matrix " ++ uidToStr u
ensureItems _ l = l
