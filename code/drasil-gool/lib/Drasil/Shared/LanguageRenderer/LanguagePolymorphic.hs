...
multiBody :: (CommonRenderSym r tp vis, Monad r) => [MSBody r] -> MS (r Doc)
...
csc :: (CommonRenderSym r TypeData vis, TypeElim r TypeData) => SValue r -> SValue r
csc v = valOfOne (fmap valueType v) #/ sin v

sec :: (CommonRenderSym r TypeData vis, TypeElim r TypeData) => SValue r -> SValue r
sec v = valOfOne (fmap valueType v) #/ cos v

cot :: (CommonRenderSym r TypeData vis, TypeElim r TypeData) => SValue r -> SValue r
cot v = valOfOne (fmap valueType v) #/ tan v

valOfOne :: (CommonRenderSym r TypeData vis, TypeElim r TypeData) =>
...
smartAdd :: (CommonRenderSym r tp vis) => SValue r -> SValue r -> SValue r
smartAdd v1 v2 = do
  v1' <- v1
  v2' <- v2
...
smartSub :: (CommonRenderSym r tp vis) => SValue r -> SValue r -> SValue r
smartSub v1 v2 = do
  v1' <- v1
  v2' <- v2
  case (RC.valueInt v1', RC.valueInt v2') of
...
var :: (CommonRenderSym r tp vis) => Label -> VS (r tp) -> SVariable r
var n t = mkStateVar n t (R.var n)

classVar :: (CommonRenderSym r tp vis) => Label -> VS (r tp) -> SVariable r
...
classVarAccessCheck :: (CommonRenderSym r tp vis) => r (Variable r) -> r (Variable r)
...
instanceVarAccess :: (CommonRenderSym r tp vis) => SValue r -> SVariable r -> SVariable r
...
arrayElem :: (OORenderSym r tp vis) => SValue r -> SValue r -> SVariable r
arrayElem arr' i' = do
  i <- IC.intToIndex i'
  arr <- arr'
...
litChar :: (CommonRenderSym r tp vis) => (Doc -> Doc) -> Char -> SValue r
litChar f c = mkStateVal IC.char (f $ if c == '\n' then text "\\n" else D.char c)

litDouble :: (CommonRenderSym r tp vis) => Double -> SValue r
litDouble d = mkStateVal IC.double (D.double d)

litInt :: (CommonRenderSym r tp vis) => Integer -> SValue r
litInt i = valFromData Nothing (Just i) IC.int (integer i)

litString :: (CommonRenderSym r tp vis) => String -> SValue r
litString s = mkStateVal IC.string (doubleQuotedText s)

valueOf :: (CommonRenderSym r tp vis) => SVariable r -> SValue r
valueOf v' = do
  v <- v'
  mkVal (variableType v) (RC.variable v)

arg :: (CommonRenderSym r tp vis) => SValue r -> SValue r -> SValue r
arg n' args' = do
  n <- n'
  args <- args'
  s <- IC.string
...
argsList :: (CommonRenderSym r tp vis) => String -> SValue r
...
call :: (CommonRenderSym r tp vis) => Doc -> Maybe Library -> Maybe Doc ->
  MixedCall r tp
...
funcAppMixedArgs :: (CommonRenderSym r tp vis) => MixedCall r tp
funcAppMixedArgs = S.call Nothing Nothing

newObjMixedArgs :: (CommonRenderSym r TypeData vis, UnRepr r TypeData) =>
  String -> MixedCtorCall r TypeData
newObjMixedArgs s tp vs ns = do
  t <- tp
  S.call Nothing Nothing (s ++ getTypeString t) (return t) vs ns

lambda :: (CommonRenderSym r tp vis) => ([r BinderD] -> r (Value r) -> Doc) ->
...
objAccess :: (CommonRenderSym r tp vis) => SValue r -> VSFunction r -> SValue r
objAccess = on2StateWrapped (\v f-> mkVal (functionType f)
  (R.objAccess (RC.value v) (RC.function f)))

objMethodCall :: (CommonRenderSym r tp vis) => Label -> VS (r tp) -> SValue r ->
...
func :: (CommonRenderSym r tp vis) => Label -> VS (r tp) -> [SValue r] -> VSFunction r
func l t vs = funcApp l t vs >>= ((`funcFromData` t) . R.func . RC.value)

get :: (OORenderSym r tp vis) => SValue r -> SVariable r -> SValue r
get v vToGet = v $. S.getFunc vToGet

set :: (OORenderSym r tp vis) => SValue r -> SVariable r -> SValue r -> SValue r
...
listAccess :: (CommonRenderSym r tp vis, TypeElim r tp) => SValue r ->
  SValue r -> SValue r
listAccess v i = do
  v' <- v
...
getFunc :: (OORenderSym r tp vis) => SVariable r -> VSFunction r
getFunc v = v >>= (\vr -> IG.func (getterName $ variableName vr)
  (toState $ variableType vr) [])

setFunc :: (OORenderSym r tp vis) => VS (r tp) -> SVariable r -> SValue r -> VSFunction r
...
param :: (CommonRenderSym r tp vis) => (r (Variable r) -> Doc) -> SVariable r ->
  MSParameter r
param f v' = do
  v <- zoom lensMStoVS v'
...
method :: (OORenderSym r tp vis) => Label -> r vis -> r (Attachment r) ->
  VS (r tp) -> [MSParameter r] -> MSBody r -> SMethod r
method n s p t = intMethod False n s p (mType t)

getMethod :: (OORenderSym r tp vis) => SVariable r -> SMethod r
...
setMethod :: (OORenderSym r tp vis) => SVariable r -> SMethod r
...
initStmts :: (OORenderSym r tp vis) => Initializers r tp -> MSBody r
initStmts = bodyStatements . map (\(vr, vl) -> IG.instanceVarSelf vr &= vl)

function :: (OORenderSym r tp vis) => Label -> r vis -> VS (r tp) ->
  [MSParameter r] -> MSBody r -> SMethod r
function n s t = S.intFunc False n s classLevel (mType t)

docFuncRepr :: (CommonRenderSym r tp vis) => FuncDocRenderer -> String ->
...
docFunc :: (CommonRenderSym r tp vis) => FuncDocRenderer -> String -> [String] ->
  Maybe String -> SMethod r -> SMethod r
docFunc f desc pComms rComm = docFuncRepr f desc pComms (maybeToList rComm)

-- Classes --

buildClass :: (OORenderSym r tp vis) =>  Maybe Label -> [CSStateVar r] ->
...
implementingClass :: (OORenderSym r tp vis) => Label -> [Label] ->
  [CSStateVar r] -> [SMethod r] -> [SMethod r] -> SClass r
implementingClass n is = S.intClass n public (implements is)

docClass :: (OORenderSym r tp vis) => ClassDocRenderer -> String -> SClass r -> SClass r
docClass cdr d = S.commentedClass (docComment $ toState $ cdr d)

commentedClass :: (OORenderSym r tp vis, Monad r) => CS (r Doc) -> SClass r
  -> CS (r Doc)
...
fileDoc :: (OORenderSym r tp vis) => String -> (r (Module r) -> r (Block r)) ->
...
--   dt is the date
--   fl is the file
docMod :: (OORenderSym r tp vis) => ModuleDocRenderer -> String -> String ->
  String -> [String] -> String -> SFile r -> SFile r
docMod mdr e wm d a dt fl = commentedMod fl (docComment $ mdr wm d a dt . addExt e
  <$> getModuleName)

fileFromData :: (OORenderSym r tp vis) => (FilePath -> r (Module r) ->
