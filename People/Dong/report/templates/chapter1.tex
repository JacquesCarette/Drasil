...
Before we conducting this research, the Drasil framework can generate software that provide numerical solutions for a first-order ODE by explicitly writing the equation. We re-write the ODE equation in a text based form, and pass it to the Drasil code generator. 

1. show the \href{https://jacquescarette.github.io/Drasil/examples/nopcm/SRS/srs/NoPCM_SRS.html#Sec:IMs}{NOPCM equation}, 
...
3. write it in explicit form
Code~\ref{code_expliciteq} shows the how to encode the Example~\ref{ex_firstorderode} by writing the explicit equation.
\begin{listing}[ht]
\begin{haskell1}
-- Pesodu Code
reciprocal tau_w * (T_c - T_w[0])
\end{haskell1}
\captionof{listing}{Explicit for the Example~\ref{ex_firstorderode}}
\label{code_expliciteq}
\end{listing}

4. ducplication problem

\begin{haskell1}
T_w'(t) = reciprocal tau_w * (T_c - T_w(t))
\end{haskell1}
completely text-base and not transformable, write the equation twice. Tell reader we will explain why the transformation is needed.
...
The $y_t$ is the dependent variable and it is Process Variable. The $y_t$(t) is a function of independent variable, often time. Depend on what time it is, the function $y_t$(t) will produce different values of $y_t$ at different time. The $y_t'$(t) is the first derivative of $y_t$(t). The $y_t''$(t) is the second derivative of $y_t$(t). The $K_d$ , $K_p$, and $r_t$ are constant variables. The $K_d$ is Derivative Gain, $K_p$ is Proportional Gain, and $r_t$ is Set-Point. We can write this equation as follows. 
