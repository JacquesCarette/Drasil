...
import GOOL.Drasil.Data (Boolean, Other, Binding(..), Terminator(..), 
  TypedType(..), td, FileType)
...
import Text.PrettyPrint.HughesPJ (Doc, text, empty, (<>), (<+>), parens,
  vcat, semi, equals)
...
varDec :: (RenderSym repr) => repr (Permanence repr) -> repr (Permanence repr) 
  -> repr (Variable repr a) -> repr (Statement repr)
varDec s d v = stateFromData (permDoc (bind $ variableBind v) <+> getTypeDoc 
  (variableType v) <+> variableDoc v) Semi
  where bind Static = s
        bind Dynamic = d

varDecDef :: (RenderSym repr) => repr (Variable repr a) -> repr (Value repr a) 
  -> repr (Statement repr)
varDecDef vr vl = stateFromData (statementDoc (S.varDec vr) <+> equals 
  <+> valueDoc vl) Semi

listDec :: (RenderSym repr) => (repr (Value repr Other) -> Doc) -> 
  repr (Value repr Other) -> repr (Variable repr a) -> repr (Statement repr)
listDec f sz v = stateFromData (statementDoc (S.varDec v) <> f sz) Semi

listDecDef :: (RenderSym repr) => ([repr (Value repr a)] -> Doc) -> 
  repr (Variable repr [a]) -> [repr (Value repr a)] -> repr (Statement repr)
listDecDef f v vs = stateFromData (statementDoc (S.varDec v) <> f vs) Semi

objDecNew :: (RenderSym repr) => repr (Variable repr Other) -> 
  [repr (Value repr Other)] -> repr (Statement repr)
objDecNew v vs = S.varDecDef v (newObj (variableType v) vs)

objDecNewNoParams :: (RenderSym repr) => repr (Variable repr Other) -> 
  repr (Statement repr)
...
ifCond :: (RenderSym repr) => repr (Keyword repr) -> repr (Keyword repr) -> 
  repr (Keyword repr) -> [(repr (Value repr Boolean), repr (Body repr))] -> 
...
for :: (RenderSym repr) => repr (Keyword repr) -> repr (Keyword repr) -> 
  repr (Statement repr) -> repr (Value repr Boolean) -> repr (Statement repr) 
  -> repr (Body repr) -> repr (Statement repr)
...
forEach :: (RenderSym repr) => repr (Keyword repr) -> repr (Keyword repr) -> 
  repr (Keyword repr) -> repr (Keyword repr) -> repr (Variable repr a) -> 
  repr (Value repr [a]) -> repr (Body repr) -> repr (Statement repr)
...
while :: (RenderSym repr) => repr (Keyword repr) -> repr (Keyword repr) -> 
  repr (Value repr Boolean) -> repr (Body repr) -> repr (Statement repr)
while bStart bEnd v b = stateFromData (vcat [
  text "while" <+> parens (valueDoc v) <+> keyDoc bStart,
  indent $ bodyDoc b,
  keyDoc bEnd]) Empty

construct :: Label -> TypedType Other
...
getMethod :: (RenderSym repr) => Label -> repr (Variable repr a) -> 
  repr (Method repr)
...
setMethod :: (RenderSym repr) => Label -> repr (Variable repr a) -> 
  repr (Method repr)
...
privMethod :: (RenderSym repr) => Label -> Label -> repr (Type repr a) -> 
  [repr (Parameter repr)] -> repr (Body repr) -> repr (Method repr)
privMethod n c = S.method n c private dynamic_

pubMethod :: (RenderSym repr) => Label -> Label -> repr (Type repr a) -> 
  [repr (Parameter repr)] -> repr (Body repr) -> repr (Method repr)
...
function :: (RenderSym repr) => Label -> repr (Scope repr) -> 
  repr (Permanence repr) -> repr (Type repr a) -> [repr (Parameter repr)] -> 
  repr (Body repr) -> repr (Method repr) 
function n s p t = S.intFunc False n s p (mType t)

mainFunction :: (RenderSym repr) => repr (Type repr Other) -> Label -> 
  repr (Body repr) -> repr (Method repr)
mainFunction s n = S.intFunc True n public static_ (mType void)
  [param (var "args" (updateType s List (++ "[]") (\d -> d <> text "[]")))]
...
docInOutFunc :: (RenderSym repr) => Label -> repr (Scope repr) -> 
  repr (Permanence repr) -> String -> [(String, repr (Variable repr Other))] -> 
  [(String, repr (Variable repr Other))] -> 
  [(String, repr (Variable repr Other))] -> repr (Body repr) -> 
  repr (Method repr)
...
intFunc :: (RenderSym repr) => Bool -> Label -> repr (Scope repr) -> 
  repr (Permanence repr) -> repr (MethodType repr a) -> [repr (Parameter repr)] 
  -> repr (Body repr) -> repr (Method repr)
...
privMVar :: (RenderSym repr) => Int -> repr (Variable repr a) -> 
  repr (StateVar repr)
privMVar del = S.stateVar del private dynamic_

pubMVar :: (RenderSym repr) => Int -> repr (Variable repr a) -> 
  repr (StateVar repr)
pubMVar del = S.stateVar del public dynamic_

pubGVar :: (RenderSym repr) => Int -> repr (Variable repr a) -> 
  repr (StateVar repr)
