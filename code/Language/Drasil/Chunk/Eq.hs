...
fromEqn nm desc _ symb un eqn atts refs = 
  EC (mkQuant nm desc symb Real (Just $ unitWrapper un) Nothing atts) eqn refs [] (shortname' "")

-- | Same as fromEqn, but has no units.
--FIXME: Space hack
fromEqn' :: String -> NP -> Sentence -> Symbol -> Expr -> Attributes -> References -> QDefinition
fromEqn' nm desc _ symb eqn atts refs = EC (mkQuant nm desc symb Real Nothing Nothing atts) eqn refs [] (shortname' "")
...
fromEqn'' nm desc _ symb abbr u eqn atts refs = 
  EC (mkQuant nm desc symb Real (fmap unitWrapper u) (Just abbr) atts) eqn refs [] (shortname' "")
...
ec c eqn = EC (qw c) eqn [] [] (shortname' "")
