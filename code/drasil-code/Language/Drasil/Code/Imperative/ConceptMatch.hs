...
chooseConcept chs = sequence $ Map.map (chooseConcept' chs $ conceptMatch chs) (conceptMatch chs)
  where 
        chooseConcept' :: Choices -> ConceptMatchMap-> [CodeConcept] -> State Doc CodeConcept
        chooseConcept' _ _ [] = error $ "Empty list of CodeConcepts in the " ++ 
          "ConceptMatchMap"
        chooseConcept' _ cmm cs = do 
            let uid = (head . Map.keys) cmm
            modify ($$ (text $ "Code Concept "++ uid ++" selected as " ++ show (head cs)))
            return $ head cs
        {-
        chooseConcept' :: Choices -> [CodeConcept] -> State Doc CodeConcept
        chooseConcept' _ [] = error $ "Empty list of CodeConcepts in the " ++ 
          "ConceptMatchMap"
        chooseConcept' _ cs = do 
            modify ($$ (text $ "Code Concept selected as" ++ show (head cs)))
            return $ head cs
        -}
