...
assumpS1 = foldlSent [
  S "The only form" `S.sOf` phrase energy, S "that" `S.sIs`
  S "relevant for this" +:+. (phrase problem `S.sIs` phrase CT.thermalEnergy),
  S "All other forms" `S.sOf` phrase energy `sC` S "such as",
  phrase mechEnergy `sC` S "are assumed to be negligible"]
assumpS2 = foldlSent [S "All", phrase CT.heatTrans, S "coefficients" `S.sAre`
                      S "constant over", phrase time]
assumpS3 = foldlSent [
  S "The", phrase water `S.sIn` S "the", phrase tank,
  S "is fully mixed, so the", phrase tempW `S.isThe` 
  S "same throughout the entire", phrase tank]
assumpS4 = foldlSent [
  S "The", phrase tempPCM `S.isThe` S "same throughout the", phrase pcmVol]
  --FIXME `sC` makeRefS likeChg1]
assumpS5 = foldlSent [
  S "The", phrase wDensity `S.sAnd` phrase pcmDensity,
...
assumpS8 = foldlSent [
  S "The", phrase tempC `S.sIs` S "constant over", phrase time]
assumpS9 = foldlSent [
...
assumpS11 = foldlSent [
  ...
  S "not" +:+. phrase discharging, S "The", phrase tempW `S.sAnd`
...
assumpS12 = foldlSent [phrase tempInit `S.the_ofThe'` phrase water `S.andThe`
...
assumpS14 mat = foldlSent [
  (S "operating" +:+ phrase temp +:+ S "range") `S.the_ofThe'` phrase system,
  S "is such that the", mat,
  S "is always in" +:+. (liquid ^. defn), S "That is" `sC`
  S "the", phrase temp, S "will not drop below the",
  phrase meltPt `S.sOf` phrase water `sC` S "or rise above its",
  phrase boilPt]
assumpS15 = foldlSent [
  S "The", phrase tank `S.sIs` phrase perfectInsul,
  S "so that there is no", phrase CT.heat, S "loss from the",
  phrase tank]
assumpS16 = foldlSent [
  S "No internal", phrase CT.heat, S "is generated by either the",
  phrase water `S.sOr` S "the", short phsChgMtrl :+:
  S "; therefore" `sC` S "the", phrase volHtGen, S "is zero"]
assumpS17 = foldlSent [
  (phrase vol +:+ phrase change) `S.the_ofThe'` short phsChgMtrl,
  S "due to", phrase CT.melting, S "is negligible"]
assumpS18 = foldlSent [
  S "The", short phsChgMtrl `S.sIs` S "either in a", liquid ^. defn,
  S "or a", solid ^. defn, S "but not a", gaseous ^. defn]
assumpS19 = foldlSent [
  S "The pressure in the", phrase tank, S "is atmospheric" `sC` S "so the",
  phrase meltPt `S.sAnd` phrase boilPt `S.sAre` S (show (0 :: Integer)) :+:
  Sy (unit_symb temp) `S.sAnd` S (show (100 :: Integer)) :+:
  Sy (unit_symb temp) `sC` S "respectively"]
assumpS20 = foldlSent [
  S "When considering the", phrase wVol `S.sIn` S "the",
  phrase tank `sC` (phrase vol `S.the_ofThe` phrase coil),
