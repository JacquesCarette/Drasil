...
import qualified Data.Drasil.Quantities.Physics as QP (accelerationUC,
  angularAccel, forceUC, gravitationalAccelUC, momentOfInertiaUC, angularVelocity, 
  timeUC, impulseSUC)
...
im1_new = im' transMot [qw vel_i, qw QP.timeUC, qw QP.gravitationalAccelUC, qw force_i, qw mass_i] 
  [sy vel_i $> 0, sy QP.timeUC $> 0, sy QP.gravitationalAccelUC $> 0, 
...
transMotRel = (sy acc_i) $= (deriv (apply1 vel_i QP.timeUC) QP.timeUC)
  $= (sy QP.gravitationalAccelUC) + ((apply1 force_i QP.timeUC) / (sy mass_i))


--fixme: need referencing
transMotDesc, transMotLeg :: Sentence
transMotDesc = foldlSent [S "The above equation expresses the total",
  (phrase QP.accelerationUC), S "of the", (phrase CP.rigidBody),
  makeRefS newA1, makeRefS newA2, S "i as the sum of",
  (phrase QP.gravitationalAccelUC),
  S "(GD3) and", (phrase QP.accelerationUC), S "due to applied",
  (phrase QP.forceUC), S "Fi(t) (T1). The resultant outputs are",
...
im2_new = im' rotMot [qw QP.angularVelocity, qw QP.timeUC, qw torque_i, qw QP.momentOfInertiaUC]
  [sy QP.angularVelocity $> 0, sy QP.timeUC $> 0, sy torque_i $> 0, sy QP.momentOfInertiaUC $> 0] 
...
rotMotRel = (sy QP.angularAccel) $= deriv
  (apply1 QP.angularVelocity QP.timeUC) QP.timeUC $= 
     ((apply1 torque_i QP.timeUC) / (sy QP.momentOfInertiaUC))
...
im3_new = im' col2D [qw QP.timeUC, qw QP.impulseSUC, qw mass_A, qw normalVect] 
  [sy QP.timeUC $> 0, sy QP.impulseSUC $> 0, sy mass_A $> 0, sy normalVect $> 0]
...
col2DRel = (apply1 vel_A time_c) $= (apply1 vel_A QP.timeUC) +
  ((sy QP.impulseSUC) / (sy mass_A)) * (sy normalVect)

--im3Rel2 = (apply1 vel_B time_c) $= (apply1 vel_B QP.timeUC) -
--  ((sy QP.impulseSUC) / (sy mass_B)) * (sy normalVect)


--fixme: these two need to use cross product and parametrized dispUnit symbol
--im3Rel3 = (apply1 angVel_A time_c) $= (apply1 angVel_A QP.timeUC) +
--  ((sy dispUnit) * ((sy QP.impulseSUC) * (sy normalVect))) / (sy QP.momentOfInertia)

--im3Rel4 = (apply1 angVel_B time_c) $= (apply1 angVel_B QP.timeUC) -
--  ((sy dispUnit) * ((sy QP.impulseSUC) * (sy normalVect))) / (sy QP.momentOfInertia)
