...
import Data.Drasil.Utils (enumSimple, getES, mappingShortNames,
...
fig_tank = fig (at_start sWHT `sC` S "with" +:+ phrase ht_flux +:+
  ...
  "TankWaterOnly.png" "Tank" (shortname' "Tank")
...
assump9_npcm = let a9 = "assump9_npcm" in mkAssump a9 assumpS9_npcm
...
s5_1_list_items = [
  ...
  unit'2Contents,
  phrase] inputVar)
  (titleize input_ +:+ titleize variable +:+ titleize requirement) False "fr1list" (shortname' "fr1list"),
...
req6 = mkRequirement' "req6" (
  ...
  "Calculate-Change-Heat_Energy-Water-Time"
...
likeChg3_npcm = mkLklyChnk "likeChg3" (
  ...
  +:+ phrase model +:+. S "would also account for discharging of the tank") "Discharging-Tank" (shortname' "Discharging-Tank")
...
s7_instaModelRef = map (mappingShortNames Theory) [eBalanceOnWtr,
  heatEInWtr]
...
s7_theories = ["T1"]
s7_theoriesRef = map (mappingShortNames Theory) [t1ConsThermE]

s7_genDefs = ["GD1", "GD2"]
s7_genDefRef = map (mappingShortNames Theory) swhsGenDefs

s7_dataDefs = ["DD1"]
s7_dataDefRef = map (mappingShortNames Data) [dd1HtFluxC]
...
s7_table3 = Table (EmptyS:s7_row_header_t3)
  ...
  titleize' item)) True "TraceyAI" (shortname' "TraceyAI")
