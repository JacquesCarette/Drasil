...
-- | Creates the HelloWorld program and necessary files.
helloWorldProc :: (ProcProg r stmt mthd prg) => GProc.GSProgram r prg
...
myOtherList :: (SharedProg r stmt mthd) => SVariable r
...
-- | Main function. Initializes variables and combines all the helper functions defined below.
helloWorldMainProc :: (ProcProg r stmt mthd prg) => MS (r mthd)
...
helloInitVariables :: (SharedProg r stmt mthd) => MS (r Block)
...
mySlicedList, mySlicedList2, mySlicedList3, mySlicedList4, mySlicedList5,
  ...
  mySlicedList10, mySlicedList11 :: (SharedProg r stmt mthd) => SVariable r
...
listSliceTests :: (SharedProg r stmt mthd) => [MS (r Block)]
listSliceTests = [
...
helloIfBody :: (SharedProg r stmt mthd) => MS (r Body)
...
helloElseBody :: (SharedProg r stmt mthd) => MS (r Body)
helloElseBody = bodyStatements [printLn (arg 5)]

-- | If-else statement checking if a list is empty.
helloIfExists :: (SharedProg r stmt mthd) => MS (r stmt)
...
helloSwitch :: (SharedProg r stmt mthd) => MS (r stmt)
...
helloTryCatch :: (SharedProg r stmt mthd) => MS (r stmt)
