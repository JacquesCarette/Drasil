...
s2_2_intro = Paragraph $ S "The scope of the" +:+
  ...
  S "acted on by forces. Given" +:+ (twoD ^. term) +:+ (rigidBodies ^. term) :+:
  S "," +:+ (chipmunk ^. term) +:+ S "is intended to simulate how these" +:+
...
s2_3_intro = [Paragraph (S "The organization of this document follows the" +:+
  ...
  (sLower (thModel ^. defn)) :+: S "s to the" +:+ (sLower (inModel ^. defn)) :+:
  S "s.")]
...
s4_intro = Paragraph $ S "This section first presents the problem" +:+
  ...
  S "s, theories, and definitions that are used for the" +:+ 
  (physLib ^. term) :+: S"."
...
s4_1_intro = Paragraph $ S "Creating a gaming" +:+ (physLib ^. term) +:+
  ...
  (twoD ^. term) +:+ (rigidBody ^. term) +:+ (physLib ^. term) :+: 
