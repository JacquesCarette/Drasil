...
theories\footnote{There's also a 4th kind: \ACFP{dd}, intended for explaining
how input variables should be interpreted by the solution program, thus also
explaining how they should be formatted; they are typically intended to be
implementation-focused, rather than theoretical.}: \ACFP{tm}, \ACFP{gd}, and
\ACFP{im}. \acsp{tm} and \acsp{gd} provide justification for the mathematics of
the grounded theories relevant to the software solution: the \acsp{im}. The
\acsp{im}, specifically, together form the calculation path.
