...
straightforward to use the generated traceability information to identify the
exact chunk from which the formula was produced. That made it clear where the
conversion logic belonged: instead of leaving the division by 1000 implicit in
the rendered equation, we made the conversion explicit in the source chunk
itself. After regeneration, the corrected expression appeared consistently in
the SRS and in the source code computations derived from that chunk,
demonstrating more explicitly how traceability supports single-point
maintenance.

\subsection{Reproducibility and Determinism}
Artifact generation in Drasil is fully deterministic: repeated generation runs,
from the same knowledge base and recipe set, produce byte-for-byte equivalent
...
\ds{Add table with shared base knowledge and generator infrastructure that does 
not need to be edited (unless adding a new chunk type). Use footnotes to denote 
``we did the hard part here'' and ``this might need to be extended''}
