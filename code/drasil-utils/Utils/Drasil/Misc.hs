...
displayStrConstrntsAsSet sym listOfVals = eS r
  where
    r :: ModelExpr 
    r = isIn (sy sym) (DiscreteS listOfVals)

-- | Produces a 'Sentence' that displays the constraints in a set {}.
displayDblConstrntsAsSet :: Quantity a => a -> [Double] -> Sentence
displayDblConstrntsAsSet sym listOfVals = eS r
  where
    r :: ModelExpr
    r = isIn (sy sym) (DiscreteD listOfVals)
