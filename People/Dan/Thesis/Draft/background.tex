...
\ds{ Add a blurb here about spinning up a kernel through parameterized options 
and how that can create a fully customized family member.}
...
Being able to reproduce computational results is fundamental to good science.
...
reproducibility crisis highlighted in the scientific literature~\cite{Baker2016}.
...
scripts, notebooks, and build infrastructure~\cite{IonescuAndJansson2013}.
...
literate workflows in which executable code is presented alongside explanatory 
text.
...
rationale needed for long-term regeneration and reuse~\cite{Canon2020,
AkhlaghiEtAl2021}.

Drasil addresses a different point in this design space. Rather than focusing
on a single linear document or a frozen execution environment, Drasil codifies
domain knowledge, assumptions, design choices, and artifact structure in a
single knowledge base. From that source it generates multiple, interrelated
artifacts, including documentation, code, and supporting materials. This
approach strengthens reproducibility because a software system can be
regenerated from an explicit declarative source, and it supports replicability
by making the underlying knowledge and decisions visible instead of tacit.
...
\ds{Not using LLMs or AI models for generation}

\ds{cover meta-programming here as well -> broad strokes. We rely on GOOL so in 
many ways it doesn't matter how much I understand about code gen. According to 
Jacques -> can pass the buck. }

\ds{Add section for model driven engineering explaining that we did not look 
into it to avoid biasing our approach (practical example-driven requirements 
for our model)}
