...
\item[A20:]When considering the volume of water in the tank, the volume of the heating coil is assumed to be negligible [R2].
\end{itemize}
...
\\
$D$ & $D>0$ & $\frac{D}{L_{max}}\leq{}\frac{D}{L}\leq{}\frac{D}{L_{min}}$ & 0.412 m & 10$\%$
\\
$V_{P}$ & $V_{P}>0$ and $V_{P}<V_{tank}$ & $V_{P}\geq{}V_{tank}$ & 5.0e-2 $\text{m}^{3}$ & 10$\%$
\\
$A_{P}$ & $A_{P}>0$ & $V_{P}\leq{}A_{P}\leq{}\frac{2}{h_{min}}V_{tank}$ & 1.2 $\text{m}^{2}$ & 10$\%$
\\
$\rho{}_{P}$ & $\rho{}_{P}>0$ and $\rho{}_{P}^{min}<\rho{}_{P}<\rho{}_{P}^{max}$ &  & 1007 $\frac{\text{kg}}{\text{m}^{3}}$ & 10$\%$
\\
$T_{melt}^{P}$ & $0<T_{melt}^{P}<T_{C}$ &  & 44.2 ${}^{\circ}C$ & 10$\%$
\\
$C_{P}^{S}$ & $C_{P}^{S}>0$ & $C_{P}^{S}_{min}<C_{P}^{S}<C_{P}^{S}_{max}$ & 1760 $\frac{\text{J}}{(\text{kg}{}^{\circ}C)}$ & 10$\%$
\\
$C_{P}^{L}$ & $C_{P}^{L}>0$ & $C_{P}^{L}_{min}<C_{P}^{L}<C_{P}^{L}_{max}$ & 2270 $\frac{\text{J}}{(\text{kg}{}^{\circ}C)}$ & 10$\%$
\\
$H_{f}$ & $H_{f}>0$ & $H_{f}_{min}<H_{f}<H_{f}_{max}$ & 211600 $\frac{\text{J}}{\text{kg}}$ & 10$\%$
\\
$A_{C}$ & $A_{C}>0$ & $A_{C}\leq{}A_{C}^{max}$ & 0.12 $\text{m}^{2}$ & 10$\%$
\\
$T_{C}$ & $0<T_{C}<100$ &  & 50 ${}^{\circ}C$ & 10$\%$
\\
$\rho{}_{W}$ & $\rho{}_{W}>0$ & $\rho{}_{W}^{min}<\rho{}_{W}\leq{}\rho{}_{W}^{max}$ & 1000 $\frac{\text{kg}}{\text{m}^{3}}$ & 10$\%$
\\
$C_{W}$ & $C_{W}>0$ & $C_{W}^{min}<C_{W}<C_{W}^{max}$ & 4186 $\frac{\text{J}}{(\text{kg}{}^{\circ}C)}$ & 10$\%$
\\
$h_{C}$ & $h_{C}>0$ & $h_{C}^{min}\leq{}h_{C}\leq{}h_{C}^{max}$ & 1000 $\frac{\text{W}}{(\text{m}^{2}{}^{\circ}C)}$ & 10$\%$
\\
$h_{P}$ & $h_{P}>0$ & $h_{P}^{min}\leq{}h_{P}\leq{}h_{P}^{max}$ & 1000 $\frac{\text{W}}{(\text{m}^{2}{}^{\circ}C)}$ & 10$\%$
\\
$T_{init}$ & $0<T_{init}<T_{melt}$ &  & 40 ${}^{\circ}C$ & 10$\%$
\\
$t_{final}$ & $t_{final}>0$ & $t_{final}<t_{final}^{max}$ & 50000 s & 10$\%$
\\
\bottomrule
...
\\
\midrule
$T_{W}$ & $T_{init}\leq{}T_{W}\leq{}T_{C}$ (by A11)
\\
$T_{P}$ & $T_{init}\leq{}T_{P}\leq{}T_{C}$ (by A11)
\\
$E_{W}$ & $E_{W}\geq{}0$
\\
$E_{P}$ & $E_{P}\geq{}0$
...
\toprule
 & IM1 (\hyperref[T:eBalanceOnWtr]{Definition~T:eBalanceOnWtr}) & IM2 (\hyperref[T:eBalanceOnPCM]{Definition~T:eBalanceOnPCM}) & IM3 (\hyperref[T:heatEInWtr]{Definition~T:heatEInWtr}) & IM4 (\hyperref[T:heatEInPCM]{Definition~T:heatEInPCM}) & Data Constraints (Table~\ref{Table:InpuVari}) & R1 (Section~\ref{Sec:FuncRequ}) & R2 (Section~\ref{Sec:FuncRequ}) & R3 (Section~\ref{Sec:FuncRequ}) & R4 (Section~\ref{Sec:FuncRequ}) & R5 (Section~\ref{Sec:FuncRequ}) & R6 (Section~\ref{Sec:FuncRequ}) & R7 (Section~\ref{Sec:FuncRequ}) & R8 (Section~\ref{Sec:FuncRequ}) & R9 (Section~\ref{Sec:FuncRequ}) & R10 (Section~\ref{Sec:FuncRequ}) & R11 (Section~\ref{Sec:FuncRequ})
\\
\midrule
