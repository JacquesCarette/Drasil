...
natural language. Instead of working with the set of natural language, the
ideology focuses on formalizing the meaning of specific subsets into principled
stores. The principled stories are then intertwined and mixed until a coherent
``whole story'' is formed, where we deterministically understand what can and
cannot be done with the knowledge described (including, but not limited to,
generating representational software artifacts and snippets).
...
into their knowledge management system. They should also hve a working
understanding of what the end-user needs. From this, they should be able to
connect the work of the domain expert into ``plug-n-play'' stories (arguably,
compilers for the end-users to use), or be able to encode/reduce friction
between knowledge encoded by domain experts.
...
characters, or we might have a slightly deeper depth/understanding  of sentences
by describing them as a language that follows a specific syntax rule set and
using a specific set of words. Breadth of knowledge is the horizontal domain of
knowledge, it is the various kinds of knowledge we have in a wide variety of
subjects and domains.

\begin{itemize}

      \item Already proven for low depth -- see WordPress \cite{Wordpress} and
            other \ACFP{cms}. WordPress contains very low depth but has a
            sufficient breadth for websites such that many plugins can be used
            to add extra depth and breadth as required to increase the usability
            of the WordPress software for different needs, thus creating a
            plethora of websites specialized to different requirements (such as
            blogs, e-commerce websites, and online discussion forums).
