...
diam         = cuc' "diam" (nounPhraseSP "diameter of tank")
  ...
  (c :/ C tank_length) :<= (c :/ C tank_length_min)] (Dbl 0.412)
...
pcm_SA       = cuc' "pcm_SA"
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> C pcm_vol :<= c :<= ((Int 2 :/ C htTransCoeff_min) :* C tank_vol)] (Dbl 1.2)
...
pcm_density  = cuc' "pcm_density" (nounPhraseSP "density of PCM")
  ...
  [physc $ \c -> c :> Int 0,
  physc $ \c -> C pcm_density_min :< c :< C pcm_density_max] (Dbl 1007)
...
temp_melt_P  = cuc' "temp_melt_P" (nounPhraseSP "melting point temperature for PCM")
  ...
  [physc $ \c -> Int 0 :< c :< C temp_C] (Dbl 44.2)
...
htCap_S_P    = cuc' "htCap_S_P" (nounPhraseSP "specific heat capacity of PCM as a solid")
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> C htCap_S_P_min :< c :< C htCap_S_P_max] (Dbl 1760)
...
htCap_L_P    = cuc' "htCap_L_P" (nounPhraseSP "specific heat capacity of PCM as a liquid")
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> C htCap_L_P_min :< c :< C htCap_L_P_max] (Dbl 2270)
...
htFusion     = cuc' "htFusion" (nounPhraseSP "specific latent heat of fusion")
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> C htFusion_min :< c :< C htFusion_max] (Dbl 211600)
...
coil_SA      = cuc' "coil_SA"
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> c :<= C coil_SA_max] (Dbl 0.12)
...
htCap_W      = cuc' "htCap_W" (heat_cap_spec `of_` water)
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> C htCap_W_min :< c :< C htCap_W_max] (Dbl 4186)
...
coil_HTC     = cuc' "coil_HTC" (nounPhraseSP
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> C coil_HTC_min :<= c :<= C coil_HTC_max] (Dbl 1000)
...
pcm_HTC      = cuc' "pcm_HTC"
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> C pcm_HTC_min :<= c :<= C pcm_HTC_max] (Dbl 1000)
...
time_final   = cuc' "time_final" (nounPhraseSP "final time")
  ...
  [physc $ \c -> c :> Int 0,
  sfwrc $ \c -> c :< C time_final_max] (Dbl 50000)
  
  
-- Output Constraints
swhsOutputs :: [ConstrConcept] --FIXME: Add typical values or use Nothing if not known
