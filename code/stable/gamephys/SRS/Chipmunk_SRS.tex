...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & The linear velocity ${\mathbf{v}_{B}}$ ($\frac{\text{m}}{\text{s}}$) of any point B in a rigid body A\ref{A:objectTy} is the sum of the linear velocity ${\mathbf{v}_{O}}$ ($\frac{\text{m}}{\text{s}}$) of the rigid body at the origin (axis of rotation) and the resultant vector from the cross product of the rigid body's angular velocity $ω$ ($\frac{\text{rad}}{\text{s}}$) and the displacement vector between the origin and point B, ${\mathbf{r}_{OB}}$ (m).
\\ \midrule \\
Source & 
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & The net torque $τ$ (Nm) on a rigid body is proportional to its angular acceleration $α$ ($\frac{\text{rad}}{\text{s}^{2}}$). Here, $\mathbf{I}$ (kg$\text{m}^{2}$) denotes the moment of inertia of the rigid body. We also assume that all rigid bodies involved are two-dimensional A\ref{A:objectDimension}.
\\ \midrule \\
Source & 
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & A\ref{A:objectTy}
        A\ref{A:objectDimension}
\\ \midrule \\
Source & 
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & The above equation expresses the total acceleration of the rigid body A\ref{A:objectTy} A\ref{A:objectDimension} i as the sum of gravitational acceleration (GD3) and acceleration due to applied force Fi(t) (T1). The resultant outputs are then obtained from this equation using \hyperref[DD:linDisp]{Definition~DD:linDisp} \hyperref[DD:linVel]{Definition~DD:linVel} \hyperref[DD:linAcc]{Definition~DD:linAcc}.  It is currently assumed that there is no damping A\ref{A:dampingInvolvement} or constraints A\ref{A:constraintsAndJointsInvolvement} involved. \hyperref[DD:ctrOfMass]{Definition~DD:ctrOfMass}.
\\ \midrule \\
Source & 
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & The above equation for the total angular acceleration of the rigid body A\ref{A:objectTy} A\ref{A:objectDimension} i is derived from T5, and the resultant outputs are then obtained from this equation using \hyperref[DD:angDisp]{Definition~DD:angDisp} \hyperref[DD:angVel]{Definition~DD:angVel} \hyperref[DD:angAccel]{Definition~DD:angAccel}. It is currently assumed that there is no damping A\ref{A:dampingInvolvement} or constraints A\ref{A:constraintsAndJointsInvolvement} involved. A\ref{A:axesDefined}
\\ \midrule \\
Source & 
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & This instance model is based on our assumptions regarding rigid body A\ref{A:objectTy} A\ref{A:objectDimension} collisions A\ref{A:collisionType} Again, this does not take damping A\ref{A:dampingInvolvement} or constraints A\ref{A:constraintsAndJointsInvolvement} into account. A\ref{A:axesDefined}. \hyperref[DD:ctrOfMass]{Definition~DD:ctrOfMass} \hyperref[DD:impulse]{Definition~DD:impulse}
\\ \midrule \\
Source & 
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
\item[Verify-Physical\_Constraints:\phantomsection\label{reqVPC}]Verify that the inputs satisfy the required physical constraints from Section~\ref{Sec:SolCharSpec}.
...
\item[Expanded-Collisions:\phantomsection\label{lcEC}]The library may be expanded to deal with edge-to-edge and vertex-to-vertex collisions. A\ref{A:collisionType}
\item[Include-Dampening:\phantomsection\label{lcID}]The library may be expanded to include motion with damping. A\ref{A:dampingInvolvement}
\item[Include-Joints-Constraints:\phantomsection\label{lcIJC}]The library may be expanded to include joints and constraints. A\ref{A:constraintsAndJointsInvolvement}
\end{itemize}
...
\toprule
 & IM1 (\hyperref[IM:transMot]{Definition~IM:transMot}) & IM2 (\hyperref[IM:rotMot]{Definition~IM:rotMot}) & IM3 (\hyperref[IM:col2D]{Definition~IM:col2D}) & R1 (\hyperref[reqSS]{FR: Simulation-Space}) & R4 (\hyperref[reqIIC]{FR: Input-Initial-Conditions}) & R7 (\hyperref[reqISP]{FR: Input-Surface-Properties}) & Data Constraints (Section~\ref{Sec:SolCharSpec})
\\
\midrule
...
\\
LC1 (\hyperref[lcVODES]{LC: Variable-ODE-Solver}) &  &  &  &  &  &  & 
\\
LC2 (\hyperref[lcEC]{LC: Expanded-Collisions}) &  &  &  &  & X &  & 
\\
LC3 (\hyperref[lcID]{LC: Include-Dampening}) &  &  &  &  &  & X & 
\\
LC4 (\hyperref[lcIJC]{LC: Include-Joints-Constraints}) &  &  &  &  &  &  & X
\\
\bottomrule
