...
buildPath file = filter (/=' ') $ unwords $ replicate (numSlash file) "../"
  where
    numSlash [] = 4
    numSlash fp = length $ filter (=='/') fp
...
alignImage img = text "<p align=\"center\">" <>
  ...
  <> contSep <> text "</p>"
-- | End section.
endNote :: String -> [String] -> Doc
endNote file auth = text "*This README is a software artifact generated by Drasil.*" <> doubleSep <> license (listToDoc auth) <> doubleSep <> drasilImage file
