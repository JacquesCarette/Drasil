...
(\refOriginalExprHaskell{}), which captures general mathematical
...
in the language of \Expr{}. \refPseudoExampleExpression{} is a great example,
...
in \Cref{lst:pseudoExampleExpressionAsQDefinition} using a trigonometric
integral (e.g., $y = \int \frac{3}{\cos^2(x) dx}$,
\Cref{lst:pseudoExampleExpressionAsQDefinitionBad}), but then transformation
...
Now, in order to restrict terms to their appropriate contexts, we need to
...
language as a base for the other two (\refLanguageDivision{}).
...
expression in different languages, we use a \ACF{ttf}~\cite{Carette2009}
