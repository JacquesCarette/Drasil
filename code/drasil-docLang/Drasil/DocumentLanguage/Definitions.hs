...
mkQField d _ l@Units fs = (show l, (Paragraph $ (unitToSentence d)):[]) : fs
...
mkDDField d _ l@Units fs = (show l, (Paragraph $ (unitToSentence d)):[]) : fs
...
firstPair (IncludeUnits) d = (P (eqSymb d), Flat (phrase d +:+ sParen (unitToSentence d)))
...
descPairs IncludeUnits =
  map (\x -> ((P (eqSymb x)), Flat $ phrase x +:+ sParen (unitToSentence x)))
