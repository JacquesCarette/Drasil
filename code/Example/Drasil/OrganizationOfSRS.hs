...
figureLabel num traceyMG contents filePath = Figure (fterm titleize figure +: 
  S num +:+ (showingCxnBw (traceyMG) (contents))) filePath

showingCxnBw :: NamedIdea c => c -> Sentence -> Sentence
showingCxnBw traceyVar contents = fterm titleize traceyVar +:+ S "Showing the" +:+
  fterm titleize' connection +:+ S "Between" +:+ contents
...
specSysDesIntro l_end = Paragraph $ foldlSent
            [S "This", fterm phrase section_, S "first presents the",
            fterm phrase problemDescription `sC` S "which gives a high-level view of the",
            fterm phrase problem, S "to be solved. This is followed by the",
            fterm plural solutionCharSpec `sC` S "which presents the",
            fterm plural assumption `sC` fterm plural theory `sC` l_end]
...
probDescF start progName ending subSec = SRS.probDesc [Paragraph intro] subSec
  where intro = foldlSent [start, (short progName), S "is a computer", 
                (fterm phrase program), S "developed to", ending]
...
termDefnF end otherContents = SRS.termAndDefn ((intro):otherContents) []
      where lastF Nothing  = EmptyS
            ...
            intro = Paragraph $ foldle (+:+) (+:) (EmptyS)
                    ...
                    S "that are used in the subsequent", fterm plural section_, S "and their",
                    S "meaning, with the", fterm phrase purpose, S "of reducing ambiguity",
                    S "and making it easier to correctly understand the",
                    fterm plural requirement, lastF end]
...
physSystDesc kWord fig otherContents = SRS.physSyst ((intro):otherContents) []
  where intro = Paragraph $ foldle (+:+) (+:) (EmptyS)
                [S "The", (fterm phrase physicalSystem), S "of", kWord `sC`
                S "as shown in", (makeRef fig) `sC` S "includes the following", 
                fterm plural element]
...
solChSpecF kWord (probDes, likeChg) gendef ddEndSent (tbRef, mid, end, trail) (a,t,g,dd,i,dc) adSubSec = SRS.solCharSpec [Paragraph intro] (subSec gendef)
  where intro = foldlSent
                [S "The", fterm plural inModel, S "that govern",
                short kWord, S "are presented in" +:+. makeRef (instModels gendef),
                S "The", fterm phrase information, S "to understand",
                (S "meaning" `ofThe` fterm plural inModel),
                S "and their derivation is also presented, so that the",
                fterm plural inModel, S "can be verified"]
...
assumpIntro r1 r2 r3 r4 r5 = Paragraph $ foldlSent 
          [S "This", (fterm phrase section_), S "simplifies the original", (fterm phrase problem),
          S "and helps in developing the", (fterm phrase thModel), S "by filling in the",
          S "missing", (fterm phrase information), S "for the" +:+. (fterm phrase physicalSystem),
          ...
          (fterm phrase assumption), S "is used"] --FIXME: use some clever "zipWith"
...
thModIntro k_word = Paragraph $ foldlSent
          [S "This", fterm phrase section_, S "focuses on",
          S "the", fterm phrase general, (fterm plural equation), S "and",
...
genDefnF otherContents = SRS.genDefn (genDefnIntro:otherContents) []
  where genDefnIntro = Paragraph $ foldlSent [S "This", fterm phrase section_, 
                       S "collects the", S "laws and", (fterm plural equation), 
                       S "that will be used in", S "deriving the", 
                       fterm plural dataDefn `sC` S "which in turn are used to",
                       S "build the", fterm plural inModel]
...
dataDefnF endingSent otherContents = SRS.dataDefn ((dataDefnIntro endingSent):otherContents) []
  where dataDefnIntro ending = Paragraph $ 
          foldlSent [S "This", fterm phrase section_, 
            S "collects and defines all the", fterm plural datum,
            S "needed to build the", fterm plural inModel] +:+ ending
...
inModelIntro r1 r2 r3 r4 = Paragraph $ foldlSent [S "This", fterm phrase section_,
          S "transforms the", fterm phrase problem, S "defined in", (makeRef r1),
          S "into one which is expressed in mathematical terms. It uses concrete",
          fterm plural symbol_, S "defined in", (makeRef r2),
          S "to replace the abstract", fterm plural symbol_, S "in the",
          fterm plural model, S "identified in", (makeRef r3) :+: end r4]
...
datConPar tableRef middleSent endingSent trailingSent = Paragraph $ foldlSent [
          tableRef, S "the", fterm plural datumConstraint, S "on the", 
          fterm phrase input_, S "and", fterm phrase output_, 
          fterm plural variable `sC` S "respectively.", S "The", 
          fterm phrase column, S "for", fterm phrase physical,
          fterm plural constraint, S "gives the", fterm phrase physical,
          fterm plural limitation, S "on the range of", fterm plural value,
          S "that can be taken by the" +:+. fterm phrase variable, middleSent, -- << if you are wondering where middleSent is
          S "The", fterm plural constraint, S "are conservative, to give",
          (fterm phrase user `ofThe` fterm phrase model), S "the flexibility to", 
          S "experiment with unusual situations. The", fterm phrase column, S "of", 
          S "typical", fterm plural value, S "is intended to provide a feel for a common scenario"]
          +:+ endS endingSent +:+ trailingSent
          where endS False = EmptyS
                endS True  = S "The" +:+ fterm phrase uncertainty +:+ fterm phrase column +:+ S "provides an" +:+
                             S "estimate of the confidence with which the" +:+ fterm phrase physical +:+
                             fterm plural quantity +:+. S "can be measured" +:+ S "This" +:+
                             fterm phrase information +:+ S "would be part of the" +:+ fterm phrase input_ +:+
                             S "if one were performing an" +:+ fterm phrase uncertainty +:+.
