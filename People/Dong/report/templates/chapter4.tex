...
In chapter 2, we stored the information of a higher-order linear ODE in the \verb|DifferentialModel|. This data type preserves the relationship of the ODE, and we can transform the ODE into other forms. In Chapter 3, we discuss how to solve a system of first-order ODEs numerically in four selected external libraries. We have not discussed how to close the gap between the \verb|DifferentialModel| and external libraries so the Drasil framework can generate a program to solve a high-order ODE numerically. All external libraries do not directly utilize \verb|DifferentialModel|, so this is the problem we will solve in this chapter. We know that most ways of solving ODEs are intended for first-order ODEs, so we want to covert most higher-order ODEs to a system of first-order ODEs~\citep{converthigherode}. Therefore, to solve the ODE, we first transform a higher-order linear ODE into a system of first-order ODEs. Then, using the external libraries to solve the system of first-order ODEs. The original higher-order linear ODE is equivalent to the system of first-order ODEs. Thus, by transitivity, the numerical solution for the system of first-order ODEs is also the numerical solution of the higher-order ODE.

In this chapter, we will first discuss how to convert a higher-order linear ODE to a system of first-order equations in theory. Then, we will discuss about how to connect explicit equations with external libraries. Last, we will discuss how to generate explicit equations based on \verb|DifferentialModel|.

\section{Higher Order to First Order}
\label{se_hightofirst}
Given a higher-order ODE, we can write it as Equation~\ref{eq_isohighode}. We put the highest derivative $y^n$ on the left-hand side and the rest of terms on the right- hand side. On the right hand side, $f (t, y, y', y'', \dots, y^{n-1})$ means a function depends on variables t, y, y', \dots, and $y^{n-1}$. The t is the independent variable, and it is time. The y, y', \dots, and $y^{n-1}$ means the dependent variable y, the first derivative of y, and until the (n-1)th derivative.
\begin{equation} \label{eq_isohighode}
  y^n = f (t, y, y', y'', \dots, y^{n-1})
\end{equation}

Later, we introduce new variables, $x_{1}$, $x_{2}$, $\dots$, and $x_{n}$. The number of the newly introduced dependent variable is equal to the highest order of the ODE. The new relationship is shown below.
...
Now, we can start differentiate $x_{1}$, $x_{2}$, $\dots$, and $x_{n}$ in Equation~\ref{eq_newvars}. Then, we get new relationships between each variable.
...
Based on Equation~\ref{eq_diffvervars}, we can simplify equations by removing derivatives of x and replacing $f (t, y_{1}, y_{2}, \dots, y_{n})$ with Equation~\ref{eq_linear}. Then, we can get:
...
In previous research conducted by Brooks, we wrote the ODEs in a text-based form and stored them in a general data pool. However, the Drasil printer can print ODEs in the SRS, and the Drasil Code Generator cannot understand the ODEs in a text-based form. We manually create an object (\verb|ODEInfo|) to make study cases work based on the original ODE. Then, the Drasil Code Generator takes the new object and generates a solution program. We can find details on how to generate external libraries in code in Brooks's thesis~\citep{brooks}. However, the previous research only completes generating external libraries for a first-order ODE. For a higher-order ODE, even if we convert it to a system of first-order ODE in \verb|ODEInfo|, the Code Generator cannot generate proper software. The Code Generator unable to retrieve proper initial values when there is more than one initial value. Therefore, we change the type of the initial value to a new one that can hold for multiple values. 
...
The \verb|ODEInfo| is a \verb|datatype| storing the initial value, and we can change the type to be a list. This will enable to store multiple initial values. Then, we have to ensure the Code Generator know how to handle the type of initial values. 
...
In Drasil Code Generator, we use \verb|matrix| to represent a list. A list is a special case of \verb|matrix| whose dimension is 1. The \vern|info| is a \verb|datatype|, \verb|ODEInfo|, and the code \verb|initVal info| retrieves the initVal from \verb|ODEInfo|. In the new code, the type of initVal is no longer \verb|CodeExpr| but a list of \verb|CodeExpr|.
...
If it is a fourth-order ODE, the \textbf{A} will be an n$\times$n matrix, and the dimension of the identity matrix will be n-1$\times$n. Equation~\ref{eq_foodeexamplecolorfour} shows a completed transformation for a fourth-order ODE.
...
In Chapter 3, we mentioned we would solve the ODE as an IVP. In Code~\ref{code_ivpinfo}, we create \verb|InitialValueProblem| to store IVP-related information.
\begin{listing}[ht]
\begin{haskell1}
