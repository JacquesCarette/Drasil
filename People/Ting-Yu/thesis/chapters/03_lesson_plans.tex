...
defined in line 12-31. At present, \macblue{Contents} is the only defined 
...
For example, the following equation is the position of a particle moving in 
...
\textbf{drasil-data} with units as a \macblue{UnitalChunk} 
...
the right side expression (\textbf{scalarPos'}) is a \macblue{PExpr} declared 
...
\macblue{LabelledContent} because we are adding a label to it, allowing us to 
...
\subsection{Reproduce the Lesson}
...
chapter, we convert them to the \macblue{Contents} type and map them together. 
We provide smart constructors like \macred{lbldExpr} \footnote{This converts a 
\textbf{ModelExpr} into a \textbf{Contents}.} for transfering different kind of 
contents to \macblue{Contents}. In Code~\ref{code:reviewContent}, we 
...
A lesson plan is represented in the \macblue{LsnDecl} type, which is a 
collection of chapters (see Code~\ref{code:mkNB}). We then use the 
\macred{mkNb} function (presented in Code~\ref{code:mkNb}) to convert the 
