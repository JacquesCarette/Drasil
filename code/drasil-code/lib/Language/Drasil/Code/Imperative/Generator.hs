...
data SomeProgGenerator where
  SomeProgGenerator
    :: forall repr tp vis smt par fun. (OOProg repr tp vis smt par fun)
    => (repr (OO.Program repr) -> ProgData)
    -> SomeProgGenerator
...
genPackage
  :: (OOProg progRepr tp vis smt par fun, SoftwareDossierSym packRepr, Monad packRepr)
...
genProgram :: (OOProg r tp vis smt par fun) => GenState (OO.GSProgram r)
genProgram = do
  g <- get
...
-- of modularity.
chooseModules :: (OOProg r tp vis smt par fun) => Modularity -> GenState [OO.SFile r]
...
genUnmodular :: (OOProg r tp vis smt par fun) => GenState (OO.SFile r)
genUnmodular = do
  g <- get
...
genModules :: (OOProg r tp vis smt par fun) => GenState [OO.SFile r]
genModules = do
  g <- get
  mn     <- genMain
  inp    <- genInputMod
...
-- be generated in.
generateCodeProc
  :: (ProcProg progRepr tp vis smt par fun, SoftwareDossierSym packRepr, Monad packRepr)
  => Lang
...
-- | Generates an SCS program based on the problem and the user's design choices.
genProgramProc :: (ProcProg r tp vis smt par fun) => GenState (Proc.GSProgram r)
genProgramProc = do
  g <- get
...
-- | Generates an entire SCS program as a single module.
genUnmodularProc :: (ProcProg r tp vis smt par fun) => GenState (Proc.SFile r)
genUnmodularProc = do
  g <- get
  umDesc <- unmodularDesc
  giName <- genICName GetInput
...
-- | Generates all modules for an SCS program.
genModulesProc :: (ProcProg r tp vis smt par fun) => GenState [Proc.SFile r]
genModulesProc = do
  g <- get
  mn     <- genMainProc
  inp    <- genInputModProc
