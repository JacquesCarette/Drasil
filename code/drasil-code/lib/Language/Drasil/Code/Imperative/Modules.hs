...
genMain :: (OOProg r vis smt md svr) => GenState (OO.SFile r)
...
genMainFunc :: (OOProg r vis smt md svr) => GenState (Maybe (MS (r md)))
genMainFunc = do
    g <- get
...
genInputMod :: (OOProg r vis smt md svr) => GenState [OO.SFile r]
genInputMod = do
  ipDesc <- modDesc inputParametersDesc
  cname <- genICName InputParameters
  let genMod :: (OOProg r vis smt md svr) => Maybe (SClass r) -> GenState (OO.SFile r)
...
constVarFunc :: (StateVarSym r vis svr) => ConstantRepr ->
...
genInputClass scp = do
  ...
  let ins = g ^. inputs
      ...
      constructors :: (OOProg r vis smt md svr) => GenState [MS (r md)]
      constructors = if cname `elem` defSet g
        then concat <$> mapM (fmap maybeToList) [genInputConstructor]
        else return []
      methods :: (OOProg r vis smt md svr) => GenState [MS (r md)]
...
-- generated.
genInputConstructor :: (OOProg r vis smt md svr) => GenState (Maybe (MS (r md)))
...
genConstMod :: (OOProg r vis smt md svr) => GenState [OO.SFile r]
genConstMod = do
...
genCalcMod :: (OOProg r vis smt md svr) => GenState (OO.SFile r)
genCalcMod = do
  g <- get
...
genOutputMod :: (OOProg r vis smt md svr) => GenState [OO.SFile r]
genOutputMod = do
...
genOutputFormat :: (OOProg r vis smt md svr) => GenState (Maybe (MS (r md)))
genOutputFormat = do
  g <- get
  modify (\st -> st {currentScope = Local})
...
-- | Generates a controller module.
genMainProc :: (NativeVector r, ProcProg r vis smt md) => GenState (Proc.SFile r)
