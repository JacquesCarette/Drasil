...
eNames (C c)                 = [c]
eNames Lit{}                 = []
eNames (FCall f x ns)        = f : concatMap eNames x <> fmap fst ns <>
                              concatMap (eNames . snd) ns
eNames (New c x ns)          = c : concatMap eNames x <> fmap fst ns <>
                              concatMap (eNames . snd) ns
eNames (Message a m x ns)    = a : m : concatMap eNames x <> fmap fst ns <>
                              concatMap (eNames . snd) ns
...
eNames' (C c)                 = [c]
eNames' Lit{}                 = []
eNames' (FCall _ x ns)        = concatMap eNames' x <> fmap fst ns <>
                               concatMap (eNames .snd) ns
eNames' (New _ x ns)          = concatMap eNames' x <> fmap fst ns <>
                               concatMap (eNames .snd) ns
eNames' (Message a _ x ns)    = a : concatMap eNames' x <> fmap fst ns <>
                               concatMap (eNames .snd) ns
eNames' (Field o f)           = [o, f]
