...
%    \textbf{Units} & Unitless \\ \hline
    ...
    \textbf{Description} & \vbox{
      ...
      \hbox{\strut $...$}
% TODO: Do we want to have all of the variables listed?
...
%      \hbox{\strut $J$ is the stress distribution factor (Unitless)}
    ...
    \textbf{Source} & \cite{ASTM2009}, \cite{BeasonEtAl1998} (Equations 4 \& 5), \& \cite{Campidelli} (Equation 14) \\ \hline % TODO: Campidelli reference?
  \end{tabular}
\end{center}
...
trivial specializations.  Instead, to illustrate this step we will turn our
attention to another example we built in our framework: a solar water heating
system (based on original FORTRAN code from colleagues in mechanical
engineering).  Since this is a heat transfer problem, the initial theory is the
general form of the conservation of thermal energy.  This general form is a
partial differential equation in 3D space and time.  To specialize the theory to
the solar water heating tank, we introduced the following assumptions:

\begin{itemize}

\item The water in the tank is fully mixed, so the temperature of the water is
the same throughout the entire tank.

\item The density of water has no spatial variation; that is, it is constant
over their entire volume.

\item The specific heat capacity of water has no spatial variation; that is, it
is constant over its entire volume.

\end{itemize}

Allows to refine to new theory.  For readers too.  like ..., to get a system of first order ordinary differential equations.  Rationale.  Reuse - change assumptions, family of models.

%% Commenting this out, as it didn't really represent 4b
%
%\subsection*{Step 4b: Choices}
%
