...
function [ErrorArray,LambdaArray,TimeArray,FSArray]=...
    SSA_Original_SliceTester(funcF,slip,strat,phi,coh,gam,gams,piez,gamw,K,ltor,Expected,nCheck)

A=length(nCheck);
ErrorArray=zeros(1,A-1);
LambdaArray=zeros(1,A-1);
...
for j=nCheck
    ...
    rt = cputime;
    [FS, lam] = ...
    funcF(slipnew,strat,phi,coh,gam,gams,piez,gamw,Q,omega,K,ltor,0);
    rtStar = cputime - rt;
