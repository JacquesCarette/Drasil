...
The data from inputFile is read sequentially to populate the state variables from
  FR1 (v\_launch, angle, and p\_target).
...
\begin{center}
 ...
 \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
 \hline
 input\_constraints & Param & - & badVelocity, badAngle, badTargetPosition \\
 \hline
 \end{tabular}
 \end{center}
...
input\_constraints(inParams):
 ...
 \item exception: exc := ( \\
 $\neg (0 < \text{inParams.v\_launch}) \Rightarrow$ badLength $|$\\
 $\neg (0 < \text{inParams.angle} < \frac{\pi}{2}) \Rightarrow$ badAngle $|$\\
 $\neg (0 < \text{inParams.p\_target}) \Rightarrow$ badTargetPosition)
 \end{itemize}
...
func\_d\_offset(inParams, p\_land):
 \begin{itemize}
 \item out: $\textit{out} := $ p\_land - p\_target
 \item exception: exc := none
 \end{itemize}
 
func\_s(inParams, d\_offset):
 \begin{itemize}
 \item out: $\textit{out} := $ ( \\
