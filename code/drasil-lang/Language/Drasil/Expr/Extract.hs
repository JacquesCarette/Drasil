...
names Str{}            = []
names Perc{}           = []
names (FCall f x)      = f : concatMap names x
names (New c x)        = c : concatMap names x
names (Message a m x)  = a : m : concatMap names x
...
names' (New _ x)        = concatMap names' x
names' (Message a _ x)  = a : concatMap names' x
names' (Case _ ls)      = concatMap (names' . fst) ls ++ concatMap (names' . snd) ls
names' (UnaryOp _ u)    = names' u
names' (BinaryOp _ a b) = names' a ++ names' b
names' (Operator _ _ e) = names' e
