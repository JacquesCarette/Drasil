...
helloWorldOO :: (OOProg r tp vis) => OO.GSProgram r
...
-- | Creates the HelloWorld program and necessary files.
helloWorldProc :: (ProcProg r tp vis) => GProc.GSProgram r
...
myOtherList :: (SharedProg r tp vis) => SVariable r
...
helloWorldMainOO :: (OOProg r tp vis) => SMethod r
...
-- | Main function. Initializes variables and combines all the helper functions defined below.
helloWorldMainProc :: (ProcProg r tp vis) => SMethod r
...
helloInitVariables :: (SharedProg r tp vis) => MSBlock r
...
objectTests :: (OOProg r tp vis) => MSBlock r
...
mySlicedList, mySlicedList2, mySlicedList3, mySlicedList4, mySlicedList5,
  ...
  mySlicedList10, mySlicedList11 :: (SharedProg r tp vis) => SVariable r
...
listSliceTests :: (SharedProg r tp vis) => [MSBlock r]
listSliceTests = [
...
helloIfBody :: (SharedProg r tp vis) => MSBody r
...
helloElseBody :: (SharedProg r tp vis) => MSBody r
...
helloWorldClass :: (OOProg r tp vis) => SClass r
