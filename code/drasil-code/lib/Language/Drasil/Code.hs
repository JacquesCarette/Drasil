...
module Language.Drasil.Code (
  ...
  defaultChoices, CodeSpec(..), funcUID, asVC, codeSpec, ($:=), Mod(Mod),
  StateVariable, Func, FuncStmt(..), pubStateVar, privStateVar, fDecDef, ffor,
  fforRange, funcData, funcDef, packmod, junkLine, multiLine, repeated, 
  singleLine, singleton, ExternalLibrary, Step, FunctionInterface, Argument,
  externalLib, choiceSteps, choiceStep, mandatoryStep, mandatorySteps, callStep,
...
import Language.Drasil.Choices (Choices(..), Comments(..), Verbosity(..),
  ...
  InputStructure(..), UValidation(..), ConstantStructure(..), ConstStoreStructure(..),
  ConstantRepr(..), CodeConcept(..), matchConcepts, SpaceMatch, matchSpaces,
  AuxFile(..), getSampleData, Visibility(..), defaultChoices, makeArchit,
  Architecture(..), DataInfo(..), makeData, Maps(..), makeMaps, spaceToCodeType,
  makeConstraints, makeODE, makeDocConfig, makeLogConfig, LogConfig(..),
  OptionalFeatures(..), makeOptFeats, ExtLib(..))
