...
\begin{enumerate}
  ...
  \item[T\refstepcounter{testnum}\thetestnum \label{test_element_support}:]
    ...
    symbol of every element can be processed as input and used throughout the
    execution of \progname{}.
...
These tests will be performed automatically. Note that the input/output format
...
The following tests are for equations of infeasible chemical reactions.
These tests will be performed automatically. Note that the input/output format
for each test is an abstract representation, as the specific format of each
input/output is a design decision that is not made at this stage.
...
\begin{enumerate}
  ...
  \item[T\refstepcounter{testnum}\thetestnum \label{test_bal_accuracy}:]
    ...
    coefficients for each feasible chemical equation, and since they are
    whole numbers \cite{lund_introduction_2023}, they can be compared for
    equality by a computer.

    The \nameref{sec_sysFunReqs} will be performed by comparing the actual
    output to the expected output to ensure that they are exactly equal (e.g.,
    by using integer comparison).
