...
getExpInput prn chs ins = inExp (modularity $ architecture chs) (inputStructure $ dataInfo chs)
  where inExp _ Unbundled = []
        ...
        ipName = genICFuncName (functionNames chs) InputParameters
...
getExpConstants n chs cs = cExp (modularity $ architecture chs) (constStructure $ dataInfo chs)
  ...
  where cExp Unmodular (Store Bundled) _ = zipCs $ repeat n
        ...
        cExp _ WithInputs Bundled = zipCs $ repeat (genICFuncName (functionNames chs) InputParameters)
        cExp _ _ _ = []
...
getConstantsCls chs cs = cnCls (constStructure $ dataInfo chs) (inputStructure $ dataInfo chs)
  where cnCls (Store Bundled) _ = zipCs $ repeat "Constants"
        cnCls WithInputs Bundled = zipCs $ repeat (genICFuncName (functionNames chs) InputParameters)
        cnCls _ _ = []
        zipCs = zip (map codeName cs)
...
getExpDerived n chs _ = dMod (modularity $ architecture chs) (inputStructure $ dataInfo chs)
  where dMod (Modular Separated) _ = [(dvNm, "DerivedValues")]
        ...
        dMod (Modular Combined) _ = [(dvNm, genICFuncName (functionNames chs) InputParameters)]
        dvNm = genICFuncName (functionNames chs) DerivedValues
...
getDerivedCls chs _ = dCls (inputModule chs) (inputStructure $ dataInfo chs)
  where dCls Combined Bundled = [(genICFuncName (functionNames chs) DerivedValues, genICFuncName (functionNames chs) InputParameters)]
        dCls _ _ = []
...
getExpConstraints n chs _ = cMod (modularity $ architecture chs) (inputStructure $ dataInfo chs)
  where cMod (Modular Separated) _ = [(icNm, "InputConstraints")]
        ...
        cMod (Modular Combined) _ = [(icNm, genICFuncName (functionNames chs) InputParameters)]
        icNm = genICFuncName (functionNames chs) InputConstraints
...
getConstraintsCls chs _  = cCls (inputModule chs) (inputStructure $ dataInfo chs)
  where cCls Combined Bundled = [(genICFuncName (functionNames chs) InputConstraints, genICFuncName (functionNames chs) InputParameters)]
        cCls _ _ = []
...
getExpInputFormat n chs _ = fMod (modularity $ architecture chs) (inputStructure $ dataInfo chs)
  where fMod (Modular Separated) _ = [(giNm, genICFuncName (functionNames chs) InputFormat)]
        fMod _ Bundled = []
        fMod Unmodular _ = [(giNm, n)]
        fMod (Modular Combined) _ = [(giNm, genICFuncName (functionNames chs) InputParameters)]
        giNm = genICFuncName (functionNames chs) GetInput
...
getInputFormatCls chs _ = ifCls (inputModule chs) (inputStructure $ dataInfo chs)
  where ifCls Combined Bundled = [(genICFuncName (functionNames chs) GetInput, genICFuncName (functionNames chs) InputParameters)]
        ifCls _ _ = []
...
getExpOutput _ _ [] = []
getExpOutput n chs _ = [(genICFuncName (functionNames chs) WriteOutput, oMod $ modularity $ architecture chs)]
  where oMod Unmodular = n
        oMod _ = "OutputFormat"

-- | Get InternalConcept name using DrasilState
genICName :: InternalConcept -> GenState Name
genICName ic = gets (flip genICFuncName ic . dsICNames)
