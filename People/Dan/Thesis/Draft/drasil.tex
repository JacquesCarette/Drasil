...
single source of domain knowledge. In this chapter, we provide a guided tour of 
...
(Section~\ref{sec:nutshell}). Next, we delve into the mechanisms for capturing
and classifying knowledge (Section~\ref{sec:kc}), introducing the concept of
``chunks'' and their role in building a reusable knowledge base. We then
describe how this knowledge is structured into recipes
(Section~\ref{sec:recipes}) that specify the content and organization of
generated artifacts. The chapter continues with a detailed look at Drasil's
generation and rendering pipeline (Section~\ref{sec:gen}), illustrating how
abstract representations are transformed into concrete outputs. Finally, we
discuss the iterative refinement process that underpins Drasil's development
(Section~\ref{sec:iterefine}) and summarize the key takeaways for our approach
to software generation (Section~\ref{sec:drasil_summary}).
...
understand why Drasil is needed, recall the challenges of manually writing 
and maintaining multiple \sfs{}: the process is redundant, tedious, and
often causes these artifacts to diverge. Drasil is a purpose-built framework created to
...
Unlike documentation generators like Doxygen and Javadoc
that take a code-centric view of the problem and rely on manual
redundancy\textemdash{}that is, natural-language explanations written as
specially delimited comments that can then be woven into API documentation
alongside code\textemdash{}Drasil takes a knowledge-centric,
redundancy-limiting, fully traceable, single-source approach to generating
\sfs{}. The framework builds up knowledge
...
have focused on SC software that follows the 
\mbox{\emph{input~$\rightarrow$~process~$\rightarrow$~output}} pattern. 
Scientific computing often appears both stable 
and change-prone, and that distinction is important for understanding 
Drasil's intended scope. The underlying scientific knowledge is often 
well-understood~\citep{CaretteEtAl2023} enough to be captured explicitly,
even when the software built around it still evolves through maintenance, 
changing assumptions, revised constraints, new outputs, or iterative model 
refinement. In other words, Drasil is aimed at domains where the foundational 
theory is stable enough to encode, while the artifacts derived from that theory 
still benefit from frequent regeneration as the software changes. SC software 
has the benefit of being grounded in relatively stable theories, so even when 
models are revised or refined, older formulations are often still applicable under a set of 
assumptions or within acceptable margins for error.
...
motion~\citep{Newton1687}. To use this law in both documentation and code, we
...
extensible way. To address these requirements, we borrow and repurpose the
...
capturing individual pieces of knowledge. A chunk hierarchy is the 
organization of chunk types from simple, general building blocks to more 
specialized ones that add additional structure or meaning. In Drasil, this 
means later chunk types are constructed by extending earlier ones and reusing 
the information they already encode. We use a hierarchy because many pieces of 
scientific knowledge share common properties, and factoring those shared 
properties into simpler chunk types reduces duplication while making it easier 
to build richer, more specialized representations as needed.
...
firstnumber=33]{code/NamedIdea.hs}}{\codeH{NamedChunk} definition}{fig:namedchunk}
...
simply define our chunks and their intermediate data types (i.e., \codeH{UID} and
...
but in this case the smart constructor used defines given properties (i.e.,
...
this thesis. More information, including detailed definitions of all the types 
and current state of Drasil can be found in the 
\href{https://github.com/JacquesCarette/Drasil/wiki/}
{wiki} or the \href{https://jacquescarette.github.io/Drasil/docs/full/}
...
Even in the small progression we have already seen, the need for multiple 
chunk types is apparent. A \codeH{NamedChunk} is sufficient for naming a 
concept, but once we also want to capture its definition we must move to a 
more specialized \codeH{ConceptChunk}. The more we extend our chunk 
hierarchy to capture additional information such as symbols, units, and 
equations, the larger the number of possible combinations we will need to 
account for. This is not only relevant to chunks themselves, but 
also to the information they encode. For instance, rather than defining 
acceleration in isolation, we may want to define it as the rate of change of 
velocity, where velocity is itself already a captured term. In our effort to 
reduce duplication and maintain a single source of information, we therefore 
intend to create chunks with references to other chunks such that a definition 
can reuse already-captured terminology.

To define terms (such as acceleration) in relation to previously captured terms
(such as velocity), we need a way to project existing knowledge into new
definitions while retaining the flexibility to extend that knowledge. With that
in mind, we created the \codeH{Sentence} DSL for creating and combining 
English-language sentences. In its most basic
...
definitions as it is not derived from existing chunks) and so doesn't require 
the sentence constructor (\codeH{S}) around its definition.
...
examples are available in the code found in the 
\href{https://www.github.com/jacquescarette/drasil}{project repository} for 
...
ones. For example, starting from an existing concept for force, we can reuse 
its name and definition while attaching a symbol and unit to construct a more 
specialized chunk without rewriting the underlying knowledge.
...
definitions particularly in our ability to capture symbols, units, equations, 
and relationships between related quantities in a reusable, traceable way.

Building on those gaps, let us return to Newton's Second Law and examine what
is required to achieve an operational definition of \emph{Force} as a chunk 
in Drasil. To fully represent force, we need not only a name and definition, 
but also a symbolic representation ($F$), a mathematical equation ($F=ma$), and 
associated units ($N$). Likewise, mass and acceleration require their own 
symbols ($m$, $a$), definitions, and units ($kg$, $m/s^2$).
...
definition. Next we need to capture the units of measurement for the force 
...
depth in the full technical documentation found on our 
\href{https://www.github.com/jacquescarette/drasil}{GitHub repository}.}. 
...
language-agnostic (mathematics) representation.
...
As an aside, we will elide the full implementation history of the expression 
DSL known as \codeH{Expr} and instead focus on the design pressures that 
shaped it. We needed a representation that was language-agnostic, 
compositional, and expressive enough to capture the recurring mathematical 
relationships seen across our case studies, while remaining simple enough to 
support projection into multiple generated artifacts. As such, \codeH{Expr} 
was designed by identifying the common operations required in practice and 
factoring them into a reusable core rather than attempting to encode all of 
mathematics at once. With that in mind, 
...
similar). Differentiation is represented structurally: \codeH{deriv a b} records the 
expression \codeH{a} and the quantity \codeH{b} with respect to which it is 
differentiated. This is enough for Drasil to render the corresponding 
mathematical notation, but Drasil does not maintain a semantic understanding of
what it means, mathematically, to differentiate.
...
description. Notice that we are building off of other chunks throughout each 
piece of the chunk's definition, thus giving us perfect traceability from 
beginning to end, regardless of whether we are looking at a defining expression 
...
unique identifier, name, symbol, unit, or space\footnote{Here, a space refers 
to the kind of values a quantity may inhabit, such as the real numbers, 
integers, booleans, or vectors, and it constrains the operations and 
representations that make sense for that quantity.}.
This ``atomic'' information is generally encoded in simpler, more fundamental 
chunks. Complex chunks are 
...
measured with units would be instances of the \codeH{Unitary} type\footnote{
  These classifications are based on the ability 
  to project specific information. We often use lenses to access 
  or manipulate these fields, as they may be projected from component chunks
  embedded within the referenced chunk or stored on it directly.
...
representative subset of the different ways we classify chunks and some example chunk types 
...
\end{center}}{A representative subset of Drasil's chunk classification system
showing some of the encoded property relationships}{fig:chunkClasses}

\fig{\begin{center}
\includegraphics[width=0.75\linewidth]{figures/ChunkClassified.png}
\end{center}}{A representative subset of Drasil's chunks showing how chunks may 
...
relationships that are easy to reason about. The next step is to operationalize
that vocabulary as artifacts people consume.
...
A recipe is, in its simplest form, a specification for one or more \sfs{}.
...
relationships between chunks within a recipe. 
...
example the knowledge could be represented on punched cards). Regardless, we 
would define the underlying knowledge once in our chunks, and use the recipe 
...
to follow, and it has led to some interesting results.

\fig{
\codeBlock{DocDecl}{25}{42}
}{Recipe language for an SRS}{fig:recipeSRS}

As an example, let us take a look at the recipe language we have defined for an 
SRS using the \smithea{}\ template. Looking at the \codeH{DocSection} 
definition from Figure~\ref{fig:recipeSRS}, we can see a striking resemblance 
to (a subset of) the template's Table of Contents (Figure~\ref{fig:SRSToC}). 
Each section is defined by a datatype, which is, through a similar mechanism, 
decomposed into multiple subsections (for example, 
Figure~\ref{fig:recipeReqSec} shows the decomposition of the requirements 
section) which then defines the types of organizational structures we use for 
specifying the contents of a given subsection. When populated, the recipe 
specifies both the structure of our expected \sf{} and the knowledge (chunks) 
contained therein. While it may seem like it, this does not collapse knowledge 
capture and presentation into a single concern: the knowledge remains encoded 
independently in reusable chunks, while the recipe specifies how that knowledge 
should be selected and organized for a particular \sf{}. For an SRS in 
particular, this is a natural fit, since the template is itself a systematic 
way of presenting scientific knowledge to stakeholders.
...
\sf{}? For that, we should look at a specific example from our \gb{} case study.
...
(e.g., \codeH{TypogConvention[Vector Bold]}) and is used by a relevant portion of
...
of chunks necessary for filling in the sections of our SRS. 
Each piece of the object consists of one or more chunks to be 
used in filling in the relevant sections. A quick description of each property 
can be found in Table~\ref{tab:siBreakdown}, but it should be noted that some 
of the defined properties are only there for convenience (and/or remain only 
until the next refinement pass) as they are derived from other chunks with the 
...
referenced from within our knowledge bases and core Drasil infrastructure,
as they can be shared, reused, and are not necessarily specific to only this 
system.} including the system information object is contained in a single file
in fewer than 370 lines of code.\footnote{Including many comments, whitespace,
and import statements.} Teasing our results: the generated SRS is a 44-page
Portable Document Format (PDF) file (with the generated \LaTeX{} source being
1,742 lines\footnote{At time of writing.}). This is partially due to our
ability to strip away all of the common boilerplate text that we need not worry
about right now, as that will be a problem for the generator
(Section~\ref{sec:gen}).
As it stands we have created the recipe language to represent what we truly 
want out of a recipe: a simplified, unnecessary-duplication-avoiding, fully 
traceable representation of our target \sf{}.

With the combination of system information and the SRS recipe we can move
...
\href{https://github.com/jacquescarette/drasil}{Drasil GitHub repository}. 
However, there is one \sf{} that is significantly
different, and
...
Choices.hs
\codeBlock{Choices}{13}{27}}{Recipe for \gb{}'s executable
code}{fig:gbCodeRecipe}

\fig{
\codeBlock{CodeSpec}{41}{72}
}{Recipe language for executable code (\codeH{CodeSpec})}{fig:codeSpec}
...
generated are defined in the \codeH{ModuleDefs.hs} file and make reference to 
other chunks (both system specific and generic) from our knowledge base. Looking
into the modules, we can see that the \codeH{readTableMod} module, for example,
is defined as shown in Figure~\ref{fig:readTableMod}. This module provides a
function for reading ASTM International glass data from a file, encapsulating
both the knowledge of the data format and the implementation logic required for
code
generation.
...
\\ \hline 
	dataInfo & Data structure and representation choices. (e.g., bundle inputs
...
\\\hline 
	...
	and types in the target language(s) (e.g., one could map the concept of $\pi$
...
\\\hline 
	...
	independently.
\\\hline 
	extLibs & List of external libraries to use (e.g., for solving specific
...
is automatically reflected in the generated code, as in the documentation, 
after re-running the generation process. This approach minimizes duplication, 
maximizes consistency, and enables reliable code generation for scientific 
computing applications.

\section{Cooking It All Up: Generation and Rendering}
\label{sec:gen}
...
Abstract Structure to Concrete Formats}
...
use (e.g., \LaTeX{}, Hypertext Markup Language (HTML), PDF, or source code in
various programming languages). Drasil achieves this through a set of dedicated
\emph{printers}
and, for code, through the use of the Generic Object-Oriented Language 
(GOOL)~\citep{Costabile2012, Carette2020GOOL}. Each printer is designed to
...
\begin{itemize}
    ...
    machine-readable JavaScript Object Notation (JSON) representation,
    supporting further processing or integration.
\end{itemize}

Each printer traverses the recipe structure, visiting each section and 
subsection, and calls formatting routines appropriate to the target format.
...
For executable code artifacts, Drasil uses GOOL~\citep{Costabile2012,
Carette2020GOOL, maclachlan2020} as an intermediate layer. GOOL provides a 
collection of type-safe combinators and abstractions for representing object-oriented and 
procedural constructs in a language-agnostic manner. The code recipe (e.g., a
\codeH{CodeSpec}) is first translated into a GOOL abstract syntax tree (AST), 
which is then emitted as source code in one or more target languages. The 
design and implementation of GOOL itself are treated in the cited works; for 
our purposes, the important point is that it provides an intermediate 
representation between the recipe and the emitted code.

In brief, code emission follows three stages. First, the code recipe and its
referenced chunks are traversed to construct a GOOL AST representing classes,
functions, variables, and control flow. Next, the GOOL backend is invoked for
each specified target language, translating the AST into concrete source code
(e.g., Python, C++, Java, or C\#). Finally, dedicated routines generate
auxiliary files such as a README, Doxygen configuration, and sample input
files.
...
links (e.g., comments referencing requirements or data definitions).
Figure~\ref{fig:generateCode} shows the relevant code path.

\fig{\codeBlock{ImperativeGenerator}{98}{113}}{\codeH{generateCode} and
\codeH{genPackage} for code rendering}{fig:generateCode}
...
includes writing the primary artifacts to disk (e.g., \texttt{.tex},
\texttt{.html}, \texttt{.py}, and \texttt{.cpp} files), generating auxiliary
files (e.g., Makefiles, Cascading Style Sheets (CSS), README files, and example
input files), linking cross-references, and ensuring the integrity of tables,
...
\fig{\codeBlock{CodeGeneration}{28}{38}}{\codeH{createCodeFiles} for code
...
formatted text in the generated SRS. The following subsection then traces the 
same chunk into the generated source code to show how the same captured 
knowledge supports both renderings.
...
When the generator is invoked to produce the SRS (e.g., as a \LaTeX{}
document), it traverses the recipe and projects each chunk according to the
section. For the Data Definitions section, it projects the full definition of
$\hat{q}$, including its symbol, description, units, and defining equation;
for the Table of Symbols, it projects only the symbol, a brief description,
and the units. The rendering engine then formats those projections for the
target output, as illustrated by the generated \LaTeX{} in
Figure~\ref{fig:rawDD}.

\fig{\TeXBlock{GlassBR_SRS}{750}{799}}{Raw generated \LaTeX{} for the $\hat{q}$
...
alongside all other relevant auxiliary files (e.g., a \texttt{.bib} file for
...
a GOOL intermediate representation. The $\hat{q}$ chunk, for example, is mapped
to a function definition within the GOOL abstract syntax tree (AST), preserving
its formula and metadata. This intermediate form enables
...
\fig{\pyBlock{GenCalculations}{34}{40}}{$\hat{q}$ as defined in generated
Python code}{fig:qhatPython}

\takeaway{Drasil separates knowledge capture from artifact production. Recipes
select and organize the relevant chunks, rendering translates their projections
into target-specific representations, and assembly combines those
representations with cross-references and supporting files. Each artifact can
present knowledge differently for its purpose while remaining traceably
connected to the same source, allowing changes to propagate without maintaining
separate copies of that knowledge.}
...
development practices~\citep{Segal2005, CarverEtAl2007, AckroydEtAl2008, 
EasterbrookAndJohns2009}. As such, and as we have alluded to throughout this and
...
(Issue \href{https://github.com/JacquesCarette/Drasil/issues/348}{\#348} on the 
GitHub issue tracker). The original SSP SRS used the pair $S_i$ and $P_i$ in 
one place and later replaced $P_i$ with $\tau_i$. This blurred the distinction 
between shear force and shear stress, since $\tau_i$ should denote shear stress
rather than resistive shear force. $\tau_i$ was also omitted from the Table of 
Symbols. To reiterate, that inconsistency existed in the original case study 
and was not a generator bug but an error in the source document we had accepted 
as our starting point. Once the SSP material was encoded as chunks in Drasil 
the fix was minimal: ensure we had encoded the correct definitions of $S_i$, 
$P_i$, and $\tau_i$ and used them in the appropriate equations. Then the 
correction automatically propagated to all generated artifacts (Table of 
Symbols, cross-references, and any code that referenced the symbol).
...
of 1~m depth ``into the page'' for one of the equations. That assumption was
...
case study and our Drasil output, or some as-yet unencoded knowledge (e.g.,
...
regenerate. That workflow was the dominant mode of refinement of the original
...
for those derived from standard SI units (e.g., $m/s^2$). This involved the
...
\takeaway{We learned how to improve Drasil by trying to reproduce existing case
...
expressions; and the Expr/Sentence DSLs give us 
language-agnostic ways to 
...
Drasil is not a universal solution\textemdash{}it is a tool targeted at domains
where the
