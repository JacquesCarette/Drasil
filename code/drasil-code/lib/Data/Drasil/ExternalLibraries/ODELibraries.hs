...
import Language.Drasil (HasSymbol(symbol), HasUID(uid), MayHaveUnit(getUnit),
  ...
  sub, Idea(getA), NamedIdea(term), Stage(..), (+++), vectND)
    ...
    integrateConst [
      ...
      customObjArg [] "Class representing an ODE system" ode odeCtor
        (customClass [constructorInfo odeCtor [] [],
          methodInfoNoReturn odeOp "function representation of ODE system"
            [unnamedParam (vectND "n" Real), unnamedParam (vectND "n" Real), lockedParam t] -- TODO: what do we put here? We need specific dimensions now.
            [assignArrayIndex]]),
      ...
      customObjArg []
        ...
        pop popCtor (customClass [
          constructorInfo popCtor [unnamedParam (vectND "n" Real)] [], -- TODO: Same as above TODO
...
odeintCurrVals = quantvar $ implVar "currVals_odeint" (nounPhrase
  ...
  (vectND "n" Real) (label "currVals") -- TODO: Same as above, what do we fill in here?
...
y = quantvar $ implVar "y_ode" (nounPhrase
  ...
  "current dependent variable value in ODE solution")
  (vectND "n" Real) (label "y") -- TODO: Same as above - what dimension do we use here?
