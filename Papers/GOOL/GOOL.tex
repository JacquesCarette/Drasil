...
%% Title information
\title[GOOL]{GOOL: A Generic Object-Oriented Language}         %% [Short %%Title] is optional;
...
\begin{abstract}
  We present GOOL, a Generic Object-Oriented Language.  GOOL demonstrates that
  ...
  GOOL is an embedded DSL in Haskell
...
In fact, with proper case, on can go even further and
...
to generate all the requirement documentation and code from expert-provided
...
thus the most potential users --- one reasons that the makers
...
attention. We paid close attention to readaibility features --- such
...
There are three standards methods of encoding EDSLs in Haskell:
...
is not need by the other languages.  Our language is defined by 
...
%% TODO: try to bring some of this back
\begin{comment}
...
\verb|argList| that represents the list of arguments, with a dedicated API.
...
\emph{statement}) is interesting as the quite it generates varies
...
used their parameters differently: inputs, outputs and both.
...
Concretely, the following Pythong code
\begin{lstlisting}
...
different kinds of parameters, ends up being rendered 
differently,
...
And so it is in GOOL,
with \verb|getMethod "FooClass" foo| and \verb|setMethod "FooClass" foo|
being enough information, and yet abstracts over the idiosyncracies of
