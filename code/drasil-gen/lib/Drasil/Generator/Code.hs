...
import Drasil.GOOL (unJC, unPC, unCSC, unCPPC, unSC, CodeType(..), ProgData,
  TypeData, OOProg, LoggingFor (unLC))
...
genCode syst chs = directory [ps|src|] <$> traverse genLangCode (lang chs)
  where
    ...
    genCall
      :: (OOProg progRepr TypeData, SoftwareDossierSym packRepr, Monad packRepr)
      => Lang
    ...
    genCallProc
      :: (ProcProg progRepr TypeData, SoftwareDossierSym packRepr, Monad packRepr)
      => Lang
