...
impulseVDerivEqn1 = sy QP.force $= sy QPP.mass `mul` sy QP.acceleration
                     $= sy QPP.mass `mul` deriv (sy QP.velocity) QP.time
...
impulseVDerivEqn2 = defint (eqSymb timeT) (sy time_1) (sy time_2) (sy QP.force) $=
                    sy QPP.mass `mul` defint (eqSymb QP.velocity) (sy velo_1) (sy velo_2) (exactDbl 1)

impulseVDerivEqn3 :: ModelExpr
impulseVDerivEqn3 = defint (eqSymb timeT) (sy time_1) (sy time_2) (sy QP.force)
                    $= (sy QPP.mass `mul` sy velo_2) $- (sy QPP.mass `mul` sy velo_1) 
                    $= sy QPP.mass `mul` sy QP.chgInVelocity
