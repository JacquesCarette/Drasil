...
- Wait until something is "complete" in a branch before making a [pull request](https://github.com/JacquesCarette/Drasil/wiki/Git2Know-for-Drasil#pull-requests) (PR). It's fine to have lots of work that is incomplete in a branch (even multiple branches), but PRs should be done when it is expected that they can be incorporated "right away" into main. Before making a PR, you should check `make pr_ready` to ensure that it will pass the automated tests necessary to get your code merged in right away.
  - While this is a good rule to follow for most PRs, there are cases where you can create a PR with work that isn't ready to merge into main. For example, if you need feedback on your current progress, it is usually fine to create a PR to get that feedback. What matters most is that you create a PR with a clear goal in mind, and that you stick to that goal once you create that PR. If the goal changes, you will likely need to close the current PR and open a new one when the time is right.
...
- I am working on an issue that requires multiple pull requests to complete, some of which are dependent on others. I have created one branch that I intend to use for my multiple pull requests for the same issue.
    - This can be done; however, it is problematic, as outlined in [this comment](https://github.com/JacquesCarette/Drasil/pull/2157#issuecomment-637079762); instead, follow the advice given in the [same comment](https://github.com/JacquesCarette/Drasil/pull/2157#issuecomment-637079762). In summary, each PR must have its own branch. You can chain PRs by setting a dependent PR's base to the earlier PR's branch. For example, given branches `main`, `A`, and `B`: file a PR from `A` to `main`, and a PR from `B` to `A`. When the dependency (`A`) merges into `main`, the dependent PR (`B`) will [automatically retarget to `main`](https://github.blog/changelog/2020-05-19-pull-request-retargeting/).
...
- After checking to make sure the changes in `build` are correct and wanted, update the files in stable. This can be done by using `make stabilize`, or for a specific example `X`, `make X_stabilize`. Alternatively, you can do this manually by placing a copy of each specific build file that changed (the ones the logs showed there was a problem with) into the stable folder, overwriting the existing ones in stable. Commit and push these changes before a PR. This will help avoid Continuous Integration (CI) build failures or errors.
