...
hFromtDD = mkDD hFromt [makeRef astm2009] [{-derivation-}] ""--temporary
  (Just $ [hMin])
...
loadDFDD = mkDD loadDF [makeRef astm2009] [{-derivation-}] ""--temporary
  Nothing
...
strDisFacDD = mkDD strDisFac [makeRef astm2009] [{-derivation-}] ""--temporary
...
nonFLDD = mkDD nonFL [makeRef astm2009] [{-derivation-}] ""--temporary
...
glaTyFacDD = mkDD glaTyFac [makeRef astm2009] [{-derivation-}] ""--temporary
...
dimLLDD = mkDD dimLL [makeRef astm2009, makeRef campidelli +:+ sParen (S "Eq. 7")] [{-derivation-}] ""--temporary
...
tolPreDD = mkDD tolPre [makeRef astm2009] [{-derivation-}] ""--temporary
  (Just $ qHtTlExtra : [])
...
tolStrDisFacDD = mkDD tolStrDisFac [makeRef astm2009] [{-derivation-}] ""--temporary
...
standOffDisDD = mkDD standOffDis [makeRef astm2009] [{-derivation-}] ""--temporary
  Nothing
...
aspRatDD = mkDD aspRat [makeRef astm2009] [{-derivation-}] ""--temporary
  (Just $ aGrtrThanB : [])
...
hRef = (ch min_thick +:+ S "is the minimum thickness" `sC` 
  S "which is based on the nominal thicknesses as shown in" +:+. makeRef hFromtDD)

hsGlass :: Sentence
hsGlass = (getAcc heatS +:+ S "is" +:+ phrase heatS +:+ S "glass")

ldfRef :: Sentence
ldfRef = (ch lDurFac +:+ S "is the" +:+ phrase lDurFac +:+ S "as defined by" +:+. makeRef loadDFDD)
...
qHtTlExtra = (ch tolLoad +:+ S "is the tolerable load which is obtained from Figure 7 using" 
  ...
  S "are defined in" +:+. makeRef tolStrDisFacDD `sAnd` makeRef aspRatDD `sC` S "respectively")

qHtTlTolRef :: Sentence
qHtTlTolRef = (ch tolLoad +:+ S "is the tolerable load defined in" +:+. makeRef tolPreDD)

qRef :: Sentence
qRef = (ch demand +:+ S "is the 3 second equivalent pressure, as given in" +:+. makeRef calOfDemandL)

gtfRef :: Sentence
gtfRef = (ch gTF +:+ S "is the" +:+. (phrase gTF `sC` S "as given by" +:+ makeRef glaTyFacDD))

qHtRef :: Sentence
qHtRef = (ch dimlessLoad +:+ S "is the" +:+ phrase dimlessLoad +:+ S "defined in" +:+. makeRef dimLLDD)
