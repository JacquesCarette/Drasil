...
Source & --
\\ \midrule \\
RefBy & \hyperref[IM:messageIM]{IM: messageIM} \hyperref[IM:hitIM]{IM: hitIM} \hyperref[calcValues]{FR: Calculate-Values}.
\\ \bottomrule \end{tabular}
\end{minipage}
\par~
...
\item[Output-Values:\phantomsection\label{outputValues}]If $isHit$ (from \hyperref[IM:hitIM]{IM: hitIM}), output the output message as a string ``The target was hit.'' Otherwise, if $isShort$ (from \hyperref[IM:hitIM]{IM: hitIM}), output the output message as a string ``The projectile fell short.'' and ${d_{offset}}$. Otherwise, output the output message as a string ``The projectile went long.'' and ${d_{offset}}$.
\end{itemize}
