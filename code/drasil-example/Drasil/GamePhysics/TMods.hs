...
import qualified Data.Drasil.Quantities.Physics as QP (accelerationUC, 
  angularAccel, angularVelocity, displacementUC, forceUC, gravitationalConstUC, 
  momentOfInertiaUC, torqueUC, velocityUC)
...
t1NewtonSL_new = tm (cw newtonSL)
  [qw QP.forceUC, qw QPP.mass, qw QP.accelerationUC] ([] :: [ConceptChunk])
  [] [(sy QP.forceUC) $= (sy QPP.mass) * (sy QP.accelerationUC)] [] [] 
  l1 [newtonSLDesc]
...
newtonSLRel = (sy QP.forceUC) $= (sy QPP.mass) * (sy QP.accelerationUC)

newtonSLDesc :: Sentence
newtonSLDesc = foldlSent [S "The net", (phrase QP.forceUC), (ch QP.forceUC),
  (sParen $ Sy $ unit_symb QP.forceUC), S "on a", (phrase CP.rigidBody),
  S "is proportional to the", (phrase QP.accelerationUC),
  (ch QP.accelerationUC), (sParen $ Sy $ unit_symb QP.accelerationUC),
...
t3NewtonLUG_new = tm (cw newtonLUG)
  [qw QP.forceUC, qw QP.gravitationalConstUC, qw mass_1, qw mass_2,
  qw dispNorm, qw dispUnit, qw QP.displacementUC] ([] :: [ConceptChunk])
  [] [(sy QP.forceUC) $= (sy QP.gravitationalConstUC) * ((sy mass_1) * 
  (sy mass_2) / ((sy dispNorm) $^ (fromInteger 2))) * (sy dispUnit) $= 
  (sy QP.gravitationalConstUC) * ((sy mass_1) * (sy mass_2) / ((sy dispNorm) 
  $^ (fromInteger 2))) * ((sy QP.displacementUC) / (sy dispNorm))] [] [] 
  l3 [newtonLUGDesc]
...
newtonLUGRel = (sy QP.forceUC) $=
  (sy QP.gravitationalConstUC) * ((sy mass_1) * (sy mass_2) /
  ((sy dispNorm) $^ (fromInteger 2))) * (sy dispUnit) $=
  (sy QP.gravitationalConstUC) * ((sy mass_1) * (sy mass_2) /
  ((sy dispNorm) $^ (fromInteger 2))) * ((sy QP.displacementUC) / (sy dispNorm))

-- Can't include fractions within a sentence (in the part where 'r denotes the
-- unit displacementUC vector, equivalent to r/||r||' (line 184)). Changed to a
...
newtonLUGDesc = foldlSent [S "Two", (plural CP.rigidBody), S "in the universe",
  S "attract each other with a", (phrase QP.forceUC), 
  (ch QP.forceUC), (sParen $ Sy $ unit_symb QP.forceUC),
  ...
  S "between them. The vector", (ch QP.displacementUC), 
  (sParen $ Sy $ unit_symb QP.displacementUC), S "is the", 
  (phrase QP.displacementUC), S "between the centres of the", 
  ...
  S "equivalent to the", (phrase QP.displacementUC), 
  S "divided by the" +:+. ((phrase dispNorm) `sC`
  S "as shown above"), S "Finally" `sC` (ch QP.gravitationalConstUC), 
  S "is the", (QP.gravitationalConstUC ^. defn), 
  (sParen $ Sy $ unit_symb QP.gravitationalConstUC)]
...
t5NewtonSLR_new = tm (cw newtonSLR)
  [qw QP.torqueUC, qw QP.momentOfInertiaUC, qw QP.angularAccel] 
  ([] :: [ConceptChunk]) [] [(sy  QP.torqueUC) $= (sy QP.momentOfInertiaUC) 
...
newtonSLRRel = (sy  QP.torqueUC) $= (sy QP.momentOfInertiaUC) * (sy QP.angularAccel)
...
newtonSLRDesc = foldlSent [S "The net", (phrase QP.torqueUC),
  (ch QP.torqueUC),
  (sParen $ Sy $ unit_symb  QP.torqueUC), S "on a", (phrase CP.rigidBody),
  S "is proportional to its", (phrase QP.angularAccel),
  (ch QP.angularAccel) +:+. (sParen $ Sy $ unit_symb QP.angularAccel),
  S "Here" `sC` (ch QP.momentOfInertiaUC),
  (sParen $ Sy $ unit_symb QP.momentOfInertiaUC),
  S "denotes the", (phrase QP.momentOfInertiaUC), S "of the" +:+.
