...
import Data.Drasil.Quantities.Physics as QP (timeUC)
...
nwtnCooling_rel = apply1 ht_flux QP.timeUC $= sy htTransCoeff *
  apply1 deltaT QP.timeUC
...
nwtnCooling_desc = foldlSent [at_start law_conv_cooling +:+.
  ...
  S "and its surroundings", E (apply1 thFluxVect QP.timeUC) `isThe`
  ...
  sParen (Sy $ unit_symb htTransCoeff), E (apply1 deltaT QP.timeUC $= 
  apply1 temp QP.timeUC - apply1 temp_env QP.timeUC) `isThe` 
...
rocTempSimp_rel = (sy QPP.mass) * (sy QT.heat_cap_spec) *
  deriv (sy QT.temp) QP.timeUC $= sy ht_flux_in * sy in_SA -
...
rocTempSimp_desc = foldlSent [S "The basic", phrase equation,
  ...
  phrase QP.timeUC, ch QPP.mass `isThe` phrase QPP.mass +:+.
  ...
  ch QP.timeUC `isThe` phrase QP.timeUC +:+. sParen (Sy $ unit_symb QP.timeUC),
