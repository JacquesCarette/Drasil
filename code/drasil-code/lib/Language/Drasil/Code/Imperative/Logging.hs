...
logBody :: (SharedProg r tp vis) => Label -> [SVariable r] -> [MSBlock r] ->
  GenState (MSBody r)
logBody n vars b = do
  g <- get
  return $ body $
...
loggedMethod :: (SharedProg r tp vis) => FilePath -> Label -> [SVariable r] -> MSBlock r
