...
def read_table(filename, z_vect3DSor, x_matrix, y_matrix):
    ...
    for i in range(0, len(linetokens) // 1, 1):
        z_vect3DSor.append(float(linetokens[i * 1 + 0]))
    outfile = open("log.txt", "a")
    print("var 'z_vect3DSor' assigned ", end="", file=outfile)
    print(z_vect3DSor, end="", file=outfile)
