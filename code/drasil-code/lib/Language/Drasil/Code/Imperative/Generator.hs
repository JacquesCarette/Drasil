...
data SomeProgGenerator where
  SomeProgGenerator
    :: forall repr tp vis smt par. (OOProg repr tp vis smt par)
...
genPackage
  :: (OOProg progRepr tp vis smt par, SoftwareDossierSym packRepr, Monad packRepr)
...
genProgram :: (OOProg r tp vis smt par) => GenState (OO.GSProgram r)
genProgram = do
  g <- get
...
-- of modularity.
chooseModules :: (OOProg r tp vis smt par) => Modularity -> GenState [OO.SFile r]
...
genUnmodular :: (OOProg r tp vis smt par) => GenState (OO.SFile r)
genUnmodular = do
  g <- get
...
genModules :: (OOProg r tp vis smt par) => GenState [OO.SFile r]
genModules = do
  g <- get
  mn     <- genMain
  inp    <- genInputMod
...
-- be generated in.
generateCodeProc
  :: (ProcProg progRepr tp vis smt par, SoftwareDossierSym packRepr, Monad packRepr)
  => Lang
...
-- | Generates an SCS program based on the problem and the user's design choices.
genProgramProc :: (ProcProg r tp vis smt par) => GenState (Proc.GSProgram r)
genProgramProc = do
  g <- get
...
-- | Generates an entire SCS program as a single module.
genUnmodularProc :: (ProcProg r tp vis smt par) => GenState (Proc.SFile r)
genUnmodularProc = do
  g <- get
  umDesc <- unmodularDesc
  giName <- genICName GetInput
...
-- | Generates all modules for an SCS program.
genModulesProc :: (ProcProg r tp vis smt par) => GenState [Proc.SFile r]
genModulesProc = do
  g <- get
  mn     <- genMainProc
  inp    <- genInputModProc
