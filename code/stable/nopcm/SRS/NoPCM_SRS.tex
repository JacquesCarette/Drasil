...
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{A:Density-Water-Constant-over-Volume}:]The density of water has no spatial variation; that is, it is constant over their entire volume {[}GD2, LC\ref{Temperature-Coil-Variable-Over-Day}{]}.
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{A:Specific-Heat-Energy-Constant-over-Volume}:]The specific heat capacity of water has no spatial variation; that is, it is constant over its entire volume {[}GD2{]}.
\end{description}
\begin{description}
...
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{A:No-Internal-Heat-Generation-By-Water}:]No internal heat is generated by the water; therefore, the volumetric heat generation per unit volume is zero {[}IM1{]}.
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{A:Atmospheric-Pressure-Tank}:]The pressure in the tank is atmospheric, so the melting point temperature and boiling point temperature of water are 0${}^{\circ}$C and 100${}^{\circ}$C, respectively {[}IM1, IM2{]}.
\end{description}
\begin{description}
...
\label{Sec:FRs}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{FR:Input-Inital-Values}:]Input the following quantities, which define the tank parameters, material properties and initial conditions:
\end{description}
...
\end{longtabu}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{FR:Find-Mass}:]Use the inputs in R\ref{FR:Input-Inital-Values} to find the mass needed for IM1 to IM2, as follows, where ${V_{W}}$ is the volume of water and ${V_{tank}}$ is the volume of the cylindrical tank:
\end{description}
\begin{dmath}
{m_{W}}={V_{W}} {ρ_{W}}=\frac{D}{2} L {ρ_{W}}
\end{dmath}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{FR:Check-Inputs-Satisfy-Physical-Constraints}:]Verify that the inputs satisfy the required physical constraint shown in Table~\ref{Table:InDataConstraints}.
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{FR:Output-Input-Derivied-Quantities}:]Outputs and inputs quantities and derived quantities in the following list: the quantities from R\ref{FR:Input-Inital-Values}, the mass from R\ref{FR:Find-Mass} and ${τ_{W}}$ (from IM1).
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{FR:Calculate-Temperature-Water-Over-Time}:]Calculate and output the temperature of the water (${T_{W}}$($t$)) over the simulation time
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{FR:Calculate-Change-Heat\_Energy-Water-Time}:]Calculate and output the change in heat energy in the water (${E_{W}}$($t$)) over the simulation time (from IM3).
\end{description}
...
\label{Sec:LCs}
\begin{description}
\item[\refstepcounter{lcnum}\lcthelcnum\label{Temperature-Coil-Variable-Over-Day}:]A\ref{A:Temp-Heating-Coil-Constant-over-Time} - The temperature of the heating coil will change over the course of the day, depending on the energy received from the sun.
\end{description}
\begin{description}
...
\end{description}
\begin{description}
\item[\refstepcounter{lcnum}\lcthelcnum\label{LC:Discharging-Tank}:]A\ref{A:Charging-Tank-No-Temp-Discharge}- The model currently only accounts for charging of the tank. That is, increasing the temperature of the water to match the temperature of the coil. A more complete model would also account for discharging of the tank.
\end{description}
\begin{description}
...
\label{Sec:UCs}
\begin{description}
\item[\refstepcounter{ucnum}\uctheucnum\label{UC:Water-Fixed-States}:]A\ref{A:Water-Always-Liquid} - It is unlikely for the change of water from liquid to a solid, or from liquid to gas to be considered.
\end{description}
\begin{description}
\item[\refstepcounter{ucnum}\uctheucnum\label{UC:No-Internal-Heat-Generation}:]A\ref{A:No-Internal-Heat-Generation-By-Water} - Is used for the derivations of IM1 (Hack: need Label to fix).
\end{description}
...
\toprule
& IM1 (\hyperref[IM:eBalanceOnWtr]{Definition~IM:eBalanceOnWtr}) & IM2 (\hyperref[IM:heatEInWtr]{Definition~IM:heatEInWtr}) & Data Constraints (Table~\ref{Table:InDataConstraints}) & R1 (R\ref{FR:Input-Inital-Values}) & R2 (R\ref{FR:Find-Mass}) & R3 (R\ref{FR:Check-Inputs-Satisfy-Physical-Constraints}) & R4 (R\ref{FR:Output-Input-Derivied-Quantities}) & R5 (R\ref{FR:Calculate-Temperature-Water-Over-Time}) & R6 (R\ref{FR:Calculate-Change-Heat_Energy-Water-Time})
\\
\midrule
...
IM2 (\hyperref[IM:heatEInWtr]{Definition~IM:heatEInWtr}) &  &  &  &  &  &  &  &  & 
\\
R1 (R\ref{FR:Input-Inital-Values}) &  &  &  &  &  &  &  &  & 
\\
R2 (R\ref{FR:Find-Mass}) &  &  &  & X &  &  &  &  & 
\\
R3 (R\ref{FR:Check-Inputs-Satisfy-Physical-Constraints}) &  &  & X &  &  &  &  &  & 
\\
R4 (R\ref{FR:Output-Input-Derivied-Quantities}) &  &  &  & X & X &  &  &  & 
\\
R5 (R\ref{FR:Calculate-Temperature-Water-Over-Time}) & X &  &  &  &  &  &  &  & 
\\
R6 (R\ref{FR:Calculate-Change-Heat_Energy-Water-Time}) &  & X &  &  &  &  &  &  & 
\\
\bottomrule
\caption{Traceability Matrix Showing the Connections Between Requirements and Instance Models}
...
\\
LC1 (LC\ref{Temperature-Coil-Variable-Over-Day}) &  &  &  &  &  &  & X &  &  &  &  &  &  & 
\\
