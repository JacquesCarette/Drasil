...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & This equation satisfies \hyperref[T:equilibrium]{TM: equilibrium} in the shear direction. Force equilibrium is derived from the free body diagram of \hyperref[Figure:ForceDiagram]{ForceDiagram} in Section~\ref{Sec:PhysSyst}.
\\ \midrule \\
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & This equation satisfies \hyperref[T:equilibrium]{TM: equilibrium} in the shear direction. Force equilibrium is derived from the free body diagram of Figure~\ref{Figure:ForceDiagram} in Section~\ref{Sec:PhysSyst}.
\\ \midrule \\
Source & \cite{chen2005}
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & From the definition of the factor of safety in \hyperref[T:factOfSafety]{TM: factOfSafety}, and the new definition of $P$, a new relation for the net mobile shear force of the slice $T$ is found as the resistive shear $P$ (\hyperref[GD:resShr]{Definition~GD:resShr}) divided by the factor of safety $FS$. \hyperref[A:Factor-of-Safety]{A: Factor-of-Safety} \hyperref[A:Soil-Layer-Homogeneous]{A: Soil-Layer-Homogeneous} \hyperref[A:Soil-Properties]{A: Soil-Properties} \hyperref[A:Soil-Layers-Isotropic]{A: Soil-Layers-Isotropic}
\\ \midrule \\
Source & \cite{chen2005}
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
...
Description & \begin{symbDescription}
              ...
              \end{symbDescription}
\\ \midrule \\
Notes & \hyperref[A:Soil-Layer-Homogeneous]{A: Soil-Layer-Homogeneous}
        \hyperref[A:Soil-Properties]{A: Soil-Properties}
        \hyperref[A:Soil-Layers-Isotropic]{A: Soil-Layers-Isotropic}
\\ \midrule \\
Source & \cite{chen2005}
\\ \midrule \\
RefBy & FIXME: This needs to be filled in
\\ \bottomrule \end{tabular}
\end{minipage}\\
The resistive shear force of a slice is defined as $P$ in \hyperref[GD:resShr]{resShr}. The effective normal force in the equation for $P$ of the soil is defined in the perpendicular force equilibrium of a slice from \hyperref[GD:bsShrFEq]{bsShrFEq}, using the effective normal force $N'$ of \hyperref[T:effStress]{TM: effStress} shown in equation (1):
\begin{dmath}
...
\end{dmath}
The values of the interslice forces $G$ and $X$ in the equation are unknown, while the other values are found from the physical force definitions of \hyperref[DD:sliceWght]{DD: sliceWght} to \hyperref[DD:lengthLs]{DD: lengthLs}. Consider a force equilibrium without the affect of interslice forces, to obtain a solvable value as done for $N*$ in equation (2):
\begin{dmath}
...
\end{dmath}
This can be further simplified by considering assumptions \hyperref[A:Seismic-Force]{A: Seismic-Force} and \hyperref[A:Surface-Load]{A: Surface-Load}, which state that the seismic coefficient and the external force, respectively, are0 Removing seismic and external forces yields  equation (4):
\begin{dmath}
...
\\ \bottomrule \end{tabular}
\end{minipage}\\
The mobilized shear force acting on a slice is defined as $S$ from the force equilibrium in \hyperref[GD:bsShrFEq]{bsShrFEq}, also shown in equation (5):
\begin{dmath}
S_{i}=\left(W_{i}-X_{i-1}+X_{i}+{U_{t,i}} \cos\left(β_{i}\right)+Q_{i} \cos\left(ω_{i}\right)\right) \sin\left(α_{i}\right)+\left(-{K_{c}} W_{i}-G_{i}+G_{i-1}-H_{i}+H_{i-1}+{U_{t,i}} \sin\left(β_{i}\right)+Q_{i} \sin\left(ω_{i}\right)\right) \cos\left(α_{i}\right)
\end{dmath}
...
\end{dmath}
Substituting the equation for $N'$ from equation (16) \hyperref[DD:resShearWO]{DD: resShearWO} and \hyperref[DD:mobShearWO]{DD: mobShearWO} into equation (17) and rearranging results in equation (18):
\begin{dmath}
...
\end{dmath}
Where $R$ and $T$ are the resistive and mobile shear of the slice, without the influence of interslice forces $G$ and $X$, as defined in \hyperref[DD:resShearWO]{DD: resShearWO} and \hyperref[DD:mobShearWO]{DD: mobShearWO} Making use of the constants, and with full equations found below in equation (19) and equation (20) respectively, then equation (18) can be simplified to equation (21), also seen in \hyperref[IM:intsliceFs]{IM: intsliceFs}:
\begin{dmath}
