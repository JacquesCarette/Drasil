...
import qualified Data.Drasil.Quantities.Physics as QP (accelerationUC, 
  angularAccel, angularDisplacement, angularVelocity, displacementUC, distanceUC, 
  forceUC, gravitationalAccelUC, gravitationalConstUC, impulseSUC, impulseVUC, 
  linearAccel, linearDisplacement, linearVelocity, momentOfInertiaUC, positionUC, 
  restitutionCoef, timeUC, torqueUC, velocityUC)
...
inputSymbols = map qw [QP.positionUC, QP.velocityUC, QP.forceUC, QM.orientation, 
  QP.angularVelocity, QP.linearVelocity, QP.gravitationalConstUC, QPP.mass, 
  QPP.len, QP.momentOfInertiaUC, QP.torqueUC] ++ [qw QP.restitutionCoef]

outputSymbols = map qw [QP.positionUC, QP.velocityUC, QM.orientation, 
  QP.angularVelocity]


cpUnits :: [UnitalChunk]
cpUnits = [QP.accelerationUC, QP.angularAccel, QP.gravitationalAccelUC, 
  QP.impulseVUC, QP.impulseSUC, iVect, jVect, normalVect, QP.distanceUC, QP.displacementUC, 
  QP.timeUC, QP.angularDisplacement, pos_CM, pos_i, mass_i, mTot, acc_i, vel_i,
...
forceParam n w = ucs'
 (dccWDS ("force" ++ n) (cn $ "force exerted by the " ++ w ++ 
  " body (on another body)") (phrase QP.forceUC)) 
  (sub (eqSymb QP.forceUC) (Atomic n)) Real newton
...
contParam n w = ucs'
 (dccWDS ("r_" ++ n ++ "P") (contdispN n) 
  (phrase QP.displacementUC)) (sub (eqSymb QP.displacementUC)
  (Concat $ [Atomic w, cP])) Real metre

contdispN :: String -> NP
contdispN n = cn $ "displacementUC vector between the centre of mass of rigid body " 
...
mass_i = ucs' (dccWDS "m_j" (compoundPhrase' (QPP.mass ^. term)
                (cn "of the j-th particle")) (phrase QPP.mass)) 
                (sub (eqSymb QPP.mass) lJ) Real kilogram

pos_i = ucs' (dccWDS "p_j" (compoundPhrase' (QP.positionUC ^. term) 
               (cn "vector of the j-th particle")) (phrase QP.positionUC))
               (sub (eqSymb QP.positionUC) lJ) Real metre

acc_i = ucs' (dccWDS "acc_i" (compoundPhrase' (cn "the i-th body's")
               (QP.accelerationUC ^. term)) (phrase QP.accelerationUC))
...
torque_i = ucs' (dccWDS "torque_i" 
               ...
               (phrase QP.torqueUC)) (sub (eqSymb QP.torqueUC) lI) Real torqueU
...
time_c = ucs' (dccWDS "time_c" (cn "denotes the time at collision") 
                (phrase QP.timeUC)) (sub (eqSymb QP.timeUC) lC) Real second

initRelVel = ucs' (dccWDS "v_i^AB" (compoundPhrase'
                 (compoundPhrase' (cn "relative") (QP.velocityUC ^. term))
                 (cn "between rigid bodies of A and B")) (phrase QP.velocityUC))
                 (sup (sub (eqSymb QP.velocityUC) lI) (Concat [cA, cB])) Real velU
...
timeT = ucs' (dccWDS "t" (cn "point in time") (phrase QP.timeUC))
                (eqSymb QP.timeUC) Real second
...
momtInert_k = ucs' (dccWDS "momentOfInertiaUCK" (compoundPhrase'
               ...
               (sub (eqSymb QP.momentOfInertiaUC) lK) Real momtInertU
...
collisionImpulse = ucs' (dccWDS "collisionImp" (compoundPhrase' 
                (cn $ "collision") (QP.impulseSUC ^. term)) (phrase QP.impulseSUC)) 
                (eqSymb QP.impulseSUC) Real impulseU


force_i = ucs' (dccWDS "force_i" (compoundPhrase' 
      (QP.forceUC ^. term) (cn "applied to the i-th body at time t")) 
      (phrase QP.forceUC)) (sub (eqSymb QP.forceUC) lI) Real newton
...
mmntOfInCons   = constrained' QP.momentOfInertiaUC    [nonNegativeConstraint] (dbl 74.5)
gravAccelCons  = constrained' QP.gravitationalConstUC [] (dbl 9.8)
posCons        = constrained' QP.positionUC           [] (dbl 0.412) --FIXME: should be (0.412, 0.502) vector
veloCons       = constrained' QP.velocityUC           [] (dbl 2.51)
...
forceCons      = constrained' QP.forceUC              [] (dbl 98.1)
torqueCons     = constrained' QP.torqueUC             [] (dbl 200)
...
transMotLegTerms = [massIRigidBody, QP.gravitationalAccelUC, timeT, inittime, pos_CM,
  QP.accelerationUC, QP.velocityUC, force_i]

rotMotLegTerms = [massIRigidBody, QP.gravitationalAccelUC, timeT, inittime,
...
col2DLegTerms = [massIRigidBody, momtInert_k, timeT, inittime, time_c, pos_CM,
  QP.velocityUC, QM.orientation, QP.angularVelocity, normalVect, -- +:+. S "Its signed direction is determined by (A4)",
