...
module GOOL.Drasil.LanguageRenderer (
  ...
  enumElementsDocD', multiStateDocD, blockDocD, bodyDocD, oneLinerD, 
  printListDoc, outDoc, prFn, prStrFn, prLnFn, printDoc, printFileDocD, 
  boolTypeDocD, intTypeDocD, floatTypeDocD, charTypeDocD, stringTypeDocD, 
  fileTypeDocD, typeDocD, enumTypeDocD, listTypeDocD, listInnerTypeD, 
  iteratorError, voidDocD, destructorError, paramDocD, paramListDocD, mkParam, 
  methodDocD, methodListDocD, stateVarDocD, constVarDocD, stateVarListDocD, 
  alwaysDel, switchDocD, assignDocD, multiAssignDoc, plusEqualsDocD, 
  plusEqualsDocD', plusPlusDocD, plusPlusDocD', listDecDocD, listDecDefDocD, 
  ...
  switchAsIfD, ifExistsD, forRangeD, tryCatchD, stratDocD, runStrategyD, 
  listSliceD, checkStateD, notifyObserversD, unOpPrec, notOpDocD, notOpDocD', 
  negateOpDocD, sqrtOpDocD, sqrtOpDocD', absOpDocD, absOpDocD', expOpDocD, 
  ...
  atanOpDocD', unOpDocD, unExpr, unExpr', powerPrec, multPrec, andPrec, orPrec, 
  ...
  objVarDocD, inlineIfD, funcAppDocD, newObjDocD, newObjDocD', constDecDefDocD, 
  varD, staticVarD, extVarD, selfD, enumVarD, classVarD, objVarD, objVarSelfD, 
  listVarD, listOfD, valueOfD, argD, enumElementD, argsListD, funcAppD, 
  extFuncAppD, newObjD, notNullD, objAccessD, objMethodCallD, 
  objMethodCallNoParamsD, selfAccessD, listIndexExistsD, indexOfD, funcDocD, 
  ...
  listAppendFuncD, iterBeginError, iterEndError, listAccessFuncD, 
  listAccessFuncD', listSetFuncD, includeD, breakDocD, continueDocD, staticDocD,
  dynamicDocD, bindingError, privateDocD, publicDocD, blockCmtDoc, docCmtDoc, 
  commentedItem, addCommentsDocD, functionDox, classDox, moduleDox, 
  commentedModD, docFuncRepr, valList, valList', valueList, prependToBody, 
...
import GOOL.Drasil.Symantics (Label, Library, RenderSym(..), BodySym(..), 
  BlockSym(..), InternalBlock(..), PermanenceSym(..), InternalType(..), 
  TypeSym(Type, getType, getTypeString, getTypeDoc, bool, float, string, infile,
    outfile, listType, listInnerType, obj, enumType, void), 
...
import GOOL.Drasil.Data (Boolean, Other, Terminator(..), FileData(..), 
  fileD, updateFileMod, ModData(..), updateModDoc, OpData(..), 
  od, ParamData(..), pd, TypeData(..), td, btd, ltd, TypedType(..), cType, 
  typeDoc, TypedValue(..), valPrec, valDoc, Binding(..), TypedVar(..), 
  getVarData, varType, varDoc, typeToVal, typeToVar, valToType)
...
iteratorError :: String -> String
iteratorError l = "Iterators not available in " ++ l
...
listDecDocD :: (RenderSym repr) => repr (Variable repr [a]) -> 
  repr (Value repr Other) -> Doc
listDecDocD v n = space <> equals <+> new <+> getTypeDoc (variableType v) 
  <> parens (valueDoc n)

listDecDefDocD :: (RenderSym repr) => repr (Variable repr [a]) -> 
...
newObjDocD :: (RenderSym repr) => repr (Type repr a) -> Doc -> Doc
...
newObjD :: (RenderSym repr) => (repr (Type repr a) -> Doc -> Doc) -> 
