...
import Data.List (concat, sortBy, transpose, intersperse, isPrefixOf)
...
-- | Compares two symbols and returns their order. See compsy for more information
...
-- | Takes an amount and adds a unit to it:
...
-- | Makes a traceability matrix from a list of row titles, list of rows
...
-- | Takes a list and a reference, generates references to 
...
-- | Returns the Sentence "@aNamedIdea@ Showing the Connections Between @contents@"
...
-- | Returns the Sentence "The @chunk@ under consideration is @definition@"
...
-- | Helper functions for making likely change statements. Outputs "The @firstSentence@ may be @verb@ @thirdSentence@"
...
-- | Similar to `fromSource` but takes a list of references
...
-- | Uses an Either type to check if a string is valid - Left with error message if invalid Char in String, else Right with String
