...
data SomeProgGenerator where
  SomeProgGenerator :: forall repr tp. (OOProg repr tp) =>
...
genPackage :: (OOProg progRepr tp, SoftwareDossierSym packRepr, Monad packRepr) =>
...
genProgram :: (OOProg r tp) => GenState (OO.GSProgram r)
genProgram = do
  g <- get
...
-- of modularity.
chooseModules :: (OOProg r tp) => Modularity -> GenState [OO.SFile r]
...
genUnmodular :: (OOProg r tp) => GenState (OO.SFile r)
genUnmodular = do
  g <- get
...
genModules :: (OOProg r tp) => GenState [OO.SFile r]
genModules = do
  g <- get
  mn     <- genMain
  inp    <- genInputMod
...
-- un-representation functions determine which target language the package will
-- be generated in.
generateCodeProc :: (ProcProg progRepr tp, SoftwareDossierSym packRepr, Monad packRepr) =>
...
-- GOOL's static code analysis interpreter is called to initialize the state
-- used by the language renderer.
genPackageProc :: (ProcProg progRepr tp, SoftwareDossierSym packRepr, Monad packRepr) =>
...
-- | Generates an SCS program based on the problem and the user's design choices.
genProgramProc :: (ProcProg r tp) => GenState (Proc.GSProgram r)
genProgramProc = do
  g <- get
...
-- | Generates either a single module or many modules, based on the users choice
-- of modularity.
chooseModulesProc :: (ProcProg r tp) => Modularity -> GenState [Proc.SFile r]
chooseModulesProc Unmodular = liftS genUnmodularProc
chooseModulesProc Modular = genModulesProc

-- | Generates an entire SCS program as a single module.
genUnmodularProc :: (ProcProg r tp) => GenState (Proc.SFile r)
genUnmodularProc = do
  g <- get
  umDesc <- unmodularDesc
  giName <- genICName GetInput
...
-- | Generates all modules for an SCS program.
genModulesProc :: (ProcProg r tp) => GenState [Proc.SFile r]
genModulesProc = do
  g <- get
  mn     <- genMainProc
  inp    <- genInputModProc
