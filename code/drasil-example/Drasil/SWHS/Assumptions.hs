...
assumpS3 = foldlSent [
  S "The", phrase water `S.in_` S "the", phrase tank,
...
assumpS5 = foldlSent [
  S "The", phrase wDensity `S.and_` phrase pcmDensity,
...
assumpS8 = foldlSent [
  S "The", phrase tempC `S.is` S "constant over", phrase time]
assumpS9 = foldlSent [
...
assumpS11 = foldlSent [
  ...
  S "not" +:+. phrase discharging, S "The", phrase tempW `S.and_`
...
assumpS12 = foldlSent [phrase tempInit `S.the_ofTheC` phrase water `S.andThe`
...
assumpS14 mat = foldlSent [
  ...
  phrase meltPt `S.of_` phrase water `sC` S "or rise above its",
  phrase boilPt]
assumpS15 = foldlSent [
...
assumpS19 = foldlSent [
  ...
  phrase meltPt `S.and_` phrase boilPt `S.are` S (show (0 :: Integer)) :+:
  Sy (unit_symb temp) `S.and_` S (show (100 :: Integer)) :+:
  Sy (unit_symb temp) `sC` S "respectively"]
assumpS20 = foldlSent [
  S "When considering the", phrase wVol `S.in_` S "the",
