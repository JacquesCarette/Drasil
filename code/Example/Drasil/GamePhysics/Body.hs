...
s2_intro = [Paragraph (S "Due to the rising cost of developing video " :+:
    ...
    (chipmunk ^. term) :+: S ", an open source " :+: (twoD ^. term) :+:
    S " " :+: (rigidBody ^. term) :+: S " " :+: (physLib ^. term) :+:
...
s2_1_intro = [Paragraph (S "This document descibes the modeling of an " :+:
    S "open source " :+: (twoD ^. term) :+: S " " :+:
    (rigidBody ^. term) :+: S " " :+: (physLib ^. term) :+:
    S " used for games. The " :+: (sLower (goalStmt ^. defn)) :+:
...
s2_2_intro = Paragraph $ S "The scope of the " :+:
    ...
    S "physical simulation of " :+: (twoD ^. term) :+: S " " :+:
    (rigidBodies ^. term) :+: S " acted on by forces. Given " :+:
    (twoD ^. term) :+: S " " :+: (rigidBodies ^. term) :+:
    S ", " :+: (chipmunk ^. term) :+: S " is intended to simulate how " :+:
    S "these " :+: (rigidBodies ^. term) :+: S " interact with one another."
...
s2_3_intro = [Paragraph (S "The organization of this document follows the " :+:
    ...
    (sLower (inModel ^. defn)) :+: S "s in " :+:
    (makeRef s4_2_5) :+: S " and trace back to any additional information " :+:
    S "they require."),
    Paragraph (S "The " :+: (sLower (goalStmt ^. defn)) :+:
    S "s are refined to the " :+: (sLower (thModel ^. defn)) :+:
    S "s, and the " :+: (sLower (thModel ^. defn)) :+:
    S "s to the " :+: (sLower (inModel ^. defn)) :+: S "s.")]
...
s4_intro = Paragraph $ S "This section first presents the problem " :+:
    ...
    S "specification, which presents the " :+: (sLower
    (assumption ^. defn)) :+: S "s, theories, and definitions that are " :+:
    S "used for the " :+: (physLib ^. term) :+: S"."
...
s4_1_intro = Paragraph $ S "Creating a gaming " :+: (physLib ^. term) :+:
    ...
    (twoD ^. term) :+: S " " :+: (rigidBody ^. term) :+: S " " :+:
    (physLib ^. term) :+: S ", game development will be more accessible " :+:
...
s4_2_1_list = Enumeration (Simple [
    ...
    (rigidBodies ^. term) :+: S " " :+: (collision ^. term) :+:
