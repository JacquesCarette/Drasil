...
import Language.Drasil.Code.Imperative.DrasilState (GenState, DrasilState(..),
  ScopeType(..), genICName, SoftwareDossierInfo(..), softwareDossierInfo)
...
genSampleInput = do
  g <- get
  dd <- genDataDesc
  if hasSampleInput (auxiliaries $ g ^. softwareDossierInfo) then return . Just $ sampleInput
    (printfo g) dd (sampleData $ g ^. softwareDossierInfo) else return Nothing
