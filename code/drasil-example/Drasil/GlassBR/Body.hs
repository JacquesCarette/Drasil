...
import Data.Drasil.Concepts.Documentation as Doc (analysis, appendix, aspect, 
  ...
  likelyChg, mis, model, organization, output_, physicalSystem, physSyst, problem, 
...
import Drasil.GlassBR.Assumptions (assumptionConstants, gbRefDB, gbRefDB', assumptions)
...
mkMIS = IntroSec (IntroMIS (S "https://github.com/smiths/caseStudies/tree/master/CaseStudies/glass")) : 
  ...
  MISModSec (MISModProg "Calc" Nothing [MISUses [inputLabel, contoursLabel, constantsLabel],
    ...
    calcLabel False) :
  MISModSec (MISModProg "GlassType" (Just (mkParagraph (S "from" +:+ makeRef glaTyFac))) --FIXME: link is broken
    [MISUses [],
    MISSyntax [MISExportedCs ([] :: [QDefinition]), MISExportedAPs [{-FILL IN-}]],
    MISSemantics [MISStateVars [{-FILL IN-}], MISStateInvariant [], MISAssumptions [], MISAccessRoutines [{-FILL IN-}]]]
    glTypeLabel True) :
  MISModSec (MISModProg "Thickness" (Just (mkParagraph (S "following" +:+ makeRef hFromt))) --FIXME: link is broken
    [MISUses [],
    MISSyntax [MISExportedCs ([] :: [QDefinition]), MISExportedAPs [{-FILL IN-}]],
    MISSemantics [MISStateVars [{-FILL IN-}], MISStateInvariant [], MISAssumptions [], MISAccessRoutines [{-FILL IN-}]]]
    thicknessLabel True) :
