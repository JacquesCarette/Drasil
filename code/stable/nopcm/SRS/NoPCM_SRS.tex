...
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{FR:Output-Input-Derived-Quantities}:]Outputs and inputs quantities and derived quantities in the following list: the quantities from R\ref{FR:Input-Inital-Values}, the mass from R\ref{FR:Find-Mass} and ${τ_{W}}$ (from IM1).
\end{description}
\begin{description}
...
\toprule
 & IM1 (\hyperref[IM:eBalanceOnWtr]{Definition~IM:eBalanceOnWtr}) & IM2 (\hyperref[IM:heatEInWtr]{Definition~IM:heatEInWtr}) & Data Constraints (Table~\ref{Table:InDataConstraints}) & R1 (R\ref{FR:Input-Inital-Values}) & R2 (R\ref{FR:Find-Mass}) & R3 (R\ref{FR:Check-Inputs-Satisfy-Physical-Constraints}) & R4 (R\ref{FR:Output-Input-Derived-Quantities}) & R5 (R\ref{FR:Calculate-Temperature-Water-Over-Time}) & R6 (R\ref{FR:Calculate-Change-Heat.Energy-Water-Time})
\\
\midrule
...
\\
R4 (R\ref{FR:Output-Input-Derived-Quantities}) &  &  &  & X & X &  &  &  & 
\\
