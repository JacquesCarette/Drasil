...
assumpS3 =
  foldlSent [D.toSent (atStartNP (NP.the (water `inThe` tank))),
  S "is fully mixed" `sC` S "so the", phrase tempW `S.isThe`
  S "same throughout the entire", phrase tank]

assumpS4 =
  foldlSent [D.toSent (atStartNP (the wDensity)), S "has no spatial variation; that is"
...
assumpS5 =
  foldlSent [D.toSent (atStartNP (the htCapW)), S "has no spatial variation; that",
...
assumpS9_npcm =
  foldlSent [D.toSent (atStartNP (the model)), S "only accounts for charging",
  D.toSent (phraseNP (the tank)) `sC` (S "not discharging" !.), D.toSent (atStartNP (the tempW)), S "can only",
