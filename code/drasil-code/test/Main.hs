...
goolTestGroup
  :: String
  -> ( forall r vis stmt mthd stvr attch prg file.
...
gProcTestGroup
  :: String
  ->
    ( forall r vis stmt mthd prg file.
      ( Literal r
      , Comparison r
...
genCodeProcNoMake
  ::
    ( NativeVector r
    , ProcProg r vis stmt mthd ProgData file
    , Monad r'
    )
  => (r ProgData -> ProgData)
  -> (r' PackageData -> PackageData)
  ->
    ( forall s vis' stmt' mthd' prg' file'.
      ( Comparison s
      , NativeVector s
      , DeclStatement s stmt'
      , ProcProg s vis' stmt' mthd' prg' file'
      )
...
genCodeGOOL
  ::
    ( OOProg r vis stmt mthd stvr attch ProgData file
    , GetSet r
    , StrategyPattern r stmt
    , ObserverPattern r stmt
  ...
  => (r ProgData -> ProgData)
  -> (r' PackageData -> PackageData)
  -> ( forall s vis' stmt' mthd' stvr' attch' prg' file'.
       ( OOProg s vis' stmt' mthd' stvr' attch' prg' file'
       , GetSet s
...
genCodeProc
  :: (ProcProg r vis stmt mthd ProgData file, SoftwareDossierSym r', Monad r')
  => (r ProgData -> ProgData)
  -> (r' PackageData -> PackageData)
  -> (forall s vis' stmt' mthd' prg' file'. (ProcProg s vis' stmt' mthd' prg' file') => Proc.GSProgram s prg')
  -> [FileLayout]
