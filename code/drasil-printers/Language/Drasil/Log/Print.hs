...
renderUsedUIDs chs = header (text "UIDs" $$ nest 40 (text "Associated Chunks")) $$ vcat (map renderUsedUID $ coerce chs)
  where
...
mkTableDepChunks PI{_ckdb = db} = text "Dependent Chunks (the chunks on the left use the chunks on the right in some capacity)" <> colon
  ...
  $$ vcat (map (testIndepLayout . coerce) traceMapUIDs)
  where
    testIndepLayout :: (String, [String]) -> Doc
...
mkTableReferencedChunks PI{_ckdb = db} = text "Referenced Chunks (other chunks build from these)" <> colon
  ...
  $$ vcat (map (testIsolateLayout . coerce) refbyUIDs)
  where
    testIsolateLayout :: (String, [String]) -> Doc
...
mkTableDepReffedChunks PI{_ckdb = db} = text "Dependent and Referenced Chunks (chunks dependent on middle UIDs and used in the chunks on the right)" <> colon
  ...
  $$ vcat (map (traceRefLayout . coerce) $ Map.assocs combinedMaps)
  where
    traceRefLayout :: (String, ([String], [String])) -> Doc
