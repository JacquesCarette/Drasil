...
uses the expression and specification languages as building blocks for 
capturing different properties. For example, a defining equation like that seen 
in Figure~\ref{Fig:GlassBRDDSRS} will be captured within a chunk using the 
expression language, whereas the English-language term referring to a chunk 
will be captured using the specification language. See Section~\ref{S:DrasilKC} 
for more of the implementation details.

The document layout language
- Give an example here of how it can be used (like the following) to show what 
NOT to do.
\begin{listing}
myDocument :: Document
...
GOOL~\cite{Costabile2012} is ... We use a modified version to facilitate 
our code generation. -Has its own printing DSLs and ASTs for a number of langs.

The printing translation language and printers are very closely related. They 
are
