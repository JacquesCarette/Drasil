...
\eztodo{Discuss what parts are necessary for a chunk to be considered ``valid.''
...
\texttt{package.yaml} files. \todo{Quickly explain what the ``existential
quantification'' language feature is.}

So, now we can re-create \Chunk{} using \ExistentialQuantification{} to hide the
type parameter:

\eztodo{So, this ExistentialQuantification is a bit more important, and I should
...
\tedioustodo{Connect these ideas here better.}
...
\eztodo{I just need an example above, its kind of boring to write though. It
