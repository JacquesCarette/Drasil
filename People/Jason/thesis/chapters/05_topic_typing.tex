...
conjunction, and if-then-else (ternary operators), we might write the syntax
...
integers. The restriction is only there for simplification. \(\tau\) is an
enumeration of all permissible \textit{types} of \textit{terms} we can have in
\(\bb{L}\).

Next, we need to add the typing rules to restrict our syntax to only those
valid. We will do so using inference judgments, as follows:
...
\intodo{However, this is done by hand... shall we automate it? :)}
