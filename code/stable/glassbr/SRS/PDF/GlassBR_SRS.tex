...
\label{Sec:Intro}
Explosion in downtown areas are dangerous from the blast itself and also potentially from the secondary effect of falling glass. Therefore, software is needed to predict whether a glass slab can withstand a blast under given conditions. For example, we might wish to know whether a pane of glass from a gas main explosion from a small fertilizer truck bomb. The program documented here is called GlassBR.
