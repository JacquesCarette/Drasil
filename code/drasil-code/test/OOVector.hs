...
ooVector :: OOProg r tp vis smt => GSProgram r
ooVector = prog "OOVector" "" [fileDoc (buildModule "OOVector" []
  [main] [vectorClass])]

vectorClass :: OOProg r tp vis smt => SClass r
...
vecVar :: (VariableSym r tp, OOTypeSym r tp) => String -> SVariable r
vecVar v = var v (obj "Vector")

localV :: VariableSym r tp => SVariable r
localV = var "v" (arrayType double)

thisV :: (SelfSym r tp, VariableValue r tp) => SVariable r
thisV = instanceVarSelf localV

dimension :: OOProg r tp vis smt => SMethod r
...
magnitude :: OOProg r tp vis smt => SMethod r
...
norm :: OOProg r tp vis smt => SMethod r
...
dot :: OOProg r tp vis smt => SMethod r
...
add :: OOProg r tp vis smt => SMethod r
...
scale :: OOProg r tp vis smt => SMethod r
