...
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{FR:Calculate-Change-Heat.Energy-Water-Time}:]Calculate and output the change in heat energy in the water (${E_{W}}$($t$)) over the simulation time (from IM3).
\end{description}
...
\toprule
 & IM1 (\hyperref[IM:eBalanceOnWtr]{Definition~IM:eBalanceOnWtr}) & IM2 (\hyperref[IM:heatEInWtr]{Definition~IM:heatEInWtr}) & Data Constraints (Table~\ref{Table:InDataConstraints}) & R1 (R\ref{FR:Input-Inital-Values}) & R2 (R\ref{FR:Find-Mass}) & R3 (R\ref{FR:Check-Inputs-Satisfy-Physical-Constraints}) & R4 (R\ref{FR:Output-Input-Derivied-Quantities}) & R5 (R\ref{FR:Calculate-Temperature-Water-Over-Time}) & R6 (R\ref{FR:Calculate-Change-Heat.Energy-Water-Time})
\\
\midrule
...
\\
R6 (R\ref{FR:Calculate-Change-Heat.Energy-Water-Time}) &  & X &  &  &  &  &  &  & 
\\
\bottomrule
\caption{Traceability Matrix Showing the Connections Between Requirements and Instance Models}
