...
*/
func func_t_flight(_ inParams: inout InputParameters, _ g: Float) throws -> Float {
    var outfile: FileHandle
    do {
        outfile = try FileHandle(forWritingTo: FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!.appendingPathComponent("log.txt"))
        try outfile.seekToEnd()
    ...
    do {
        try outfile.write(contentsOf: Data("  g = ".utf8))
    } catch {
        throw "Error printing to file."
    }
    do {
        try outfile.write(contentsOf: Data(String(g).utf8))
        try outfile.write(contentsOf: Data("\n".utf8))
    } catch {
        throw "Error printing to file."
    }
...
*/
func func_p_land(_ inParams: inout InputParameters, _ g: Float) throws -> Float {
    var outfile: FileHandle
    do {
        outfile = try FileHandle(forWritingTo: FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!.appendingPathComponent("log.txt"))
        try outfile.seekToEnd()
...
*/
func func_s(_ inParams: inout InputParameters, _ epsilon: Float, _ d_offset: Float) throws -> String {
    var outfile: FileHandle
    do {
        outfile = try FileHandle(forWritingTo: FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!.appendingPathComponent("log.txt"))
        try outfile.seekToEnd()
    ...
    do {
        try outfile.write(contentsOf: Data("  epsilon = ".utf8))
    } catch {
        throw "Error printing to file."
    }
    do {
        try outfile.write(contentsOf: Data(String(epsilon).utf8))
    } catch {
        throw "Error printing to file."
    }
    do {
...
do {
    try outfile.write(contentsOf: Data("var 'g' assigned ".utf8))
} catch {
    throw "Error printing to file."
}
do {
...
do {
    try outfile.write(contentsOf: Data("var 'epsilon' assigned ".utf8))
} catch {
    throw "Error printing to file."
}
do {
...
var t_flight: Float = try func_t_flight(&inParams, g)
do {
    outfile = try FileHandle(forWritingTo: FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!.appendingPathComponent("log.txt"))
    try outfile.seekToEnd()
} catch {
...
var p_land: Float = try func_p_land(&inParams, g)
do {
    outfile = try FileHandle(forWritingTo: FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!.appendingPathComponent("log.txt"))
    try outfile.seekToEnd()
} catch {
...
var s: String = try func_s(&inParams, epsilon, d_offset)
do {
    outfile = try FileHandle(forWritingTo: FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!.appendingPathComponent("log.txt"))
    try outfile.seekToEnd()
} catch {
