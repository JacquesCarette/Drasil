...
acceleration           = uc CP.acceleration           (Concat [vec lA, label "(", lT, label ")"]) (vectNDS "n" Real)   accelU
...
displacement           = uc CP.displacement           (vec lU)                                    (vectNDS "n" Real)         metre
distance               = uc CP.distance               lD                                          Real        metre
energy                 = uc CP.energy                 cE                                          Real        joule
force                  = uc CP.force                  (vec cF)                                    (vectNDS "n" Real)         newton
frequency              = uc CP.frequency              lF                                          Real        hertz
gravitationalAccel     = uc CP.gravitationalAccel     (vec lG)                                    (vectNDS "n" Real)         accelU
...
impulseV               = uc CP.impulseV               (vec cJ)                                    (vectNDS "n" Real)        impulseU
...
momentOfInertia        = uc CP.momentOfInertia        (vec cI)                                    (vectNDS "n" Real)         momtInertU
chgMomentum            = uc CP.chgMomentum            (Concat [cDelta,vec cP])                    Real        impulseU
momentum               = uc CP.momentum               (vec cP)                                    (vectNDS "n" Real)         impulseU
moment                 = uc CP.moment                 (vec cM)                                    (vectNDS "n" Real)         torqueU
...
positionVec            = uc CP.positionVec            (vec lR)                                    (vectNDS "n" Real)         metre
...
tension                = uc CP.tension                (vec cT)                                    (vectNDS "n" Real)         newton
time                   = uc CP.time                   lT                                          Real        second
torque                 = uc CP.torque                 (vec lTau)                                  (vectNDS "n" Real)         torqueU
velocity               = uc CP.velocity               (Concat [vec lV, label "(", lT, label ")"]) (vectNDS "n" Real)        velU
