...
import Drasil.System (HasSystemMeta(..), ToFiles(..), projName, projTitleS)
...
data JupyterGenOptions = JupyterGenOptions {
  ...
  titleComb :: Sentence -> Sentence -> Sentence,
...
instance ToFiles LessonPlan JupyterGenOptions where
  ...
  toFiles plan JupyterGenOptions{..} = files
    where
      ...
      as = foldlList Comma List $ S . fullName <$> plan ^. authors
