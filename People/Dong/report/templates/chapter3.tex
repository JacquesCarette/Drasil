...
\end{flalign}
\myequations{System of First-Order ODE for PDContoller}
...
% Another output of the calculation module for solving the ODE is to output the solution as a function $\mathbb{R} \rightarrow \mathbb{R}^m$. The i is the number of equations in the ODE. The input is the independent variable, often time, and the output is a sequence of real numbers. In Example~\ref{ex_firstorderode}, the function type will be $\mathbb{R} \rightarrow \mathbb{R}^2$. The idea of outputting an ODE as a function can be useful when the Drasil framework generates a library. Users have the option to generate a runnable program or a standalone library. 
...
%     \item $\mathbb{R} \rightarrow \mathbb{R}^m$ (i is the number of equations in the ODE)
%     \end{itemize}  \\ \hline
%     ACM-Java &     
%     \begin{itemize}[wide]
%     \item $\mathbb{R}^k$
