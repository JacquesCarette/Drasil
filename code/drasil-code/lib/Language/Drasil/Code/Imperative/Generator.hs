...
data SomeProgGenerator where
  SomeProgGenerator
    :: forall repr vis stmt mthd stvr attch mod file prg. (OOProg repr vis stmt mthd stvr attch mod file prg)
...
genPackage
  ::
    (OOProg progRepr vis stmt mthd stvr attch mod file prg
...
genUnmodular
  :: (OOProg r vis stmt mthd stvr attch mod file prg) => GenState (FS (r file))
genUnmodular = do
  g <- get
...
generateCodeProc
  ::
    ...
    , ProcProg progRepr vis stmt mthd mod file prg
    , SoftwareDossierSym packRepr
    , Monad packRepr
    )
  => Lang
...
-- | Generates an SCS program based on the problem and the user's design choices.
genProgramProc
  :: (NativeVector r, ProcProg r vis stmt mthd mod file prg)
