...
renderParam :: (OORenderSym r TypeData vis, UnRepr r TypeData) =>
  r (Variable r) -> Doc
renderParam v = renderType (variableType v) <+> variable v

renderMethod :: (OORenderSym r tp vis, UnRepr r TypeData) => String ->
...
renderListDec :: (CommonRenderSym r TypeData vis, UnRepr r TypeData) =>
  r (Variable r) -> r (Value r) -> Doc
renderListDec v n = space <> equals <+> new' <+> renderType (variableType v)
  <> parens (value n)

renderConstDecDef :: (CommonRenderSym r TypeData vis, UnRepr r TypeData) =>
  r (Variable r) -> r (Value r) -> Doc
