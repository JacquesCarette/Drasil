...
cartSysRDesc = atStartNP (the cartesian) `S.is` S "right-handed" +:+ S "where" +:+ 
    phraseNP (combineNINP positive (xAxis `and_` yAxis)) +:+. S "point right up"

yAxisDirDesc :: Sentence
yAxisDirDesc = atStartNP (direction `the_ofThe` yAxis) `S.is` S "directed opposite" +:+ S "to" +:+. phrase gravity
