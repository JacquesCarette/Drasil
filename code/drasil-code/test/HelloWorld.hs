...
helloWorldOO :: (OOProg r vis smt mthd svr att prg) => OO.GSProgram r prg
...
-- | Creates the HelloWorld program and necessary files.
helloWorldProc :: (ProcProg r vis smt mthd prg) => GProc.GSProgram r prg
...
myOtherList :: (SharedProg r vis smt mthd) => SVariable r
...
helloWorldMainOO :: (OOProg r vis smt mthd svr att prg) => MS (r mthd)
...
-- | Main function. Initializes variables and combines all the helper functions defined below.
helloWorldMainProc :: (ProcProg r vis smt mthd prg) => MS (r mthd)
...
helloInitVariables :: (SharedProg r vis smt mthd) => MS (r Block)
helloInitVariables = block [comment "Initializing variables",
  ...
  arrayDec 3 (litInt 0) (var "arr2" (arrayType int)) mainFn,
  ...
  printLn $ arrayLength (valueOf $ var "arr2" (arrayType int)),
  listDecDef myOtherList mainFn [litDouble 1.0,
    litDouble 1.5],
  arrayDec 3 (litInt 0) (var "arr_copy" (arrayType int)) mainFn,
...
objectTests :: (OOProg r vis smt mthd svr att prg) => MS (r Block)
...
mySlicedList, mySlicedList2, mySlicedList3, mySlicedList4, mySlicedList5,
  ...
  mySlicedList10, mySlicedList11 :: (SharedProg r vis smt mthd) => SVariable r
...
listSliceTests :: (SharedProg r vis smt mthd) => [MS (r Block)]
listSliceTests = [
...
helloIfBody :: (SharedProg r vis smt mthd) => MS (r Body)
...
helloElseBody :: (SharedProg r vis smt mthd) => MS (r Body)
helloElseBody = bodyStatements [printLn (arg 5)]

-- | If-else statement checking if a list is empty.
helloIfExists :: (SharedProg r vis smt mthd) => MS (r smt)
...
helloSwitch :: (SharedProg r vis smt mthd) => MS (r smt)
...
helloTryCatch :: (SharedProg r vis smt mthd) => MS (r smt)
helloTryCatch = tryCatch (oneLiner (throw "Good-bye!"))
  (oneLiner (printStrLn "Caught intentional error"))

helloWorldClass :: (OOProg r vis smt mthd svr att prg) => CS (r Class)
