...
Notes & $a$, $b$ are dimensions of the plate, where ($a\geq{}b$).
        ...
        $LDF$ is the load duration factor as defined in DD3.
...
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{System-Set-Values-Following-Assumptions}:]The system shall set the known values as follows: $m$, $k$, $E$, ${t_{d}}$ following A4, $LDF$ (from \hyperref[DD:loadDurFactor]{Definition~DD:loadDurFactor}), LSF following A5, $h$ (from \hyperref[DD:min.thick]{Definition~DD:min.thick}), $GTF$ (from \hyperref[DD:gTF]{Definition~DD:gTF}), $SD$ (from \hyperref[DD:standOffDist]{Definition~DD:standOffDist}), and $AR$ (from \hyperref[DD:aspect.ratio]{Definition~DD:aspect.ratio}).
\end{description}
\begin{description}
