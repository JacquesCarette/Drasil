...
software should be a ``clear matter of principled engineering'' \todo{cite
    github README?}. However, with the existing programming methodology, it is not
...
Perhaps oversimplifying\todo{Not really?}, but, the essence of this ideology
...
\intodo{The ideology also vaguely relates to ``picking programming languages for
    projects''. This is a silly question under this ideology. There is no 1
    single language that we even use to describe everything, so how can we
    possibly write all kinds of software for all kinds of knowledge?}
