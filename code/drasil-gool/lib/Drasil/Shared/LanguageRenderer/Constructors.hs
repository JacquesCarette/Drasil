...
unExpr :: (CommonRenderSym r tp) => VSUnOp r -> SValue r -> SValue r
...
mkUnExpr :: (CommonRenderSym r tp) => (Doc -> Doc -> Doc) -> r OpData ->
  r (Value r) -> SValue r
...
unExprNumDbl :: (CommonRenderSym r TypeData, TypeElim r TypeData) => VSUnOp r ->
  SValue r -> SValue r
unExprNumDbl u' v' = do
  u <- u'
  v <- v'
...
unExprCastFloat :: (CommonRenderSym r TypeData, TypeElim r TypeData) =>
...
typeUnExpr :: (CommonRenderSym r tp) => VSUnOp r -> VS (r tp) -> SValue r -> SValue r
typeUnExpr u' t' s' = do
  u <- u'
  t <- t'
  s <- s'
...
binExpr :: (CommonRenderSym r TypeData, TypeElim r TypeData) =>
...
-- values v and w
binExpr' :: (CommonRenderSym r TypeData, TypeElim r TypeData) => VSBinOp r ->
...
binExprNumDbl' :: (CommonRenderSym r TypeData, TypeElim r TypeData) =>
  VSBinOp r -> SValue r -> SValue r -> SValue r
...
binExprCastFloat :: (CommonRenderSym r TypeData, TypeElim r TypeData) =>
...
-- resulting expression. The type of the result is passed as a parameter.
typeBinExpr :: (CommonRenderSym r tp) => VSBinOp r -> VS (r tp) -> SValue r ->
  SValue r -> SValue r
...
numType :: (CommonRenderSym r TypeData, TypeElim r TypeData) =>
...
mkExpr :: (CommonRenderSym r tp) => Int -> r tp -> Doc -> SValue r
mkExpr p t = valFromData (Just p) Nothing (toState t)

binOpDocDRend :: (CommonRenderSym r tp) => r OpData -> r (Value r) ->
  r (Value r) -> Doc
...
exprParensL :: (CommonRenderSym r tp) => r OpData -> r (Value r) -> Doc
...
exprParensR :: (CommonRenderSym r tp) => r OpData -> r (Value r) -> Doc
...
binExprRender :: (CommonRenderSym r tp) =>  r OpData -> r (Value r) ->
  r (Value r) -> Doc
