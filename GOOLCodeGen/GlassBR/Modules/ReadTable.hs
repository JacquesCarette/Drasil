...
read_z_array_func = pubMethod methodTypeVoid "read_z_array" [p_filename] 
    [ 
      block [
        ...
        closeFile v_infile,
        varDecDef l_z_array_str (listT string) $ v_line$.(stringSplit ",")$.(listSlice (Just $ litInt 1) Nothing (Just $ litInt 2)),
...
read_x_array_func = pubMethod methodTypeVoid "read_x_array" [p_filename] 
    [ 
      block [
        ...
        closeFile v_infile,
        v_lines &= v_lines$.(listSlice (Just $ litInt 1) Nothing Nothing),
        listDec' l_x_array_str (listT string) 0,
        for (varDecDef l_i int (litInt 0)) (v_i ?< v_lines$.listSize) ((&++) v_i)
          (oneLiner $ valStmt $ v_x_array_str$.(listAppend $ v_lines$.(listAccess v_i)$.(stringSplit ",")$.(listSlice (Just $ litInt 0) Nothing (Just $ litInt 2)))),
...
read_y_array_func = pubMethod methodTypeVoid "read_y_array" [p_filename] 
    [ 
      block [
        ...
        for (varDecDef l_i int (litInt 0)) (v_i ?< v_lines$.listSize) ((&++) v_i)
          (oneLiner $ valStmt $ v_y_array_str$.(listAppend $ v_lines$.(listAccess v_i)$.(stringSplit ",")$.(listSlice (Just $ litInt 1) Nothing (Just $ litInt 2)))),
