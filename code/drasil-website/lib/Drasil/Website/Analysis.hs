...
graphTableEntry knd path pkg = [S pkg, namedRef (getGraphsInTableRef knd "" path pkg) (S "Dot Graph"), namedRef (getGraphsInTableRef knd "circo_" path pkg) (S "Circo Graph")]
...
tableOfTypeGraphsDesc = S "The following" +:+ namedRef tableTypeGraphRef (S "Table of Type Graphs") +:+ S "is another artifact generated by Drasil. \
  \These graphs explore the dependency of data types upon each other. For more convenient viewing, the graphs have been generated using both the dot \
  \algorithm (left side) and the circo algorithm (right side). These graphs include record-defined types, newtype wrappers, \
...
-- | Creates a table that links to all generated type graphs.
mkTypeGraphTable :: FilePath -> [String] -> Contents
mkTypeGraphTable typePath pkgs = LlC $ llcc tableTypeGraphRef $ Table 
  [S "Drasil Package", S "Generated using Dot algorithm", S "Generated using Circo algorithm"] -- Header row
  (graphTable "datatype" typePath pkgs) -- Create the body of the table
  (S "Type Graphs") True -- Label the table

-- | Type graph table reference.
tableTypeGraphRef :: Reference
tableTypeGraphRef = makeTabRef "TypeGraphs"

-- ** Table of Class-Instance Graphs
...
-- | Creates a table that links to all generated class-instance graphs.
mkClassInstTable :: FilePath -> [String] -> Contents
mkClassInstTable clsInstPath pkgs = LlC $ llcc classIGraphRef $ Table 
  [S "Drasil Package", S "Generated using Dot algorithm", S "Generated using Circo algorithm"] -- Header row
  (graphTable "classInst" clsInstPath pkgs) -- Create the body of the table
  (S "Class Instance Graphs") True -- Label the table

-- | Class-instance table reference.
classIGraphRef :: Reference
