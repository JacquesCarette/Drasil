...
specLength (Sp _)      = 1
specLength (Ref Internal r _ _) = length r
specLength (Ref Cite2    r i n) = length r specLength i + specLength n
specLength (Ref External _ t _) = specLength t
specLength EmptyS      = 0
...
spec (Ref Internal r sn _) = snref r (spec sn)
spec (Ref Cite2    r i n)  = cite r (spec i) (info n)
  where
    info EmptyS = Nothing
    info x      = Just (spec x)
spec (Ref External r sn _) = externalref r (spec sn)
spec EmptyS              = empty
