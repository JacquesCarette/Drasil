...
\improvement{Perhaps a graph of all $35$ classes as a picture would be a
nice illustration?}
...
To demonstrate a case where a language feature must be rendered very 
differently in each target language, consider for-each loops. The opening line 
of a for-each loop in Python looks like:
\begin{lstlisting}
for age in ages:
\end{lstlisting}
in Java:
...
for (std::vector<int>::iterator age \
  ...
  age++) {
\end{lstlisting}
There are differences in how each language renders for-each loops, especially 
when comparing \Cplusplus to the others. But these differences are easily 
handled by GOOL, by defining the custom rendering behaviour of each language in 
its definition for \verb|forEach|.
...
\verb|listAccess| \verb|(valueOf ages)| \verb|(litInt 1)| will generate
...
line breaks to that the code fits in this paper's narrow column margins.
