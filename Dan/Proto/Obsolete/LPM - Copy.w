...
@ DD\ref{L-hc} in the SRS gives the heat transfer coefficient ($h_{c}$) as: 
\begin{equation}
h_{c} =\frac{2k_{c}h_{b}}{2k_{c}+\tau_{c}h_{b}}, \label{eq:hc}
\end{equation}
...
@ DD\ref{L-hg} in the SRS gives the gap conductance ($h_{g}$) as:
\begin{equation}
