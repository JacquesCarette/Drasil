...
observer :: (OOProg r tp vis smt par fun) => SFile r
...
helperClass :: (ClassSym r tp vis smt par, IOStatement r tp smt, Literal r tp,
...
observerConstructor :: (OOMethodSym r tp vis smt par, Literal r tp) => SMethod r
observerConstructor = initializer [] [(x, litInt 5)]

-- | Create the @printNum@ method.
printNumMethod
  :: (OOMethodSym r tp vis smt par, IOStatement r tp smt, OOVariableValue r tp)
  => SMethod r
