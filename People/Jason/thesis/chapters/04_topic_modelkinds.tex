...
\intodo{Re-write the below point-form notes.}

\begin{itemize}

	\item In generating families of software artifacts via \acs{gool} that solve
	      scientific problems, we need for the generated expressions to all have
	      definite values\todo{I need some sort of citation for this.}.

	\item At the moment, \Expr{} is the language we use to discuss all the
	      things we've shown interest in exploring through our case studies.

	\item Of course, \Expr{} contains some things without definite values. For
	      example,
	      \begin{itemize}

		      \item \intodo{Go through ModelExpr and add them here.}

	      \end{itemize}

	\item The information is already there, it's just not yet \textit{exposed}.
	...
	\item There are many ways we can expose this information, but we've chosen
	      ...
	      split off into \ModelExpr{} (\refCurrentModelExprHaskell{}), a deeply
	      ...
	      without the language terms related to ``code''\todo{e.g., \ldots{}}.

	\item In order to alleviate the stress involved with writing out the same
	      expression in 2 different languages, we use a \ACF{ttf} encoding of
	      the two (2) languages (\refCurrentExprTTFHaskell{} and
	      \refCurrentModelExprTTFHaskell{}).
	...
	\item Finally, we end up with 3 languages: \Expr{}
	      (\refCurrentExprHaskell{}), \ModelExpr{}
	      (\refCurrentModelExprHaskell{}), and \CodeExpr{}
	      (\todo{refCurentCodeExprHaskell}).

\end{itemize}


\intodo{Discuss how the mathematical language needed to be split up into 3 variants.}
