...
modern compilers have a single internal IR,
...
complete list, as well as some reasoning behind each requirement.
...
\expressivity~captures the idea that GOOL is intended to be a language for
expressing (the ideas contained in) OO programs.  Having it as a requirement
reminds us to test it against real-world examples of OO programs.

The last requirement, that language commonalities (\common) be abstracted, is
an internal requirement: we noticed a lot of repeated code in our initial
...
We work to abstract from concrete OO programs, not just to meet our
...
may be the case in the future that generating idiomatic Java may use it);
...
feasible but in fact easy to provide template-like, and even aspects of
...
communication; they must be valid, executable progams, but machines, but
...
\common ality between target languages. This genericity
...
would be around $600$ lines of code. Comparing the number of lines of code of 
each of our renderers to $600$ therefore gives an indication of the specificity 
of that renderer. The \Cplusplus{} source code renderer is our largest, with 
...
\Csharp{}'s, with $691$ lines of code, meaning \Csharp{} has the most in common 
...
Below is a complete example of a GOOL-defined function:
\begin{lstlisting}
