...
\begin{abstract}
  This paper presents GOOL, a Generic Object-Oriented Language.  GOOL shows the
  feasibility of devising a language that, with the right abstractions, captures
  the essense of mainstream object oriented languages.  To demonstrate that GOOL
  captures the conceptual meaning of OO programs, we show that programs
  expressed in GOOL can automatically be translated to OO code (in multiple
  languages) that is human-readable, idiomatic (to the target language), and
  well documented.  Moreover, GOOL can be used to express common patterns, from
  ...
  mixture of input, output and in-out parameters, and finally on to design
  patterns, like the Observer, State and Strategy patterns.  GOOL is implemented
  as a DSL embedded in Haskell that can currently generate code in Python, Java,
  \Csharp, and \Cplusplus.  The GOOL syntax consists of an imperative core, OO
  features and a means to express modules, programs, auxiliary files (like
  Makefiles) and packages.  The GOOL code generator is implemented in the
...
\improvement{Agreed. Hopefully there will be time to do this}
...
code. \change{We need to fill in the section reference.}
...
is a measure of delete priority. \unsure{What is delete priority?}  \verb|constVar| can be used for constant state
...
at a particular position exists.  \unsure{Should we show some code here?  There
  doesn't seem to be any information other than saying we can do command line
  arguments.  Maybe we could show one of the convenience functions?}
...
\unsure{Any chance we could show an example of one of these?  I didn't even know
  we could do this.  SS}
...
generate well-formed code, but not to insure that it is well-typed.
