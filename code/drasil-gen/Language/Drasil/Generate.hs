...
import Language.Drasil.Printers (Format(TeX, HTML), DocSpec(DocSpec), 
  ...
  genTeX, PrintingInformation(..), HaveNotationSetting(..))
import Language.Drasil.Code (generator, generateCode, Choices, CodeSpec)

-- | Generate a number of artifacts based on a list of recipes.
gen :: (HasSymbolTable s, HaveNotationSetting s) => DocSpec -> Document -> s -> IO ()
gen ds fn sm = prnt sm ds fn

-- | Generate the output artifacts (TeX+Makefile or HTML)
prnt :: (HasSymbolTable s, HaveNotationSetting s) => s -> DocSpec -> Document -> IO ()
...
prntDoc :: (HasSymbolTable s, HaveNotationSetting s) => DocSpec -> Document -> s -> IO ()
...
prntDoc' :: (HasSymbolTable s, Show a, HaveNotationSetting s) => a
...
writeDoc :: (HasSymbolTable s, HaveNotationSetting s) => s -> Format -> Filename -> Document -> Doc
