...
module Drasil.GProc (Label, GSProgram, File, Body, Block, FS, MS, VS, SVariable,
  SValue, Module, NamedArgs, SharedProg, ProcProg, ProgramSym(..), FileSym(..),
...
import Drasil.Shared.InterfaceCommon (Label, Body, Block, SVariable, SValue,
  NamedArgs, SharedProg, BodySym(..), bodyStatements, oneLiner, BlockSym(..),
  TypeSym(..), BinderSym(..), StatementSym(..), AssignStatement(..), (&=),
