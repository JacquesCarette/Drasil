...
The majority of the aforementioned case studies were developed at McMaster 
University to address real engineering problems outside of our research group. 
Table~\ref{tab:case-studies} provides a high-level reference to each case 
study. For the specifics of each system, all relevant case study artifacts can 
...
\begin{tikzpicture}
\node[inner sep=0] 
(img){\fbox{\includegraphics[width=\textwidth]{figures/gb_SRS_ToS.png}}};
...
\draw[domainbox] (0.01,0.01) rectangle (0.99,0.62);
\node[annotlabel] at (0.95,0.57) {P};
\end{scope}
\end{tikzpicture}
...
\ref{fig:gbrtos}, and~\ref{fig:gbrtoa}) we 
...
unique to that one program; rather, it is domain knowledge that the SRS uses to 
derive the system-specific models, constraints, and calculations presented 
...
does have minor variations. For an easy visual comparison, the earlier \gb{} 
example in Figure~\ref{fig:gbrtos} can be compared directly with 
Figure~\ref{fig:gptos} (\gp{}): in both figures, `B' highlights repeated 
...
systems is greater than simply a difference in choice of symbols, and so there 
is problem-specific knowledge being encoded. While we can intuitively infer 
this conclusion based solely on each system addressing a different problem, our 
...
\begin{tikzpicture}
\node[inner sep=0] 
(img){\fbox{\includegraphics[width=\textwidth]{figures/gp_SRS_ToS.png}}};
...
\draw[domainbox] (0.01,0.01) rectangle (0.99,0.49);
...
problem-specific knowledge that will not be useful across 
...
Non-Factored Load ($NFL$), in the SRS calculation of Capacity ($LR$) through 
intermediary quantities such as $NFL$, in the MG, and in the source code 
(Figure~\ref{fig:gbrsrccalc}, lines 11-14).
\end{itemize}
...
(see example in Figure~\ref{fig:swhstm1} which is the same in both SRS 
documents). However, when we follow the derivations from the TMs to the 
Instance Models (IMs), we find the resulting equations have changed due to the 
context of the system; the lack of PCM has changed the relevant equations for 
calculating the energy balance on water in the tank as shown in 
Figure~\ref{fig:swhsnopcmim1}. In that comparison, the overall similarity of 
the two IMs already shows that most of the content is shared. The only 
...
\centering
\begin{tikzpicture}
\node[inner sep=0] (img) {\fbox{\includegraphics[width=0.8\textwidth]{figures/swhs_IM1.png}}};
...
may include the identity projection (i.e. the full, unabridged definition) as 
