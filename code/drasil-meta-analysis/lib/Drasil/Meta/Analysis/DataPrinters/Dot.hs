...
subgraph handle nm nds edgs = do
    ...
    mapM_ (hPutStrLn handle) $ concatMap (\ns -> makeNodesSub (fst ns) <$> snd ns) nds
...
makeEdgesDi nm = fmap (\c -> replaceInvalidChars nm <> " -> " <> replaceInvalidChars c <> ";")

-- Creates an edge between a type and its dependency (indented for subgraphs)
makeEdgesSub :: String -> [String] -> [String]
makeEdgesSub nm = fmap (\c -> "\t\t" <> replaceInvalidChars nm <> " -> " <> replaceInvalidChars c <> ";")
...
replaceInvalidChars = fmap (\x -> if x `elem` invalidChars then '_' else x)
  where
    invalidChars = "[]!} (){->,$'"
