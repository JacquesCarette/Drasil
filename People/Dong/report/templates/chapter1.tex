...
$y_t$ is the dependent variable and it is Process Variable. $y_t$(t) is a function of independent variable, often time. $y_t'$(t) is the first order derivative of $y_t$(t). 
$y_t''$(t) is the second order derivative of $y_t$(t). $K_d$ , $K_p$, and $r_t$ are constant variables. $K_d$ is Derivative Gain, $K_p$ is Proportional Gain, and $r_t$ is Set-Point. We can write this equation as follows. 
...
For new types, the \verb|LHS|, the short name for the left-hand side, is a list of \verb|Term|. This corresponds to the left hand side is a collection of terms. Each \verb|Term| has an \verb|Expr| and \verb|Unknown|. This corresponds to a term consists of a coefficient and a derivative of the function. Although \verb|_unk| is integers, combining \verb|_unk| and \verb|_depVar| we can get the derivative of the function. For new operators, they are inspired by the linear equation~\ref{eq_linearDE}. The \verb|$^^| operator take a variable and a integer, and it represent the derivative of the function. For example, in example~\ref{eq_odeexmaple}, we can write $y_t(t)$\^{}\^{}2 to represent $y_t(t)''$. Although the function is provide by \verb|_depVar| and the first parameter in \verb|$^^| will be ignored, the dummy variable maintain the whole input structure as close as a linear ODE.

The \verb|$^^| operator take a variable and a integer, and return a integer (Unknown is a synonym of Integer).

connects a variable and the order of its derivative. 


We call it a dummy dependent variable because it is a placeholder to increase users' readability. 

The \verb|$*| operator creates a term by combining a coefficient matrix and an unknown variable. Last, the \verb|$+| operator connects all terms. Let's write pseudo code (Code \ref{code_exinputl}) for the example matrix form~\ref{eq_odeexmaple} in the newly introduced input language. The full detail of the input language for the PDController example will show up in ~\ref{code_scexinputl}.
