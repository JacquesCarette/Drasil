...
import Data.Drasil.Concepts.Thermodynamics (melting, boil_pt, temp, thermal_energy,
  heat_cap_spec)
import Control.Lens ((^.))
...
htWtrDesc = foldlSent [S "The above", phrase equation, S "is derived using" +:+. acroT 2, 
  (getS w_E) `isThe` S "change in", phrase thermal_energy, S "of the", phrase liquid,
...
htPCMDesc = foldlSent [S "The above", phrase equation, S "is derived using" +:+.
  (acroT 2 `sAnd` acroT 3), (getS pcm_E) `isThe` phrase change, S "in",
  phrase thermal_energy, S "of the", short phsChgMtrl, S "relative to the", phrase energy,
  S "at the", (phrase temp_init), sParen (getS temp_init) +:+. (unwrap $ getUnit pcm_initMltE), (getS pcm_E), 
  S "for the", phrase solid, short phsChgMtrl, S "is found using", acroT 2, S "for sensible heating, with", S "specific heat capacity" `ofThe`
