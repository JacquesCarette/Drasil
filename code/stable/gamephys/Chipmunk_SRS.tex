...
\\ \midrule \\
Equation & ${\mathbf{a}_{i}}=\frac{d{\mathbf{v}_{i}}\left(t\right)}{dt}=g+\frac{\mathbf{F}\left(t\right)}{{m_{i}}}$
\\ \midrule \\
...
\\ \midrule \\
Equation & $\mathbf{v}\left({t_{c}}\right)=\mathbf{v}\left(t\right)+\frac{j}{m}\mathbf{n}$
\\ \midrule \\
...
Units & m
\\ \midrule \\
Equation & ${\mathbf{p}_{CM}}$ = $\frac{\displaystyle\sum{\left[{m_{i}}{\mathbf{p}_{i}}\right]}}{M}$
\\ \midrule \\
Description & ${\mathbf{p}_{CM}}$ is the the mass-weighted average position of a rigid body's particles (m)\newline${m_{i}}$ is the mass of the i-th particle (kg)\newline${\mathbf{p}_{i}}$ is the position vector of the i-th particle (m)\newline$M$ is the total mass of the rigid body (kg)
\\ \bottomrule \end{tabular}
\end{minipage}\\
~\newline
\noindent \begin{minipage}{\textwidth}
...
\\
\midrule
$L$ & $L\geq{}0.0$ & $44.2$ m & 0.1
\\
$m$ & $m\geq{}0.0$ & $56.2$ kg & 0.1
\\
$\mathbf{I}$ & $\mathbf{I}\geq{}0.0$ & $74.5$ kg$\text{m}^{2}$ & 0.1
\\
...
\\
${C_{R}}$ & ${C_{R}}\geq{}0.0$ and ${C_{R}}\leq{}1.0$ & $0.8$ & 0.1
\\
\bottomrule
...
\item[R4:]Verify that the inputs satisfy the required physical constraints from Table~\ref{Table:InpuDataCons}
...
@article{Wilson_Aruliah_Titus_Chue_Hong_Davis_Guy_Haddock_Huff_Mitchell_Plumblet_Waugh_White_Wilson_2013,
...
@article{Parnas_Clements_Wiess_1984,
