...
knowledge. Drasil currently focuses on \ACF{scs} conforming to a specific
\ACF{srs} template\cite{SmithAndLai2005}.
...
relationships with other things (discussed in \Cref{chap:modelkinds}). The
solution builds on a prototype by Dr. Jacques Carette\todo{Cite Dr. Carettes
ModelKinds prototype.} that facilitates structured theories. In continuing the
...
(discussed in \Cref{chap:modelkinds}). Continuing, this works resolves the issue
of validity by creating a system of typing rules for the expression language
(\Cref{chap:typedExpr}). Finally, to enable capturing data with type parameters
...
\Cref{chap:storingChunks} focuses on how knowledge is stored in Drasil, and how
