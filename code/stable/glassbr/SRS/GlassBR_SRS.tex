...
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{empty}:]The standard E1300-09a for calculation applies only to monolithic, laminated, or insulating glass constructions of rectangular shape with continuous lateral support along one, two, three, or four edges. This practice assumes that: (1) the supported glass edges for two, three and four-sided support conditions are simply supported and free to slip in plane; (2) glass supported on two sides acts as a simply supported beam; and (3) glass supported on one side acts as a cantilever.
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{empty}:]Following \cite{astm2009} (pg. 1), this practice does not apply to any form of wired, patterned, etched, sandblasted, drilled, notched, or grooved glass with surface and edge treatments that alter the glass strength.
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{empty}:]This system only considers the external explosion scenario for its calculations.
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{empty}:]The values provided in Section~\ref{Sec:AuxConstants} are assumed for the duration of load (${t_{d}}$), and the material properties of $m$, $k$, and $E$. [IM1, DD3, DD5, DD7, DD9]
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{empty}:]Glass under consideration is assumed to be a single lite; hence, the value of LSF is equal to 1 for all calculations in GlassBR. [IM2, DD7]
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{empty}:]Boundary conditions for the glass slab are assumed to be 4-sided support for calculations. [IM1]
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{empty}:]The response type considered in GlassBR is flexural. [IM1]
\end{description}
\begin{description}
\item[\refstepcounter{assumpnum}\atheassumpnum\label{empty}:]With reference to A\ref{A:standardValuesA}, the value of load duration factor ($LDF$) is a constant in GlassBR. [DD3]
\end{description}
...
\\
${P_{btol}}$ & $0<{P_{btol}}<1$ & -- & $8\cdot{}10^{-3}$ & 0.1\%
\\
$SD$ & $SD>0$ & ${SD_{min}}\leq{}SD\leq{}{SD_{max}}$ & $45$ m & 10.0\%
\\
$TNT$ & $TNT>0$ & -- & ${1}$ & 10.0\%
\\
$w$ & $w>0$ & ${w_{min}}\leq{}w\leq{}{w_{max}}$ & $42$ kg & 10.0\%
\\
\bottomrule
...
\label{Sec:FRs}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{Input-Glass-Props}:]Input the quantities from Table~\ref{Table:R1ReqInputs}, which define the glass dimensions, type of glass, tolerable probability of failure and the characteristics of the blast Note: $a$ and $b$ will be input in terms of millimetres and will be converted to the equivalent value in metres.
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{System-Set-Values-Following-Assumptions}:]The system shall set the known values as follows: $m$, $k$, $E$, ${t_{d}}$ following A4, $LDF$ following A8, LSF following A5, $h$ (from \hyperref[DD:min.thick]{Definition~DD:min.thick}), $GTF$ (from \hyperref[DD:gTF]{Definition~DD:gTF}), and $SD$ (from \hyperref[DD:standOffDist]{Definition~DD:standOffDist}).
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{Check-Input-with-Data\_Constraints}:]The system shall check the entered input values to ensure that they do not exceed the data constraints mentioned in Section~\ref{Sec:DataConstraints}. If any of the input parameters is out of bounds, an error message is displayed and the calculations stop.
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{Output-Values-and-Known-Quantities}:]Output the input quantities from R1 and the known quantities from R2.
\end{description}
\begin{description}
\item[\refstepcounter{reqnum}\rthereqnum\label{Check-Glass-Safety}:]If $is\_safePb$ ∧ $is\_safeLR$ (from \hyperref[T:safetyReqPb]{Definition~T:safetyReqPb} and \hyperref[T:safetyReqLR]{Definition~T:safetyReqLR}) are true, output the message ``For the given input parameters, the glass is considered safe.'' If the condition is false, then output the message ``For the given input parameters, the glass is NOT considered safe.''
\end{description}
\begin{itemize}
...
\label{Sec:LCs}
\begin{description}
\item[\refstepcounter{lcnum}\lcthelcnum\label{Calculate-Internal-Blask-Risk}:]A\ref{A:explsnScenarioA} - The system currently only calculates for external blast risk. In the future, calculations can be added for the internal blast risk.
\end{description}
\begin{description}
\item[\refstepcounter{lcnum}\lcthelcnum\label{Variable-Values-of-m,k,E}:]A\ref{A:standardValuesA}, A\ref{A:ldfConstantA} - Currently, the values for $m$, $k$, and $E$ are assumed to be the same for all glass. In the future, these values can be changed to variable inputs.
\end{description}
\begin{description}
\item[\refstepcounter{lcnum}\lcthelcnum\label{Accomodate-More-than-Single-Lite}:]A\ref{A:glassLiteA} - The software may be changed to accommodate more than a single lite.
\end{description}
\begin{description}
\item[\refstepcounter{lcnum}\lcthelcnum\label{Accomodate-More-Boundary-Conditions}:]A\ref{A:bndryConditionsA} - The software may be changed to accommodate more boundary conditions than 4-sided support.
\end{description}
\begin{description}
\item[\refstepcounter{lcnum}\lcthelcnum\label{Consider-More-than-Flexure-Glass}:]A\ref{A:responseTyA} - The software may be changed to consider more than just flexure of the glass.
\end{description}
\section{Unlikely Changes}
\label{Sec:UCs}
\begin{description}
\item[\refstepcounter{ucnum}\uctheucnum\label{Predict-Withstanding-of-Certain-Degree}:]The goal of the system is to predict whether the glass slab under consideration can withstand an explosion of a certain degree.
\end{description}
\begin{description}
\item[\refstepcounter{ucnum}\uctheucnum\label{Accommodate-Altered-Glass}:]A\ref{A:glassConditionA} requires that the glass is not altered in any way. Therefore, this cannot be used on altered glass.
\end{description}
...
\\
\midrule
${AR_{max}}$ & maximum aspect ratio & ${5}$ & --
\\
${d_{max}}$ & maximum value for one of the dimensions of the glass plate & ${5}$ & m
\\
${d_{min}}$ & minimum value for one of the dimensions of the glass plate & $100\cdot{}10^{-3}$ & m
\\
$E$ & modulus of elasticity of glass & $71.7\cdot{}10^{6}$ & Pa
\\
...
\\
${SD_{max}}$ & maximum stand off distance permissible for input & $130$ & m
\\
${SD_{min}}$ & minimum stand off distance permissible for input & ${6}$ & m
\\
${t_{d}}$ & duration of load & ${3}$ & s
\\
${w_{max}}$ & maximum permissible input charge weight & $910$ & kg
\\
