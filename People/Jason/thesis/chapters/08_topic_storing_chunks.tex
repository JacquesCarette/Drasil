...
Additionally, all chunks also have a specific \textit{type} they belong to. For
example, \refOriginalQuantityDictExampleHaskell{} is a \QuantityDict{}
...
chunks, they are ``typed'' with a single fully monomorphic type signature.
Commonly, we create chunks to discuss relations between other chunks. As such,
chunks commonly also refer to other instances of chunks, but, instead of storing
whole constituent chunks within them, we store \UID{} references to those
constituent chunks. In doing this, we ensure that all references to that chunk
refer to the same piece of immutable data.
...
\Chunk{}s are currently an informational void. So, this results in a type error
(\refChunkRetrieverError{}). Haskell is unable to coerce the rigid type \(a\)
into the \QuantityDict{} type.
...
simpler\footnote{We can also wrap up the constraints together for easier use by
enabling ``constraint kinds'' language extension \cite{GHC2020ConstraintKinds},
and bundling the constraints together (e.g., \refProtoIsChunkHaskell{}).}. If
we're interested, we can add a ``dump to X'' mechanism, where $X$ is any
encoding of our choice (such as \acs{json}), similarly. This is omitted for
...
memory for a frequent and expensive search operation: \refProtoChunkDBHaskell{}.
...
repository, but can only be merged into the stable branch once we resolve the
\acs{uid} conflicts.
...
In \refPseudoChunkNestingHaskell{}\footnote{The examples do not exist in Drasil,
but they represent how chunks are ``gradually built.''}, chunks rely on a common
data type for functionality related to displaying text related to the name,
short name, and abbreviation of things. The current system assumes that the
chunks share a \acs{uid}. In the previous generation of \ChunkDB{}, this is
allowed because there was no \UID{} uniqueness checking across types. However,
the new \ChunkDB{}s require \UID{} uniqueness across chunk types, leading to
