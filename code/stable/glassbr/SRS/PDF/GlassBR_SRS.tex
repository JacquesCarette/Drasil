...
\item[standardValues:\phantomsection\label{assumpSV}]{The values provided in \hyperref[Sec:AuxConstants]{Sec:Values of Auxiliary Constants} are assumed for the duration of load ($t_{\text{d}}$), and the material properties of $m$, $k$, and $E$. (RefBy: \hyperref[IM:sdfTol]{IM:sdfTol}, \hyperref[IM:nFL]{IM:nFL}, \hyperref[IM:dimlessLoad]{IM:dimlessLoad}, \hyperref[DD:loadDurFactor]{DD:loadDurFactor}, \hyperref[varValsOfmkE]{LC:Variable-Values-of-m,k,E}, and \hyperref[assumpLDFC]{A:ldfConstant}.)}
...
\item[ldfConstant:\phantomsection\label{assumpLDFC}]{With reference to \hyperref[assumpSV]{A:standardValues}, the value of load duration factor ($\mathit{LDF}$) is a constant in GlassBR. (RefBy: \hyperref[DD:loadDurFactor]{DD:loadDurFactor} and \hyperref[varValsOfmkE]{LC:Variable-Values-of-m,k,E}.)}
\end{description}
