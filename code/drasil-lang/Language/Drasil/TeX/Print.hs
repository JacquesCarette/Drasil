...
lo (Paragraph contents _) _  = toText $ spec contents
lo (EqnBlock contents _)  _  = makeEquation contents
lo (Table _ rows r bl t) _  = toText $ makeTable rows (spec r) bl (spec t)
lo (Definition _ ssPs l) sm  = toText $ makeDefn sm ssPs $ spec l
lo (List l _)               _  = toText $ makeList l
...
lo (Bib bib _)            sm = toText $ makeBib sm bib
