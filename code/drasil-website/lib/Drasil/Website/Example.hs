...
getSRSRef path format ex = makeURI refUID (getSRSPath path format ex) $ shortname' $ S refUID
  where
    refUID = show format ++ "Ref" ++ (ex ^. projHRName)
...
getSRSPath path format ex = path ++ (ex ^. projRepoName) ++ "/SRS/" ++ show format ++ "/" ++ sufx format
  where
    sufx MDBook  = "book"
    sufx HTML    = (ex ^. projHRName) ++ "_SRS.html"
    sufx TeX     = (ex ^. projHRName) ++ "_SRS.pdf"
    sufx Jupyter = (ex ^. projHRName) ++ "_SRS.html"
