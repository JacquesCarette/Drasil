...
import Language.Drasil hiding (Ref, CodeIdea(..), Code)
import Language.Drasil.Chunk.Code (CodeIdea(..), Code)
...
data Param = PC {_pcc :: Code
                         , passBy :: PassBy}
makeLenses ''Param

-- | Finds the 'UID' of the 'Code' used to make the 'Param'.
instance HasUID      Param where uid = pcc . uid
-- | Finds the term ('NP') of the 'Code' used to make the 'Param'.
instance NamedIdea   Param where term = pcc . term
-- | Finds the idea contained in the 'Code' used to make the 'Param'.
instance Idea        Param where getA = getA . view pcc
-- | Finds the 'Space' of the 'Code' used to make the 'Param'.
instance HasSpace    Param where typ = pcc . typ
-- | Finds the 'Stage' dependent 'Symbol' of the 'Code' used to make the 'Param'.
instance HasSymbol   Param where symbol = symbol . view pcc
-- | 'Param's have a 'Quantity'.
instance Quantity    Param
-- | Finds the code name and 'Code' of a 'Param'.
...
-- | Finds the units of the 'Code' used to make the 'Param'.
