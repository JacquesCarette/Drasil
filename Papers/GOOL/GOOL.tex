...
The last two requirement, that language commonalities (\common) be abstracted,
...
successful code generation.  \improvement{example?}
...
\improvement{Should show examples of language differences, where semantically
things are the same, but render quite differently. Rather like the 
sin example in the next section, but at the language level}
...
in each language tends to very quite a lot, so we need to provide a single
