...
flowchart TD
    IdeaDict
    CC[ConceptChunk]
    CC-->DQD[DefinedQuantityDict]
    DQD-->QDefinition
    DQD-->ConstrConcept
    ConstrConcept-->UncertQ
    DQD-->MultiDefn
    QDefinition-->DataDefition
...
- `implVarAU' :: UID -> NP -> Sentence -> Maybe String -> Space -> Symbol -> Maybe UnitDefn -> DefinedQuantityDict`
  - Internal CC not inserted into CDB

Question:
- Should a DQD *always* have a ConceptChunk that has been inserted into the database?
