...
module GOOL.Drasil (Label, GSProgram, SFile, MSBody, MSBlock, VSType, 
  ...
  VectorStatement(vectorized, vectorizedScale, vectorizedAdd), setVectorized,
  ...
  libNewObj, exists, objMethodCallMixedArgs, FunctionSym(..), ($.), selfAccess,
  GetSet(..), List(..),  listSlice, listIndexExists, at, StatePattern(..),
...
import GOOL.Drasil.ClassInterface (Label, GSProgram, SFile, MSBody, MSBlock, 
  ...
  ControlStatement(..), switchAsIf, ifNoElse, VariableSym(..),
  VariableElim(..), ($->), listOf, listVar, ValueSym(..), Argument(..),
  Literal(..), MathConstant(..), VariableValue(..), CommandLineArgs(..),
  NumericExpression(..), BooleanExpression(..), Comparison(..),
  VectorExpression(..), ValueExpression(..), funcApp, funcAppNamedArgs,
