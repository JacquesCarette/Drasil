...
s1, s1_1,  s1_2, s1_3, s2, s2_1, s2_2, s2_3, s3, s3_1, s3_2, s4, s4_1, s4_2,
  ...
  s6_2_3, s6_2_4, s6_2_5, s7, s7_1, s8, s9, s10 :: Section
s1_intro, s1_1_intro, s1_1_table, s1_2_intro, s1_2_table, s1_3_table, s2_intro,
  ...
  s7_intro, s7_list1, s7_table, s7_list2, s7_1_intro, s8_list, s9_list, s10_intro, 
...
s2_3_intro1 = Paragraph $
  ...
  S "any additional information they require."

s2_3_intro2 = Paragraph $
  S "The goal statements are refined to the theoretical models, and theoretical models to " :+: 
  S "the instance models. The data definition are used to support the definitions of the " :+: 
  S "different models." 
...
s5_2_bullets = Enumeration $ Bullet $ map Flat
  ...
  (S " Use Case 2 Glass-BR outputs the glass slab will be safe by comparing whether capacity > " :+:
    ...
    S "If both conditions return true then its shown that the glass slab is safe to use, else if " :+:
    S "both returns false then the glass slab is considered unsafe. All the supporting calculated " :+:
...
s6_2 = Section 1 (S "Solution Characteristics Specification") 
  [Con s6_2_intro, Sub s6_2_1, Sub s6_2_2, Sub s6_2_3, Sub s6_2_4]
...
s6_2_1_list = Enumeration $ Simple $ map (\(a,b) -> (a, Flat b)) [
  ...
  [(S "A4", Nested (S "Standard values used for calculation in Glass-BR are: ") 
    (Simple $ map (\(a,b) -> (a, Flat b)) [
    (S "(a)", (U $ sflawParamM ^. symbol) :+: S " = 7 " :+: Sy (sflawParamM ^. unit)), 
    (S "(b)", (U $ sflawParamK ^. symbol) :+: S " = 2.86 * 10^(-53) " :+: Sy (sflawParamK ^. unit)), 
    (S "(c)", (U $ mod_elas ^. symbol) :+: S " = 7.17 * 10^7 " :+: Sy (mod_elas ^. unit)),
    (S "(d)", (U $ load_dur ^. symbol) :+: S " = 3 " :+: Sy (load_dur ^. unit))]))] ++
  map (\(a,b) -> (a, Flat b)) [
...
s7 = Section 0 (S "Fuctional Requirements") [Con s7_intro, Con s7_list1, Con s7_table, Con s7_list2, Sub s7_1]
...
s7_list1 = Enumeration $ Simple $ map (\(a,b) -> (a, Flat b))
...
s7_table = Table [S "Symbol", S "Units", S "Description"] (mkTable
  [(\ch -> U (ch ^. symbol)),  
   (\ch -> Sy $ ch ^. unit),
   (\ch -> ch ^. descr)
   ]
...
s7_list2 = Enumeration $ Simple $
  ...
  [(S "R6", Nested (S "Output the following quantities:")
    (Bullet $ map (\c -> Flat $ (c ^. descr) :+: S " (" :+: (U $ c ^. symbol) :+: S ")") 
      [prob_br,lRe,demand,act_thick] ++
      [Flat $ (loadDF ^. descr) :+: S " (" :+: (U $ loadDF ^. symbol) :+: S ")"] ++
      [Flat $ (sdf ^. descr) :+: S " (" :+: (U $ sdf ^. symbol) :+: S ")"] ++
      [Flat $ (nonFL ^. descr) :+: S " (" :+: (U $ nonFL ^. symbol) :+: S ")"] ++
      map (\c -> Flat $ (c ^. descr) :+: S " (" :+: (U $ c ^. symbol) :+: S ")") [gTF,dimlessLoad,tolLoad,sdf_tol,ar]))]

s7_1 = Section 1 (S "Nonfunctional Requirements") [Con s7_1_intro]
