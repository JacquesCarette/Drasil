...
import Language.Drasil.Chunk.Concept (Conception, cw)
...
-- Contains a 'Conception', a 'Symbol' dependent on 'Stage', a 'Space', and maybe a 'UnitDefn'.
...
data DefinedQuantityDict = DQD { _con :: Conception
                               , _symb :: Stage -> Symbol
                               , _spa :: Space
...
-- | Finds the 'UID' of the 'Conception' used to make the 'DefinedQuantityDict'.
instance HasUID        DefinedQuantityDict where uid = con . uid
-- | Equal if 'UID's are equal.
instance Eq            DefinedQuantityDict where a == b = (a ^. uid) == (b ^. uid)
-- | Finds the term ('NP') of the 'Conception' used to make the 'DefinedQuantityDict'.
instance NamedIdea     DefinedQuantityDict where term = con . term
-- | Finds the idea contained in the 'Conception' used to make the 'DefinedQuantityDict'.
instance Idea          DefinedQuantityDict where getA = getA . view con
-- | Finds the definition contained in the 'Conception' used to make the 'DefinedQuantityDict'.
instance Definition    DefinedQuantityDict where defn = con . defn
-- | Finds the domain of the 'Conception' used to make the 'DefinedQuantityDict'.
...
-- | Smart constructor that creates a DefinedQuantityDict with a 'Conception', a 'Symbol' independent of 'Stage', a 'Space', and a unit.
dqd :: (IsUnit u) => Conception -> Symbol -> Space -> u -> DefinedQuantityDict
dqd c s sp = DQD c (const s) sp . Just . unitWrapper

-- | Similar to 'dqd', but without any units.
dqdNoUnit :: Conception -> Symbol -> Space -> DefinedQuantityDict
dqdNoUnit c s sp = DQD c (const s) sp Nothing

-- | Similar to 'dqd', but the 'Symbol' is now dependent on the 'Stage'.
dqd' :: Conception -> (Stage -> Symbol) -> Space -> Maybe UnitDefn -> DefinedQuantityDict
dqd' = DQD
...
dqdQd :: (Quantity c, MayHaveUnit c) => c -> Conception -> DefinedQuantityDict
