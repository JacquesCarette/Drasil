...
instance RuleTransformer CodeHarness where
  makeRule (Ch b r s m dc d) = maybe [mkRule ["Generated by Drasil " ++ version dc,"Project Name: " ++ progName m,
    ...
    in [
    mkRule ["Generated by Drasil " ++ version dc, "Project Name: " ++ progName m, progPurpAdd m]
    buildTarget [outnm] [],
...
makeBuild :: Maybe DocConfig -> Maybe BuildConfig -> Maybe Runnable ->
  GOOLState -> ProgData -> DrasilConfig -> Doc
makeBuild d b r s p dc = genMake [Ch {
  buildConfig = b,
  runnable = r,
  goolState = s,
  progData = p,
