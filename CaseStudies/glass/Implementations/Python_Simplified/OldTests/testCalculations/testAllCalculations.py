...
class TestCalculations(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.numTests = length(input)
        self.inputFileName    = [row[0] for row in input]
        self.val1             = [row[1] for row in input]
        self.val2             = [row[2] for row in input]
        self.q_hat_tol_val    = [row[3] for row in input]
        self.lrShouldBe       = [row[4] for row in input]
        self.nflShouldBe      = [row[5] for row in input]
        self.lrVal            = [row[6] for row in input]
        self.qVal             = [row[7] for row in input]
        self.is_safe1ShouldBe = [row[8] for row in input]
        self.is_safe2ShouldBe = [row[9] for row in input]
        self.params           = 
          [inputFormat.get_input(os.path.join("Test/Inputfiles", self.inputFileName), param.Param()) for i in range(self.numTests)]
    ...
    def test_calc_pb(self):
        ...
        for i in range(self.numTests):
            with self.subTest(i=i):
                print(i)
                pb = calculations.calc_pb(self.val1[i], self.params)
                #self.assertNotEqual(pb, self.val2[i])
               
    def test_calc_lr(self):
        for i in range(self.numTests):
            with self.subTest(i=i):
                nfl = calculations.calc_nfl(self.q_hat_tol_val[i], self.params)
    ...
    def test_is_safe(self):
        for i in range(self.numTests):
            with self.subTest(i=i):
                is_safe1 = calculations.is_safe1(self.val2[i], self.params)
