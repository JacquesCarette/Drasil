...
\documentclass[sigplan,review,prologue,dvipsnames]{acmart}
...
\begin{abstract}
  We present GOOL, a Generic Object-Oriented Language.  GOOL demonstrates that a
  language, with the right abstractions, can capture the essence of
  object-oriented programs.  GOOL programs can be used to generate
  human-readable, documented and idiomatic source code in multiple languages.
  With GOOL we can express common programming idioms and patterns, from simple
  library-level functions, to simple tasks (command-line arguments, list
  processing, printing), to more complex patterns, such as methods with a
  mixture of input, output and in-out parameters, and finally Design Patterns
  (such as Observer, State and Strategy).  GOOL is an embedded DSL in Haskell
  that generates code in Python, Java, \Csharp, and \Cplusplus.
\end{abstract}
...
%
%Basic types in GOOL are \verb|bool|, \verb|int|, \verb|float|, \verb|char|, 
%and 
%\verb|string|. \verb|infile| and \verb|outfile| are types for files in read 
%and 
...
%``ages'' that is a list of integers.  GOOL offers shortcuts for defining 
%common 
...
analysis passes. By using \emph{type families}, each instance of the classes 
can choose different underlying data structures for GOOL's types, for when a 
language needs to store more or less information than others. For example the 
...
\begin{comment}
For example, these functions allow easy 
...
\begin{comment}
such as this one for \verb|Variable|s:
\begin{lstlisting}
