...
acroNumGen (frst:rst) num = (f frst) : acroNumGen rst (num + 1)
  where f (Assumption a) = Assumption $ ac (a ^. id) (assuming a) (S $ extrctStrng (short assumption) ++  (show num))
        ...
        f (LikelyChange lch) = LikelyChange $ chc' lch (S $ extrctStrng (short likelyChg) ++ (show num))
