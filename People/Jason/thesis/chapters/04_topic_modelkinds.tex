...
as \refOriginalLandPosRCtoQD{}, where \inlineHs{E.landPosExpr} is a variable
...
\ModelKinds{} \footnote{\ModelKinds{} is based on a prototype by Dr. Jacques
    ...
    the \acs{srs} template \cite{DrasilIssue2599RenamingModels}.}\latertodo{Citation
    for the footnote + ModelKinds?} (\refOriginalNewModelKindsHaskell{}) is created
...
Now, with \EquationalModel{}, we may finally rebuild \inlineHs{landPosIM} using
the improved structure (\refOriginalLandPosIMQD{}). At the moment, it does not
...
bringing some of the information back into the core ``theories'' (e.g.,
\acsp{im}, \acsp{gd}, and \acsp{tm}) to use in the intended flow of knowledge,
should be possible \cite{Chen2022MEng}. Drasil also currently has many more
kinds of theories, not all usable in code generation \textemdash{} more than
just equational models! However, we won't focus on them \textit{quite yet},
we'll get back to this \Cref{chap:more-theory-kinds}.
...
\begin{enumerate}[label={(\alph*)}]
    ...
    \item and gain information about theory capabilities (based on type
          information).
\end{enumerate}
...
alternative printing form of the theory. However, now, we might, for examlpe,
