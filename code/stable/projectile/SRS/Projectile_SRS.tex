...
\item[accelXZero:\phantomsection\label{accelXZero}]The acceleration in the x-direction is zero. \hyperref[IM:calOfLandingDist]{IM: calOfLandingDist}.
...
\\ \midrule \\
Equation & \begin{displaymath}
           d'=\frac{2 {v^{i}}^{2} \sin\left(θ\right) \cos\left(θ\right)}{g}
           \end{displaymath}
\\ \midrule \\
Description & \begin{symbDescription}
...
\\ \bottomrule \end{tabular}
\end{minipage}
Detailed derivation of launch distance:
...
{p_{x}}={{v_{x}}^{i}} t
\end{displaymath}
To find the vertical distance travelled, we want to find the ${p_{x}}$ value ($d'$) at launch duration (from \hyperref[IM:calOfLandingTime]{IM: calOfLandingTime}):
\begin{displaymath}
d'=\frac{{{v_{x}}^{i}}\cdot{}2 {v^{i}} \sin\left(θ\right)}{g}
\end{displaymath}
From \hyperref[DD:speedIX]{DD: speedIX} we can replace ${{v_{x}}^{i}}$:
\begin{displaymath}
d'=\frac{{v^{i}} \cos\left(θ\right)\cdot{}2 {v^{i}} \sin\left(θ\right)}{g}
\end{displaymath}
