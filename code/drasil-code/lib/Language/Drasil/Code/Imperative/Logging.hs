...
logBody :: (SharedProg r tp) => Label -> [SVariable r] -> [MSBlock r] ->
  GenState (MSBody r)
logBody n vars b = do
  g <- get
  return $ body $
...
loggedMethod :: (SharedProg r tp) => FilePath -> Label -> [SVariable r] -> MSBlock r
