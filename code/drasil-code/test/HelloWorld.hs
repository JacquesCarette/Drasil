...
helloWorldOO :: (OOProg r tp vis smt par) => OO.GSProgram r
...
-- | Creates the HelloWorld program and necessary files.
helloWorldProc :: (ProcProg r tp vis smt par) => GProc.GSProgram r
...
myOtherList :: (SharedProg r tp vis smt par) => SVariable r
...
helloWorldMainOO :: (OOProg r tp vis smt par) => SMethod r
...
-- | Main function. Initializes variables and combines all the helper functions defined below.
helloWorldMainProc :: (ProcProg r tp vis smt par) => SMethod r
...
helloInitVariables :: (SharedProg r tp vis smt par) => MSBlock r
...
objectTests :: (OOProg r tp vis smt par) => MSBlock r
...
mySlicedList, mySlicedList2, mySlicedList3, mySlicedList4, mySlicedList5,
  ...
  mySlicedList10, mySlicedList11 :: (SharedProg r tp vis smt par) => SVariable r
...
listSliceTests :: (SharedProg r tp vis smt par) => [MSBlock r]
listSliceTests = [
...
helloIfBody :: (SharedProg r tp vis smt par) => MSBody r
...
helloElseBody :: (SharedProg r tp vis smt par) => MSBody r
helloElseBody = bodyStatements [printLn (arg 5)]

-- | If-else statement checking if a list is empty.
helloIfExists :: (SharedProg r tp vis smt par) => MS (r smt)
...
helloSwitch :: (SharedProg r tp vis smt par) => MS (r smt)
...
helloTryCatch :: (SharedProg r tp vis smt par) => MS (r smt)
helloTryCatch = tryCatch (oneLiner (throw "Good-bye!"))
  (oneLiner (printStrLn "Caught intentional error"))

helloWorldClass :: (OOProg r tp vis smt par) => SClass r
