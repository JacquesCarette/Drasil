...
\subparagraph*{Program Task} Compute the probability that a particular pane of
...
\subparagraph*{Base Domain Knowledge}
...
\subparagraph*{Characteristics of a good solution}
For example, as one of our outputs is a probability, which means that that
quantity should be checked to be between \(0\) and \(1\). This can result
...
\subparagraph*{Basic Examples}
For the purposes of testing, it is always good to have very simple examples,
especially ones where the correct answer is known a priori, even the simple
examples are considered ``toy problems''. They provide a useful extra check
that the narrative is coherent with out expectations.

\subparagraph*{Specialization of theories}
In general, the theories involved will be quite a bit more general than what is
...
\subparagraph*{Coherent narrative}
...
\subparagraph*{Code-level choices}
From a \emph{deterministic model of the solution}. From there, it should be
...
\end{lstlisting}
  \caption{Code-level choices for GlassBR.}
  \label{fig:CodeChoices}
\end{figure}

\subparagraph*{Recipe to generate artifacts}
...
Finally we can put all of these things together and 
...
% TODO
% - future work on "more ontology"
% - link with Eelco's work

Others~\cite{bjorner2021domeng} have already remarked that for most software,
...
In other words, if we capture the fundamental
domain knowledge of specific domains (such as mechanics of rigid body motion, dynamics
of soil, trains, etc), most later development time can be spent on the
\emph{specifics} of the requirements of a specialized application that may
well contain novel ideas at the refinement or recipe level.

In Drasil, as a side-effect of organizing things as we have,
we obtain traceability and consistency, by construction.
