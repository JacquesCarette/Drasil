...
\begin{enumerate}
	...
	\item \textbf{Theory Models}: A ``theory presentation'' \todo{Cite ``Theory
		      Presentations.''}: an abstract theory, highly general and
	...
	\item \textbf{General Definitions}: A partially refined\todo{Discuss what
		      refinement means here.} theory, typically bearing a stronger relation
...
This transcription relies on smart constructors, such as \todo{Reference a new
...
(\todo{Reference the definitions of DDs, IMs, GDs, and TMs}), we may observe
...
\begin{enumerate}
	...
	\item Drasil is limited to using theories with expressions written in a very
	      ...
	      \begin{itemize}
		      ...
		      \item The conversion relied on the \relToQD{} hack.
			  
			 		 \intodo{Add code snippet of original relToQD and discuss
			            	how it wasn't a sustainable solution for the
			            	long-term. relToQD was exactly the implicit
			            	knowledge I mentioned, it assumes, without
			            	statically checking consistency, that the
			            	Relations/Exprs provided as input for code
			            	generation were all of the form: \(y =
			            	f(\ldots{})\). See
			            	\url{https://github.com/JacquesCarette/Drasil/issues/628}
			            	for more information about state of relToQD w.r.t.
			            	RCs.}
...
\MultiDefn{} (\refCurrentMultiDefnHaskell{}), and is intended for forming
...
Drasil\todo{Cite Dongs work.}, \NewDEModel{} is the replacement for \DEModel{},
...
\intodo{ModelKinds already has some success, shown in Dongs work, where he added
	a lot more code generation capabilities through ModelKinds. We can now also
	create an updated copy of the "Table 3.3: Drasil Case Studies Artifacts
	Generated".}
...
\intodo{Note the ``residual issues'' in the ``Future Work'' chapter.}
