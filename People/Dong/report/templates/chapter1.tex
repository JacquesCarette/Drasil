...
Before we conducting this research, the Drasil framework can generate software that provide numerical solutions for a first-order ODE by explicitly writing the equation. We re-write the ODE equation in a text based form, and pass it to the Drasil code generator. In Equation~\ref{eq_nopcmorginal}, it is the model describes the energy balance on water in \href{https://jacquescarette.github.io/Drasil/examples/nopcm/SRS/srs/NoPCM_SRS.html#Sec:IMs}{NOPCM case study}, and we can find the temperature of the water base on it.

\begin{equation} \label{eq_nopcmorginal}
	T_{w}'(t) +  \frac {T_{w}(t)}{\tau_{w}} = \frac{T_{c}}{\tau_{w}}
\end{equation}

The $T_w(t)$ is a function of the independent variable, in this case time. The $T_w$ is the temperature of water ($ ^\circ C $). The $T_w'(t)$ is the first directive of the function $T_w(t)$ respect time. The $T_c$ is the temperature of the heating coil (°C), and the $\tau_w$ is the ODE parameter for water related to decay time (s). We can later isolated the $T_w'(t)$ to the left hand side, and move rest terms to right hand side of the equation. Then, we can get Equation~\ref{eq_nopcmderive}.
\begin{equation} \label{eq_nopcmderive}
	T_{w}'(t) = \frac{T_{c} - T_{w}(t)}{\tau_{w}}
\end{equation}

Base on Equation~\ref{eq_nopcmderive}, we can write it into a pure text-based form, and pass it to the Drasil code generator. Code~\ref{code_expliciteq} shows the how to encode the Example~\ref{ex_firstorderode} by writing the explicit equation. The details on how to use explicit equations to get numerical solutions for ODEs will be discussed on section~\ref{se_connecteetolib}.
\begin{listing}[ht]
\begin{haskell1}
-- Pesodu Code
...
In Code~\ref{code_expliciteq}, we encode Equation~\ref{eq_nopcmderive}, and pass it to code generator. The information is necessary for a ODE libraries to solve the ODE numerically. In Code~\ref{code_expliciteqsrs}, we also encode Equation~\ref{eq_nopcmderive}, and pass it to the Drasil printer. The information is necessary for printing ODEs in SRS. They both describe the same ODE, but we write it twice in the Drasil framework. Therefore, there is a information duplication. We can transform from Code~\ref{code_expliciteq} to Code~\ref{code_expliciteqsrs} with human interfere. However, without human interfere, we can not transfer the between two expressions because they are all in a text-based form. In order to reduce the information duplication, the Drasil team decide to make an advanced data structure to hold the ODE information.
