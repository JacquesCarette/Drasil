...
instance StatementSym lang => StatementSym (LoggingFor lang) where
  type Statement (LoggingFor lang) = Block lang

instance (BlockSym lang, AssignStatement lang, IOStatement lang, Literal lang, VariableElim lang) => AssignStatement (LoggingFor lang) where
  (&-=) = _
  (&+=) = _
  (&++) = _
  (&--) = _
  assign x e = do
    ...
    -- Total vibes here: not sure if the state
    -- needs to work like this, but this *feels* less dubious...
    LC <$> block
      [ assign (unLC <$> x) (unLC <$> e)
      , do
          x' <- variableName . unLC <$> zoom lensMStoVS x
          print $ litString $ "var '" <> x' <> "' assigned"
      , print (unLC <$> e)
