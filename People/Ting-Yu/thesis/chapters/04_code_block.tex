...
types, a \textbf{CodeBlock} is defined within \macblue{RawContent}, which 
requires a \macblue{CodeExpr} as shown in \ref{code:newRawContent}.
...
\macblue{CodeExpr} is a language that allows us to define code expressions. It 
shares similarities with \macblue{Expr} functions, constructors, and operators, 
...
type. For instance, to encode the code, we can use the \macred{unlbldCode} 
(Code~\ref{code:unlbldCode}) function, which converts a \macblue{CodeExpr} to 
\macblue{Contents}.
...
\macblue{RawContent}, is translated into a printable object, a 
\macblue{LayourObj}, before being processed by the document language printer. 
...
code cells, the \macred{codeCell} function generates the required metadata and 
...
To conclude this chapter, we can say that the benefits of using Jupyter 
