module GOOL.Drasil.AST (Terminator(..), ScopeTag(..), QualifiedName, qualName, 
  ...
  paramName, updateParam, ProgData(progName, progPurp, drasilVers, progMods), progD, emptyProg, 
...
data ProgData = ProgD {progName :: String, progPurp :: String, 
  drasilVers :: String, progMods :: [FileData]}

progD :: String -> String -> String -> [FileData] -> ProgData
progD n st vers fs = ProgD n st vers (filter (not . isEmpty . modDoc . fileMod) fs)
