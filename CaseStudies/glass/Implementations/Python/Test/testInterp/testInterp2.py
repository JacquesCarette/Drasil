...
class TestInterp(unittest.TestCase):
    ...
    def test_lin_interp(self):
        y = interp.lin_interp(3.5, 8.2, 1.3, 4, 2.1) # x in [x1,x2], x is a Float
        self.assertEqual(y, 4.892592592592592)

    def test_find_bounds(self):
        idx, jdx, kdx, num_interp1, num_interp2 = interp.find_bounds(self.data1, self.data2, 18, 3.86)
        self.assertEqual(idx, 2)
        self.assertEqual(jdx, 5)
        self.assertEqual(kdx, 1)
        self.assertEqual(num_interp1, 0)
        self.assertEqual(num_interp2, 1)

    def test_interp(self):
        interp_value = interp.interp(2, 5, 1, 0, 1, self.data1, self.data2, self.data3, 18, 3.86)
        self.assertEqual(interp_value, 4.66)
