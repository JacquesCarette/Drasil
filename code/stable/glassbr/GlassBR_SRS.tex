...
\item[A8:]With reference to A4 the value of load duration factor ($LDF$) is a constant in GlassBR. It is calculated by the equation: $LDF=(\frac{t_{d}}{60})^{\frac{m}{16}}$. Using this, $LDF=0.27$.
\end{itemize}
...
\\
$b$ & $b$ > 0 and $b$ < $a$ & $d_{min}$ <= $b$ <= $d_{max}$ and $a$/$b$ < $AR_{max}$ & 1200 mm & 10%
\\
...
$TNT$ & $TNT$ > 0 & - & 1 & 10%
\\
$SD$ & $SD$ > 0 & $SD_{min}$ < $SD$ < $SD_{max}$  & 45 m & 10%
\\
\bottomrule
