...
newtonLUGModel = equationalRealm' $ mkMultiDefnForQuant newtonForceQuant EmptyS $ NE.fromList [
    mkDefiningExpr "newtonLUGviaDeriv" [] EmptyS (sy gravitationalConst `mul` (sy mass_1 `mul` sy mass_2 $/ square (sy dispNorm)) `mul` sy dVect),
    mkDefiningExpr "newtonLUGviaForm"  [] EmptyS (sy gravitationalConst `mul` (sy mass_1 `mul` sy mass_2 $/ square (sy dispNorm)) `mul` (sy distMass $/ sy dispNorm))
  ]
...
-- sentence, supposed to include "6.673 `mul` 10^{-11} m/kgs^2" (line 187)).
...
newtonSLRExpr :: PExpr
newtonSLRExpr = sy momentOfInertia `mul` sy angularAccel
