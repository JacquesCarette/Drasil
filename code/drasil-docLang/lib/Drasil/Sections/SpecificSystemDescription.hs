...
specSysDescr = SRS.specSysDes "specSysDes" 0 [intro_]
...
probDescF prob = SRS.probDesc "specSysDes" 1 [mkParagraph $ foldlSent [atStartNP (a_ system) `S.is` S "needed to", prob]]
...
termDefnF end lst = SRS.termAndDefn "probDesc" 2 [intro, enumBulletU $ map termDef lst]
...
termDefnF' end otherContents = SRS.termAndDefn "probDesc" 2 (intro : otherContents)
...
physSystDesc progName parts fg other = SRS.physSyst "probDesc" 2 (intro : bullets : LlC fg : other)
...
goalStmtF givenInputs otherContents = SRS.goalStmt "probDesc" 2 (intro:otherContents)
...
assumpF otherContents = SRS.assumpt "solCharSpec" 2 (assumpIntro : otherContents)
  where
...
thModF progName otherContents = SRS.thModel "solCharSpec" 2 (thModIntro progName : otherContents)
...
genDefnF otherContents = SRS.genDefn "solCharSpec" 2 (generalDefinitionIntro otherContents : otherContents)
...
dataDefnF endingSent otherContents = SRS.dataDefn "solCharSpec" 2
...
inModelF probDes datDef theMod genDef otherContents = SRS.inModel "solCharSpec" 2
...
datConF :: (HasUncertainty c, Quantity c, Constrained c, HasReasVal c, MayHaveUnit c) => 
  Sentence -> [c] -> Section
datConF _ [] = SRS.datCon "solCharSpec" 2 [mkParagraph (S "There are no" +:+. plural datumConstraint)]
datConF t c  = SRS.datCon "solCharSpec" 2 [dataConstraintParagraph t, LlC $ inDataConstTbl c]
...
propCorSolF []  [] = SRS.propCorSol "solCharSpec" 2 [mkParagraph noPropsSent]
propCorSolF [] con = SRS.propCorSol "solCharSpec" 2 con
propCorSolF c  con = SRS.propCorSol "solCharSpec" 2 ([propsIntro, LlC $ outDataConstTbl c] ++ con)
...
tmStub = SRS.thModel  "solCharSpec" 2 [] 
ddStub = SRS.dataDefn "solCharSpec" 2 []
imStub = SRS.inModel  "solCharSpec" 2 []
pdStub = SRS.probDesc "specSysDes"  1 []
